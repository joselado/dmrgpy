...
class Chain
    ...
    // "python" routes: a correct run stays at a ratio of at most 1,
    // truncated (kpmmaxm or energy truncation) or not. It used to be
    // 1e3*(bound+1.0), which let spectra up to 109 times the true peak
    // through below kpm_scale=1/2 and was an absolute threshold for
    // operators of small norm, the moments being bilinear in vi and vj
    // (2026-09-24b audit, finding 3). One moment per call: the
    // accelerated loops check both moments of their step. Valid for a
    // Hermitian operator, which is what every caller expands
    // (general_kpm's X included).
    //
