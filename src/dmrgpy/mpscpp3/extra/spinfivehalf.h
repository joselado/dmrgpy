...
//
// Port of mpscpp2/extra/spinfivehalf.h to the ITensor v3 API, see
...
class SpinFiveHalfSite
    ...
    SpinFiveHalfSite(int n, Args const& args = Args::global())
		{
        auto ts = TagSet("Site,S=5/2,n="+str(n));
...
read(std::istream& s)
    ...
    if(N > 0)
        ...
        for(int j = 1; j <= N; ++j)
            ...
            auto I = Index{};
            I.read(s);
            if(dim(I) == 6) store.set(j,SpinFiveHalfSite(I));
            else if(dim(I) == 2) store.set(j,SpinHalfSite(I));
            else throw ITError(tinyformat::format("SpinFiveHalf cannot read index of size %d",dim(I)));
            }
        init(std::move(store));
        }
	}
