...
def dmrg_BooB(indict,integrate_right=True):
    ...
    idc = sp.eye(hr.shape[0],dtype=np.complex128) # block identity operator
    idl = sp.eye(diml,dtype=np.complex128) # block identity operator
    idr = sp.eye(dimr,dtype=np.complex128) # block identity operator
    ...
    idr2 = sp.eye(hr.shape[0],dtype=np.complex128) # right identity operator
    idl2 = sp.eye(hl.shape[0],dtype=np.complex128) # right identity operator
