...
def dmrg_BooB(dmrgp,integrate_right=True,LO=True,dmrgout=None):
    ...
    # idensity operators
    id_lB = sp.eye(dim_lB,dtype=np.complex128) # identity operator
    id_rB = sp.eye(dim_rB,dtype=np.complex128) # identity operator
    id_lS = sp.eye(dim_lS,dtype=np.complex128) # identity operator
    id_rS = sp.eye(dim_rS,dtype=np.complex128) # identity operator
    id_rSB = sp.eye(dim_rS*dim_rB,dtype=np.complex128) # right identity operator
    id_lSB = sp.eye(dim_lS*dim_lB,dtype=np.complex128) # right identity operator
...
def initial_BooB(right,left,onsite,
  ...
  id_rS = sp.eye(dim_rS,dtype=np.complex128) # left identity operator
  id_lS = sp.eye(dim_lS,dtype=np.complex128) # left identity operator
  # initial Hamiltonian
