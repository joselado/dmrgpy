...
earlier records. Nothing here is fixed yet; each entry gains a `**Status**`
line when it is acted on, and keeps its repro. The fix pass that follows is
meant to take these findings together with the open items and leads the
earlier records carry, the 2026-09-25 record's "Left open, and new leads" in
particular.
...
| # | Finding | Lens | Severity | Introduced | Numbers change on fix |
|---|---|---|---|---|---|
| 1 | non-unit-norm set state | `construction`, `session` | MEDIUM | older, reach widened | yes |
| 2 | ED `gs_energy` drops keywords | `construction` | MEDIUM | older | yes |
| 3 | `mode="DMRG"` pin overrides `mode="ED"` | `construction` | MEDIUM | older | yes |
| 4 | `check_settings` admits non-settings | `construction` | LOW | e7b1196 | no |
| 5 | current chain swallows keywords | `construction` | LOW | older | no |
| 6 | no-Hamiltonian errors | `construction` | LOW | older | no |
| 7 | `julia_live` product-start trap | `session` | HIGH | older | yes |
| 8 | generalized-state origin split | `session` | MEDIUM | older | yes |
| 9 | NH `gs_energy(H=)` stored as current | `session` | LOW | older, reach widened | yes |
| 10 | `julia_live` generalized KPM window | `session` | LOW | older | no |
| 11 | `get_rdm` divides by the squared norm | `session` | LOW | older | yes |
| 12 | `julia_live` last-site `get_rdm` | `session` | LOW | older | no |
| 13 | CVM absolute CG tolerance | `scale` | HIGH | older | yes |
| 14 | CVM early exits on exact CG | `scale` | HIGH | older | yes |
| 15 | ED `dex` silent above the spectrum | `scale` | MEDIUM | older | yes |
| 16 | absolute A^dagger == B gate | `scale` | MEDIUM | older | yes |
| 17 | anneal early return | `scale` | MEDIUM | older | yes |
| 18 | anneal first-order drift | `scale` | MEDIUM | older | yes |
| 19 | anneal zero steps above T=5 | `scale` | MEDIUM | older | yes |
| 20 | ED deflation partner window | `scale` | LOW | older | yes |
| 21 | ROOTN absolute Lanczos breakdown | `scale` | LOW | older | yes |
| 22 | ED `normalize()` floor | `scale` | LOW | older, reach widened | no |
| 23 | KPM `same_mps` absolute test | `scale_cpp` | MEDIUM | older, reach widened | yes |
| 24 | v3 VUMPS residual floor | `scale_cpp` | MEDIUM | older | yes |
| 25 | NH SRTieBreak window | `scale_cpp` | MEDIUM | older | yes |
| 26 | NH certificate units | `scale_cpp` | MEDIUM | older | no |
| 27 | v3 Arnoldi absolute stops | `scale_cpp` | LOW | older | yes |
| 28 | `hscale_up_` from raw terms | `scale_cpp` | LOW | e7b1196 | yes |
| 29 | verbose log of the scaled energy | `scale_cpp` | LOW | e7b1196 | no |
