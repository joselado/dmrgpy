...
than expanding in Chebyshev moments. Requires
\texttt{itensor\_version="python"} (calls \texttt{gs\_energy()}
automatically if needed, like \texttt{vev}/\texttt{correlator}):
...
evolved forward in time; \texttt{opname\_i} is inserted at the shifted
...
calls) would need. \texttt{connected=True} (default) subtracts the
