...
since this is the one place the guide invites you to tune \texttt{maxm}
by watching it. On a 10-site Heisenberg chain at the stock
\texttt{maxm=30}, \texttt{itensor\_version=3} reports 1.7e-07 and
\texttt{itensor\_version="python"} 6.3e-06 --- \texttt{"python"}'s floor
is its Lanczos eigenvector accuracy ($\sim10^{-6}$; its ground-state
solver stops on the Ritz \emph{value}, whose error is quadratic in the
eigenvector error), not the double-precision cancellation floor the C++
backend reaches. A \texttt{"python"} fluctuation that stops falling at
$10^{-6}$ has hit that cap, not a bond-dimension limit.
