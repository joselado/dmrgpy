...
hoppings and $A=c^\dagger_0$, $B=c_2$: the exact $\langle AB\rangle$ is
$0.1105-0.2714i$; \texttt{submode="KPM"} integrates to that within 9e-11
under \texttt{mode="ED"} and 3.8e-06 under \texttt{mode="DMRG"}, while
...
Section~\ref{sec:whatraises}). Do not
read that as \texttt{mode="ED"} and \texttt{mode="DMRG"} agreeing
pointwise in general: they share a convention, not a kernel or a
resolution. The default \texttt{submode="KPM"} in particular still
differs between the two routes by an amount comparable to the peak ---
on the 6-site Heisenberg chain with $A=B=S^z_0$ the two curves differ by
a sizeable fraction of the peak height at $\delta=0.1$--$0.3$. No
absolute figure is quoted here on purpose: the number depends on the
$\omega$ grid (a spacing coarser than $\delta$ samples the peaks rather
than resolving them), and three independent measurements on three grids
gave three different answers. That is a resolution difference, not a convention one ---
both integrate to $\langle AB\rangle$, as the sum-rule measurement above
shows. The one qualifier is
the real-time pair \texttt{submode="TD"}/\texttt{"TDZ"}, which returns
the complex one-sided Fourier transform rather than the density itself:
see that section's own formula and the note there. Otherwise they differ
...
Note the $\mathrm{Re}$ --- and that the returned array is the
\textbf{whole complex transform, not its real part}, so it is not the
density this section defines and does not become the density for a
Hermitian pair either. A one-sided transform of $C(t)$ is a
\emph{resolvent}, not a spectral density: in the $T\to\infty$ limit what
comes back is $-\frac{i}{\pi}G^A_{AB}(\omega)$, whose real part is
$\frac1\pi\mathrm{Im}\,G^A_{AB}$, and that real part equals the density
whenever $M_n$ is real, since
$\mathrm{Im}\,G^A=-\mathrm{Im}\,G^R$ only holds there. \textbf{So take
\texttt{.real}} if you want the same quantity the other submodes return;
the imaginary part is the dispersive term, and it is not small. Measured
on the 6-site Heisenberg chain with the Hermitian pair $A=B=S^z_0$
($\delta=0.3$, \texttt{nt=2000}, \texttt{dt=0.02}, density peak 0.1421):
max $|\mathrm{Im}\,y|$ is 9.96e-02, i.e.\ 70\% of the peak, and the
array as returned sits that same 9.96e-02 from the density --- while its
real part matches the density to 4.0e-05 and the array as a whole
reproduces $-\frac{i}{\pi}G^A_{AB}$ to 5.5e-05. On a pair with complex
$M_n$ not even the real part is the density, so cross-check
\texttt{"TD"}/\texttt{"TDZ"} there against \texttt{submode="KPM"} or
\texttt{"ED"} rather than reading it directly.
...
\texttt{NotImplementedError} under \texttt{mode="ED"}.
...
0.2\% of its maximum at every bias point on the same chain. It was
...
findings, 35 of them now fixed; the one exception is marked PARTIAL
there (the iDMRG matrix-free item, whose compute half landed and whose
memory half did not). The list below is the physics-facing
...
and \texttt{"SECTOR"} are untouched. \texttt{mode="ED" submode="ROOTN"}
