...
\lstinline|dmrgpy.pyitensor.nhdmrg.nhdmrg_generalized| is the underlying
routine; no ITensor v3 port yet (unlike the Hermitian case above), so
\lstinline|itensor_version=3| raises \lstinline|NotImplementedError| for
a non-Hermitian $H$ specifically. See
\texttt{examples/non\_hermitian/nhdmrg\_generalized\_benchmark}, which
heads it up against \lstinline|mpsiram_generalized| on a non-Hermitian
...
inverse, \lstinline|nhdmrg_generalized| is both far more accurate and (as
the chain grows) up to two orders of magnitude faster.
