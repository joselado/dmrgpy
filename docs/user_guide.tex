...
routine --- and on compiled ITensor v3 (\lstinline|itensor_version=3|,
...
instead of pyitensor's hand-rolled two-site sweep.
\lstinline|itensor_version=2| (mpscpp2) and \lstinline|julia_live| don't
have this session method yet and raise \lstinline|NotImplementedError|.
...
(\lstinline|dmrgpy.pyitensor.nhdmrg.nhdmrg_generalized|) and compiled
ITensor v3 (\lstinline|Chain::nhdmrg_generalized|,
\texttt{mpscpp3/chain\_session.h}, a line-for-line port against this
file's own hand-rolled two-site sweep) --- \texttt{mpscpp2}
...
genuinely useful, narrower regime. Available only for
\texttt{itensor\_version="python"} (there is no C++ v2/v3 port);
requesting it on another backend is silently a no-op (the existing,
always-safe rescaling is used instead). Available only for the
$(A,B)$-operator dynamical correlator, not the lower-level ``arbitrary
operator'' KPM (\texttt{general\_kpm}/\texttt{kpm\_wfa\_wfb}), which has
no ground-state reference to anchor to. See
...
purpose) and \texttt{src/dmrgpy/pyitensor/kpm\_energy\_truncation.py}
...
\textbf{A genuinely infinite-chain KPM is possible in principle, but not
implemented here.} Two separate ingredients would be needed, both
reusing machinery this module already has: (1) cap the window's two
ends with the \emph{converged environment} (the \texttt{HL}/\texttt{HR}
accumulated Hamiltonian blocks \texttt{idmrg\_ground\_state} already
builds during growth, or an equivalent construction from the
fixed-point machinery \texttt{apply\_mpo}/\texttt{imps\_overlap} already
use) instead of plain open boundaries -- infinite boundary conditions
(IBC, Phien/McCulloch/Vidal, PRB 86, 245107 (2012)) -- which removes a
real error source \texttt{n\_window} alone cannot fix: an open chain's
own ground state carries boundary artifacts that contaminate even the
\emph{central} region, not just the two edges; (2) independently, let
the \emph{active} window grow by roughly one site per Chebyshev moment
as the recursion proceeds (reusing the converged unit-cell tensors to
extend the state on demand) rather than fixing a static
\texttt{n\_window} up front, removing the
\texttt{n\_window}-vs-moment-count constraint entirely and bounding
cost only by the usual bond-dimension budget of any DMRG calculation.
Assembling these into a working implementation is a substantially
larger feature than \texttt{kpm\_finite} -- flagged as a known,
deliberate follow-up (see \texttt{kpm\_finite}'s own docstring for the
same note in more detail).
