...
\texttt{tc.MBChain}. A keyword that is not a setting (a misspelling, a
private name, a method, the chain's state such as \texttt{hamiltonian},
\texttt{conserved\_sector}, \texttt{wf0}, \texttt{e0} or
\texttt{computed\_gs}, or an operator list the model class builds, as in
\texttt{Fermionic\_Chain(4, N=5)}) raises \texttt{TypeError} naming every
offending key. Until the 2026-09-25 fixes every such keyword was dropped,
so a bond dimension asked for at construction silently ran at
\texttt{maxm=30} (\S21). Note that \texttt{cvm\_maxm} is set from the
...
\texttt{sc.operator\_norm(A)} & an estimate of $\lVert A\rVert$; \texttt{sc.is\_zero\_operator(A)} thresholds it at $10^{-4}$ \\
\bottomrule
\end{tabular}
\end{center}
...
energy lost digits. What one scale cannot reach is a hierarchy inside the
...
altogether. Four more routes are not scale-free, and on those the
...
\texttt{"python"} and v3 stopping on an absolute error), NH-DMRG on v2,
and the Lanczos of \texttt{"python"}, which stops on an absolute test
($1.2\times10^{-6}$ relative at $J=10^{-9}$). \texttt{mode="ED"} is
unaffected by any of this, and \texttt{"julia\_live"} by the units.
...
\texttt{None}, with a warning, if the norm is below \texttt{tol}),
\texttt{wf.get\_conjugate()} the complex conjugate state, and
...
\lstinline|get_gs()| takes the same \lstinline|wf0=|/\lstinline|reconverge=|
keywords as \lstinline|gs_energy()| and returns the stored state under the
same condition, a current state and no \lstinline|wf0=|:
\lstinline|get_gs(wf0=x)| sweeps from x, and
\lstinline|get_gs(wf0=x, reconverge=False)| returns x itself with
\lstinline|e0| = $\langle x|H|x\rangle$, on a solved chain as on a fresh
one, and every later reader measures x. Until the 2026-09-25 fixes a
solved chain returned its stored state without reading x.
...
instead takes the state as it is, unswept), the ramp's starting bond
...
generalized solve. A bare \lstinline|gs_energy()| afterwards returns
$\lambda$, since the stored state is current, so call
...
suspicious \texttt{"CVM"} curve. It assumes $A^\dagger=B$ and raises
\texttt{NotImplementedError} naming the submode otherwise (it used to be
a bare \texttt{RuntimeError: No active exception to reraise} after a
bare \texttt{print}). On a non-Hermitian $H$ it is one of the two
submodes (with \texttt{"CVM"}) that has a genuine non-Hermitian
implementation.
...
\lstinline|reconverge=False| takes x as it is, on every session backend.
...
against $-0.0694$). Note that a dynamical correlator of the purified state
...
\texttt{get\_rdm()} on any ED route & \texttt{AttributeError} several frames deep & \texttt{NotImplementedError} \\
the bond entanglement entropy on any ED route & \texttt{AttributeError} several frames deep & \texttt{NotImplementedError} pointing at \texttt{get\_site\_\allowbreak entropy}, \texttt{get\_pair\_\allowbreak entropy} \\
...
\texttt{submode="CVM\_explicit"} with $A^\dagger\neq B$ & a \texttt{print} then \texttt{RuntimeError: No active exception to reraise} & \texttt{NotImplementedError} naming the submode \\
...
\item \textbf{\texttt{get\_gs(wf0=x, reconverge=False)} on a solved
  ...
  \texttt{get\_gs(wf0=x)} sweeps from x (\S3, get-gs-wf0).
