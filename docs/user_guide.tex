...
routine; the compiled ITensor v2/v3 sessions and \lstinline|julia_live|
raise \lstinline|NotImplementedError|. See
\texttt{examples/groundstate/dmrg\_generalized\_benchmark}, which heads
it up against \lstinline|mpsiram_generalized| on the same
interacting-fermion-chain test problem: needing no approximate inverse,
the DMRG route is both noticeably more accurate and (since ARPACK mode
2's own correction-vector solve gets more expensive, and less accurate,
as the chain grows) an order of magnitude or more faster on an 8-site
chain.
