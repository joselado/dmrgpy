...
chains (BLAS pinned to one thread), sector mode is \textbf{0.6x} --- i.e.
...
Sector targeting requires \lstinline|itensor_version=3| and DMRG. A
...
\lstinline|metts_dynamical_correlator|), \lstinline|tevol_method="TEBD"|,
and the infinite-chain algorithms (iDMRG, VUMPS). Those assemble tensors
by hand in a way that is dense-only, so they refuse rather than produce a
wrong number. See
...
\emph{after} the expensive sweeps, not before. The Hamiltonian and the
...
\lstinline|promote_to_dense()| is \lstinline|itensor_version=3| only,
