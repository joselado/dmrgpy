...
default when the compiled C++ extension is available, \texttt{"python"}
otherwise) or \texttt{"python"} for DMRG/ED results to actually agree;
...
\texttt{sfc.set\_hoppings\_spinful(fun)} is the same hopping,
spin-diagonal, with \texttt{fun} indexed by \emph{physical} site rather
than by interleaved spinless site; and
\texttt{sfc.set\_swave\_pairing(fun)} adds
$\sum_i\Delta(i)\,c_{i\uparrow}c_{i\downarrow}+\mathrm{h.c.}$
...
Note that \texttt{set\_swave\_pairing}'s \texttt{fun} takes a
\textbf{single} argument (the physical site index), unlike the
two-argument functions the others want --- passing
\texttt{lambda i,j: ...} raises \texttt{TypeError}.
...
\texttt{mode=}/\texttt{**kwargs} as the rest of the API:
...
explicitly.} Computes the same $G_{AB}(\omega)$ as
...
otherwise. On a non-Hermitian $H$ it is one of the two submodes (with
\texttt{"CVM"}) that has a genuine non-Hermitian implementation.
...
\textbf{Currently non-functional.} \texttt{meanfield.py} reads the
exchange couplings back off the chain as \texttt{sc.exchange}, which was
populated by \texttt{Spin\_Chain.set\_exchange(fun)}. That builder was
removed in favour of writing the Hamiltonian out explicitly with
\texttt{SS(i,j)} and \texttt{set\_hamiltonian()}, so \texttt{sc.exchange}
is now always the integer \texttt{0} and \texttt{spinchain\_meanfield}
raises \texttt{TypeError: 'int' object is not iterable} on any chain. The
same removal leaves \texttt{Spin\_Chain.set\_fields(fun)} unsafe: it
assigns \texttt{self.hamiltonian = self.exchange + self.fields} directly,
so it silently \emph{replaces} the Hamiltonian you built with the field
term alone, instead of adding to it. Build the Weiss field into the
Hamiltonian by hand (\texttt{h = h + b[2]*sc.Sz[i]}, then
\texttt{set\_hamiltonian(h)}) and iterate self-consistency in your own
loop until both are fixed.
