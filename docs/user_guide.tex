...
comparison above was only run for \texttt{"TD"}. What they no longer
share is the last step: \texttt{"TD"} and \texttt{"TDZ"} evaluate the
damped sum directly at each requested frequency, while
\texttt{sxt\_to\_skomega} stays on the FFT grid, since at its defaults
$\delta T=1$ and the direct evaluation is unmeasured there.
...
\texttt{delta} from $E_0$, and \texttt{n\_gs>1} raises
\texttt{NotImplementedError} on \texttt{itensor\_version=2} and
...
\texttt{potentialdc}'s docstrings). The grid may be non-uniform, since
both terms integrate with trapezoid weights of the grid they are given;
the potential term weighted every point with the first spacing until the
...
the \emph{same} damping/FFT tail (\texttt{delta} $\to$
Lorentzian broadening) \texttt{submode="TD"} uses, with one deliberate
difference at the end of it: what comes back is the raw one-sided
...
\begin{itemize}
  \item \texttt{backend.set\_pad\_bonds(K)} (with $K$ your bond
    ...
    stripped once at trajectory entry, so a padded \texttt{TDVP\_GSE}
    run follows the unpadded one to roundoff
...
infinite-chain $S(k,\omega)$ reduction, stays on the FFT stage and
returns the same bits as before (Section~\ref{sec:dynamical}, finding
7).
