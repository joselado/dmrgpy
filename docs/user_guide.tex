...
and \texttt{"TEBD"}; the legacy \texttt{"MPO"} path raises
...
\textbf{An alternative ground-state solver: VUMPS.}
\texttt{ic.gs\_method = "vumps"} (default \texttt{"idmrg"},
\texttt{itensor\_version="python"} only) switches \texttt{gs\_energy()}
from the growing algorithm above to VUMPS (Variational Uniform Matrix
Product States, Zauner-Stauber et al., arXiv:1701.07035; see
\texttt{pyitensor/vumps.py}'s own module docstring for the algorithm)
-- instead of growing a finite window and truncating it down to
\texttt{maxm} at every step, VUMPS solves directly, in the
thermodynamic limit, for the actual \texttt{maxm}-dimensional
variational optimum (\texttt{ic.maxm} sets VUMPS's own target bond
dimension $D$ here too):
...
Like \texttt{itensor\_version=3}, \texttt{gs\_method="vumps"} supports
\texttt{gs\_energy()} only -- \texttt{vev}/\texttt{correlator}/
\texttt{excitation\_energies}/\texttt{excitation\_gap}/
\texttt{local\_excitation\_gap} all raise \texttt{NotImplementedError}
(VUMPS's own converged state is a grouped-supersite representation with
no per-sublattice \texttt{U\_list}, see
\texttt{pyitensor.vumps.VUMPSResult}'s own docstring).
