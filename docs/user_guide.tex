...
carry, a same-site identity ($S^xS^x=1/4$) or two factors on one site
commuting without both being diagonal. When a chain is at hand,
...
\texttt{mode="DMRG"}, so the two routes sit a few per cent of the peak
apart instead of a whole peak apart. On a 6-site Heisenberg chain with
$A=B=S^z_0$ at $\delta=0.2$, over
\texttt{es=np.linspace(0.01,4,120)}, the two curves sit 1.24e-02 apart
...
O2). What remains between KPM and a resolvent submode is the kernel
...
for the same $\delta$. \texttt{kpm\_n\_scale} is a multiplier on that calibrated
count, default 1: raise it to buy a line sharper than the $\delta$ you
asked for, at proportionally higher cost. Both the calibration and that
default are new in the 2026-09 audit's open item O2
...
near-Gaussian rather than Lorentzian, so at equal FWHM and equal
integrated weight its peak stands about 1.6x higher than a resolvent
submode's, measured 0.315 against 0.195 on a 6-site Heisenberg chain at
$\delta=0.2$ with both integrating to the same 0.250000. One guard rail
...
second evolution and never a wrong number. Note that the cost
...
two. What \texttt{"TDZ"} keeps, and \texttt{"TD"} does not, is an error
of its own: on the 2026-09 audit's 4-site complex-hopping chain
($A=c^\dagger_0$, $B=c_2$, $\delta=0.4$, \texttt{dt=0.1}, exact peak
0.2313, \texttt{itensor\_version=3}) it lands 3.31e-02 from the exact
Lehmann density where \texttt{"TD"} lands 2.57e-04. That residual is the
complex-time contour plus the Taylor-in-$\alpha_0$ reconstruction, the
same 1.8e-02 the audit measured on a Hermitian pair, and it is not a
convention question: reduce it with a smaller \texttt{alpha0} or a
larger \texttt{n\_max}, not by post-processing the array.
...
computed at one value of it, not across values.
...
most one direction out of a degenerate eigenspace, so the C++ solver runs
one deflated Lanczos per eigenvalue rather than one Lanczos for all of
them. A plain single-vector Lanczos there returns distinct eigenvalues
where two should coincide, every one of them a genuine eigenpair, so a
residual check passes it; \texttt{"python"} never had that exposure,
since ARPACK's restarts recover the second copy.
...
\texttt{mode="ED"}, and from 1.13e-01 to 3.31e-02 for \texttt{"TDZ"},
whose own contour error the fix does not touch. On a Hermitian pair the
...
95 to 124 per cent down to 4 to 7 per cent. Peak heights change with the
