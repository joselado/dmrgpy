...
\midrule
\texttt{Fermionic\_Chain}, \texttt{Spinful\_\allowbreak Fermionic\_Chain} (Jordan--Wigner) & \texttt{Nf} \\
...
Sector targeting requires DMRG and either
\lstinline|itensor_version=3| or \lstinline|itensor_version="python"|. A
sector-mode chain deliberately raises rather than falling back to ED (or
to any other backend), since a solver without quantum numbers would
silently answer with the \emph{global} ground state instead.
...
\textbf{\texttt{itensor\_version=3} currently raises here, on purpose.}
The v3 window tiles the raw per-micro-step \texttt{idmrg\_U\_} factors
rather than the gauge-consistent unit cell every other v3 static
observable uses, so its $S(x,t)$ is quantitatively wrong for \emph{any}
operator, spin included: the exact $S(x,0) = $
\texttt{correlator(...)} identity misses by up to $\sim$1.7e-1 on a plain
Heisenberg chain whose energy density the two backends agree on to
6.7e-11. Shape and finiteness look fine, which is how it went unnoticed.
Rather than hand back numbers like that,
\texttt{td\_dynamical\_correlator} raises \texttt{RuntimeError} for
\texttt{itensor\_version=3} and
\texttt{Chain::td\_dynamical\_correlator\_window} throws unless a caller
explicitly opts in (\texttt{Chain::set\_allow\_defective\_window}, used
only by the tests that pin the defect). Use
\texttt{itensor\_version="python"}, which is exact on that identity ---
see \texttt{docs/known\_issue\_v3\_window\_gauge.md} for the diagnosis and
the fix, and
\texttt{examples/idmrg/td\_dynamical\_correlator\_python\_VS\_v3/main.py},
which now measures the defect instead of asserting agreement.
