...
along (it builds its own finite window), and so do
\texttt{vev}/\texttt{correlator} on \texttt{itensor\_version="python"}.

Two things do not follow along.
...
so rewrite it on a longer unit cell if you need the excitations. And on
\texttt{itensor\_version=3}, \texttt{vev}/\texttt{correlator} raise for
anything the sequential solver answered -- a pre-existing gap of that
backend (\texttt{Chain::vumps\_onsite\_expectation}/
\texttt{vumps\_two\_point\_correlator} read the grouped snapshot, and
\texttt{vms\_ground\_state} has never had a static-observable port) that
was previously reachable only at \texttt{n\_uc>2} and that a long-range
Hamiltonian at \texttt{n\_uc<=2} now reaches too; use
\texttt{itensor\_version="python"} for those.
