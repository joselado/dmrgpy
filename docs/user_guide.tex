...
\texttt{fermionchain.\allowbreak Spinful\_\allowbreak Fermionic\_\allowbreak Chain\_\allowbreak Native} & same physics, genuinely 4-dimensional local space, one site per physical site (\texttt{itensor\_version=3} and \texttt{"python"} only) & identical operator lists/formulas as \texttt{Spinful\_Fermionic\_Chain} \\
...
\texttt{bosonchain.\allowbreak SpinBoson\_Chain} & mixes truncated-boson sites and genuine spin-$S$ sites in the same chain, one entry per location (the constructor takes the list of site labels, e.g.\ \texttt{SpinBoson\_Chain(["B",\allowbreak "S=1/2",\ldots])}) & boson location: $a_i,a_i^\dagger,n_i$ and the occupation projectors \texttt{sb.D[i][k]} (plus \texttt{D0}..\texttt{D3} when every boson location has $\ge4$ levels); spin location: $S^x_i,S^y_i,S^z_i$. As in \texttt{Mixed\_Spin\_Fermion\_Chain}, operators that do not apply at a location read as the integer \texttt{0} \\
...
\texttt{itensor\_version=3} and \texttt{itensor\_version="python"} (the
two DMRG backends this class wires up; \texttt{itensor\_version=2} and
\texttt{"julia\_live"} have no native spinful site). Despite halving the site count, it is \emph{not} generally faster
...
$\sim30\%$ win). Leave \texttt{ctmode} at its default for this class ---
the resolver already picks the fastest available method
(\texttt{"fold"} on \texttt{itensor\_version=3}, \texttt{"batched"}
under \texttt{"python"}), both of which are faster than the
\texttt{"full"} this comparison was made against. Otherwise prefer \texttt{Spinful\_Fermionic\_Chain}; no other
...
\texttt{get\_boson\_site()} factory) -- \texttt{itensor\_version=2}
understands only the single fixed 4-level boson site (ITensor's
\texttt{BosonFourSite}), and now says so: any \texttt{maxnb} entry other
than 4 raises \texttt{ValueError} naming
\texttt{itensor\_version=3}/\texttt{"python"} as the alternatives (with
\texttt{mode="ED"} on either of them for exact diagonalization, since
\texttt{mode="ED"} next to \texttt{itensor\_version=2} does not avoid the
check: a constructor keyword is applied after the session is built),
\emph{before} a session is built. It used to abort the
whole interpreter with SIGABRT from inside ITensor instead, which no
Python code could catch. So run a non-default \texttt{maxnb} under
\texttt{itensor\_version=3} (the default when the compiled C++ extension
is available, \texttt{"python"} otherwise) or \texttt{"python"}; see
\texttt{examples/boson\_models/boson\_maxnb\_v3\_VS\_ED}. On a 3-site
chain at \texttt{maxnb=[3,5,3]} the \texttt{"python"} DMRG and ED
ground-state energies agree to machine precision (measured 8.9e-15).
\texttt{itensor\_version=3} agrees on the same physics but neither to
that tolerance nor reproducibly: it starts from a random MPS with QNs
off, so its agreement varies run to run --- three runs of that example
here disagreed by 1.05e-03, 1.78e-03 and 1.88e-03, each one over its own
\texttt{tol = 1e-3}. Expect a loose agreement from v3 here, not a tight
one, and re-run before concluding anything from a single number. The
same restriction is \emph{believed} to hold for
\texttt{itensor\_version="julia\_live"}, but that has not been verified
here and is not guarded -- treat it as unchecked rather than as a
documented limitation.

\texttt{SpinBoson\_Chain(sitesin, maxnb=None)} takes one label per
location: a spin label (\texttt{"S=1/2"}, \texttt{"1/2"},
\texttt{"S=1"}, \ldots\ as in \texttt{Spin\_Chain}) or a boson label,
which is \texttt{"B"} for the default 4-level site or \texttt{"B<k>"}
(\texttt{"B6"}) to name the local dimension --- \texttt{"B4"} and
\texttt{"B"} are the same site. \texttt{maxnb=} is the alternative
spelling of the same thing and is honored rather than dropped: one entry
per location with \texttt{None} at the spin positions, so
\texttt{SpinBoson\_Chain(["B","S=1/2"], maxnb=[6,None])} and
\texttt{SpinBoson\_Chain(["B6","S=1/2"])} both build sites
\texttt{[106, 2]}. A list of the wrong length, an \texttt{n=} that
contradicts \texttt{len(sitesin)}, a \texttt{maxnb} at a spin position,
and an unrecognized label all raise \texttt{ValueError} naming what was
wrong (an unknown label used to surface as \texttt{RuntimeError: No
active exception to reraise}). Per-site occupation projectors are
\texttt{sb.D[i][k]}, an empty list at a spin location, mirroring
\texttt{Bosonic\_Chain}. This class has no ED backend:
\texttt{mode="ED"} raises \texttt{NotImplementedError} pointing at
\texttt{mode="DMRG"} (\texttt{itensor\_version=3} or \texttt{"python"}),
or at \texttt{Bosonic\_Chain} for a purely bosonic model.

\texttt{itensor\_version=} is now accepted by \emph{every} chain
constructor. \texttt{Bosonic\_Chain}, \texttt{SpinBoson\_Chain},
\texttt{Parafermionic\_Chain} and
\texttt{spinfermionchain.Spin\_Fermion\_Hamiltonian} used to raise
\texttt{TypeError: unexpected keyword argument} for it --- which is
awkward precisely where the paragraph above tells a boson user to pick a
backend --- and had to be constructed first and switched afterwards with
\texttt{setup\_python()} / \texttt{setup\_cpp(version=3)}. Both
spellings work now.
...
\texttt{Spin\_Chain(sites, maxm=60, nsweeps=20)} is
\texttt{sc = Spin\_Chain(sites)} followed by
\texttt{sc.maxm, sc.nsweeps = 60, 20}, through the same checks
(\texttt{maxm=0} raises \texttt{ValueError}). This holds for
\texttt{mode=} too: \texttt{Spin\_Chain(sites, mode="ED")} builds its DMRG
session all the same, so \texttt{sc.mode = None} later goes back to DMRG,
and \texttt{Thermal\_Spin\_Chain(sites, T, mode="ED")} sets the wrapper's
own \texttt{tc.mode}, which its \texttt{get\_gs()} hands to
\texttt{tc.MBChain}. A chain's \texttt{mode="ED"} overrides the
\texttt{mode=} of every call made on it, while \texttt{mode="DMRG"} on the
chain means what no mode means, so an explicit \texttt{mode="ED"} call is
still answered by ED; until the 2026-09-25b pass a chain set to
\texttt{"DMRG"} answered that cross-check by DMRG, and
\texttt{Thermal\_Spin\_Chain} now defaults to \texttt{mode=None}. The
settings are an explicit list, \texttt{sites.SETTINGS}, and a keyword that
is not one of them (a misspelling, a private name, a method, the chain's
state such as \texttt{hamiltonian}, \texttt{conserved\_sector},
\texttt{wf0}, \texttt{e0} or \texttt{computed\_gs}, the Hamiltonian pieces
\texttt{hopping}, \texttt{hubbard}, \texttt{pairing} and
\texttt{exchange} that the \texttt{set\_*} methods accumulate, or an
operator list the model class builds, as in
\texttt{Fermionic\_Chain(4, N=5)}) raises \texttt{TypeError} naming every
offending key, \texttt{Thermal\_Spin\_Chain} included. Until the
2026-09-25 fixes every such keyword was dropped, so a bond dimension asked
for at construction silently ran at \texttt{maxm=30} (\S21), and until the
2026-09-25b pass nine public attributes that are not settings were
admitted, so \texttt{Fermionic\_Chain(4, hubbard=2.0)} followed by
\texttt{set\_hoppings()} added the constant $2\,\mathrm{Id}$ to the
free-fermion Hamiltonian ($-0.236068$ against $-2.236068$). Note that
\texttt{cvm\_maxm} is set from the
default \texttt{maxm} when the chain is built, so it stays 30 unless it is
given too, exactly as when \texttt{maxm} is assigned afterwards.
...
\textbf{Hamiltonian shortcuts on fermionic chains.} Rather than writing
every term out, the fermionic classes offer builders that take a function
of the site indices and add the whole family of terms at once:
\texttt{fc.set\_hoppings(fun)} adds $\sum_{ij}f(i,j)\,c_i^\dagger c_j$;
\texttt{fc.set\_hubbard(fun)} adds $\sum_{ij}f(i,j)\,n_in_j$, with
$n_i=n_{i\uparrow}+n_{i\downarrow}$ on a spinful chain;
and \texttt{sfc.set\_hoppings\_spinful(fun)} is the same hopping,
spin-diagonal, with \texttt{fun} indexed by \emph{physical} site rather
than by interleaved spinless site.
...
a \texttt{MultiOperator} into an already-built matrix product operator,
on every backend: a \texttt{StaticOperator} for
\texttt{itensor\_version} 2, 3 and \texttt{"python"}, an
\texttt{EDOperator} under \texttt{mode="ED"}, and \texttt{mpsjulialive}'s
own \texttt{MPO} for \texttt{itensor\_version="julia\_live"}. The
\texttt{+}, \texttt{-}, unary \texttt{-} and scalar
\texttt{*}/\texttt{/} algebra below is \texttt{StaticOperator}-only ---
the \texttt{julia\_live} \texttt{MPO} supports \texttt{*} alone --- so it
is available for \texttt{itensor\_version} 2, 3 and \texttt{"python"},
not yet for \texttt{"julia\_live"}. Two \texttt{StaticOperator}s can be
combined directly, without going back through the symbolic
\texttt{MultiOperator} form:
...
\textbf{MPS and operator algebra.} Every chain also exposes the
primitives that act on wavefunctions and operators directly, which the
worked examples later in this guide use (Section~\ref{sec:groundstate}'s
\texttt{promote\_to\_dense} example builds a photoemission weight out of
\texttt{applyoperator} and \texttt{overlap}). Each takes the same
\texttt{mode=}/\texttt{**kwargs} as the rest of the API, with one
qualifier: \texttt{applyoperator}, \texttt{summps},
\texttt{applyinverse}, \texttt{scale\_mps} and \texttt{exponential} take
the backend from the \emph{wavefunction's own type}
(\texttt{mps.MPS} $\to$ DMRG, \texttt{edtk.edchain.State} $\to$ ED),
because for these the wavefunction is the backend. An explicit
\texttt{mode=} there is a consistency check, not a switch: naming the
other backend raises \texttt{TypeError} telling you to rebuild the
wavefunction with \texttt{get\_gs(mode=...)} rather than silently
running the solver the state did not come from. \texttt{overlap},
\texttt{aMb}, \texttt{trace}, \texttt{inverse\_trace},
\texttt{operator\_norm} and \texttt{is\_zero\_operator} route on
\texttt{mode=} directly --- note \texttt{overlap}/\texttt{aMb} default to
\texttt{mode="DMRG"}, so handing them ED states without saying so still
fails inside the session rather than with a message.
...
Call & Returns \\
\midrule
\texttt{sc.overlap(a, b)} & $\langle a|b\rangle$ \\
\texttt{sc.aMb(a, M, b)} & $\langle a|M|b\rangle$ \\
\texttt{sc.applyoperator(A, wf)} & $A|\psi\rangle$ \\
\texttt{sc.applyinverse(A, wf)} & $A^{-1}|\psi\rangle$ (approximate --- see Section~\ref{sec:excited}) \\
\texttt{sc.exponential(h, wf)} & $e^{h}|\psi\rangle$ \\
\texttt{sc.summps(a, b)} & $|a\rangle+|b\rangle$ \\
\texttt{sc.scale\_mps(x, wf)} & $x|\psi\rangle$ (a single-tensor rescale, not an MPO sweep) \\
\texttt{sc.trace(A)} & $\mathrm{Tr}\,A$; \texttt{sc.inverse\_trace(A)} for $\mathrm{Tr}\,A^{-1}$ \\
\texttt{sc.operator\_norm(A)} & an estimate of $\lVert A\rVert$; \texttt{sc.is\_zero\_operator(A)} tests $A/c_{\max}$ against $10^{-20}$, $c_{\max}$ its largest coefficient (see below) \\
\bottomrule
\end{tabular}
\end{center}

\texttt{exponential(h, wf)} really is $e^{+h}$ on the DMRG backends now,
as it always was under \texttt{mode="ED"}. It was neither before: for a
\emph{multi-site} \texttt{h} --- every Hamiltonian-shaped operator,
since the symbolic Hermiticity test reported \texttt{False} for
$S^x_iS^x_j+S^y_iS^y_j+S^z_iS^z_j$ back then, which it no longer does
--- it fell through to an
uncontrolled two-term Taylor truncation (1.5\%/16\%/247\% relative error
at $z=0.25/0.5/1$ on a 4-site Heisenberg chain under
\texttt{itensor\_version="python"}, unbounded in $z$ and compounding if
used in a loop), and for a single-site \texttt{h}, where
the Hermitian branch did run, it computed $e^{-h}$. Anything built on an
anti-Hermitian argument, i.e.\ real-time evolution written as
\texttt{exponential(1j*dt*h, wf)}, was therefore evolving
\emph{backwards}. An operator that is neither Hermitian nor
anti-Hermitian now raises \texttt{NotImplementedError} on the DMRG
backends instead of printing a warning and returning a number; use
\texttt{mode="ED"} on a small chain for that case.

On the operator itself, \texttt{A.simplify()} returns it in canonical
form, the same operator with every term's factors sorted by site
(carrying the sign of the fermionic reordering) and equal terms
collected, and \texttt{A.is\_hermitian()},
\texttt{A.is\_antihermitian()} and \texttt{A.is\_zero()} read the answer
off that form. Those three are one-sided: \texttt{True} is a proof and
needs no calculation at all, while \texttt{False} only means the
canonical form did not collapse, which a Hermitian operator can survive
whenever its Hermiticity rests on something the operator names do not
carry, a same-site identity ($S^xS^x=1/4$), two factors on one site
commuting without both being diagonal, or a term that names both a
pre-Jordan-Wigner fermion (\texttt{C}, \texttt{Cdag}, \texttt{Cup},
\ldots) and a bare post-transform ladder operator (\texttt{A},
\texttt{Adag}, \texttt{Aup}, \ldots), which is left spelled as
written because the sign of exchanging the two depends on which sits on
the lower site. That last rule is new in the 2026-09-24 audit: such
terms used to be sorted with the wrong sign, so \texttt{simplify()}
changed their value and an exactly anti-Hermitian operator could be
proven Hermitian (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md},
finding 1); no chain class builds one, so no ordinary Hamiltonian lost
its proof. When a chain is at hand,
\texttt{sc.is\_hermitian(A)} answers the same question without that
caveat, taking the proof when it lands and probing numerically when it
does not, which is what \texttt{gs\_energy()} and
\texttt{exponential()} gate on. Both halves decide on $A/\max|c|$, $c$
running over the coefficients of $A$, so the answer is a property of the
operator and not of its units, and the probe resolves an anti-Hermitian
part down to about $10^{-10}$ of the largest coefficient; on
\texttt{mode="ED"} the same question is a relative test on the matrix,
$\lVert h-h^\dagger\rVert_F\le10^{-10}\lVert h\rVert_F$. Until the third
2026-09-24 pass the probe compared an unnormalized norm against an
absolute $10^{-4}$, so a weak loss term on an $O(1)$ Hamiltonian was
called Hermitian and its decay rate dropped (\S21).
\texttt{sc.is\_zero\_operator(A)} is relative in the same way since the
2026-09-25b pass: an operator with no terms, only zero coefficients or an
empty canonical form is zero outright, and otherwise $A/c_{\max}$,
$c_{\max}$ its largest coefficient, is probed against $10^{-20}$ on the
mean squared norm, and on \texttt{mode="ED"} the exact matrix is tested as
...
written at $10^{-2}$ read as zero; \texttt{cvm\_solver="variational"}
asks it whether $A^\dagger=B$ when \texttt{canonical.is\_dagger\_pair}
cannot prove it, before choosing its solver (\S6).

A term counts as absent only when its coefficient is at or below
$10^{-12}$ of the largest coefficient of the operator, and in the
canonical form at or below $10^{-12}$ of the larger of that and the
magnitudes summed into it, so an operator means the same thing in any unit
of energy while the rounding dust of $H-H^\dagger$ still cancels. Until
the 2026-09-25 fixes it was an absolute $10^{-8}$, so $10^{-9}S^z_0$ had
no terms, its expectation value and every correlator of it were exactly 0
on every backend, ED included, and a Hamiltonian written below that scale
was the zero operator. The price of the relative rule is a hierarchy of
more than twelve decades inside one operator: in $10^6+10^{-7}S^z_0$ the
field is dropped, which the absolute rule kept.

On \texttt{itensor\_version=2} and \texttt{3} three numbers inside ITensor
are absolute, calibrated for an operator of order one: \texttt{toMPO}
discards squared singular values that sum to its cutoff of $10^{-13}$ and
skips a coefficient below $10^{-14}$, and its Davidson solver replaces a
Krylov direction by a random vector once its residual is below
$10^{-10}$. Since the 2026-09-25 fixes a Hamiltonian whose largest
coefficient is below 1 is handed to all three multiplied by the power of
two that brings that coefficient into $[1,2)$, and the factor is divided
back out exactly, so the ground state, the excited states, the band edges
and every KPM spectrum come out the same in any unit of energy. Before, a
Heisenberg chain written at $J\lesssim4\times10^{-7}$ (v3) or $10^{-7}$
(v2) lost its exchange channels and returned the N\'eel energy, $-1.25J$
against $-2.4936J$ on 6 sites, and from about $J=10^{-6}$ down every
energy lost digits. The largest coefficient is read after ITensor has
merged duplicate terms, the same reading the MPO is built from, so
$sH+S^z_0-S^z_0$ is solved exactly as $sH$; until the 2026-09-25b pass the
solver read the raw term list, ran that operator unscaled, and returned
$E_0/s$ 0.09 to 0.53 off on a 6-site Heisenberg chain at $s=10^{-10}$.
Since then \texttt{"python"} builds its operators at the same unit scale
too (a Heisenberg MPO at $s=10^{-7}$ was $1.1\times10^{-9}$ off relative
to itself on numpy with OpenBLAS, and is now at roundoff at every scale),
and NH-DMRG, on v2, v3 and \texttt{"python"}, solves $2^kH$ and divides the
energy back in the same way. What one scale cannot reach is a hierarchy inside the
Hamiltonian: \texttt{toMPO} truncates bond by bond, so a bond whose
strongest term is below about $4\times10^{-7}$ (v3) or $3\times10^{-7}$
(v2) of the largest coefficient loses channels at any units, which is an
$O(1)$ energy offset or one-site field next to exchange written at
$10^{-7}$, or a weak link $J'=10^{-7}$ in a $J=1$ chain, both read at a
third of their exchange, and \texttt{"python"} drops such a bond
altogether. Three more routes are not scale-free, and on those the
Hamiltonian is best written in units where its largest coefficient is of
order 1: a Hamiltonian set as an already-built MPO
(\texttt{set\_hamiltonian(\allowbreak toMPO(H))} on v3,
$9.8\times10^{-6}$ off at $J=10^{-8}$), real-time evolution
($6.4\times10^{-4}$ off with TDVP at $J=10^{-8}$ on v3, the MPO-Taylor
stepper of v2 diverging there, and v3's Krylov exponentiator stopping
on an absolute error; \texttt{"python"}'s has been scale-free since
2026-09-26, see \S21), and the Lanczos
of \texttt{"python"}'s finite DMRG, which stops on an absolute test
($1.2\times10^{-6}$ relative at $J=10^{-9}$, measured before its MPOs were
built at unit scale). NH-DMRG on v2 was a fourth until the 2026-09-25b
pass. \texttt{mode="ED"} does not meet ITensor's thresholds, but until that
pass it had absolute ones of its own: its iterative eigensolver above 2000
states, which now runs at unit infinity norm,
\texttt{submode="ROOTN"}'s Lanczos breakdown, now relative to the seed's
own $\lVert Hq_0\rVert$ on ED and on the MPS route alike, and the
\texttt{dex} cutoff (\S6) and \texttt{State.normalize()}'s floor, which
stay absolute and now warn. \texttt{"julia\_live"} is unaffected by the
units.

\texttt{trace}, \texttt{operator\_norm} and \texttt{is\_zero\_operator}
take a \texttt{MultiOperator}, not a compiled \texttt{StaticOperator}. On
the wavefunction itself, \texttt{wf.dot(other)} and \texttt{wf.norm()}
give $\langle\psi|\phi\rangle$ and $\lVert\psi\rVert$,
\texttt{wf.normalize(tol=1e-8)} returns the normalized state (or
\texttt{None}, with a warning, if the norm is below \texttt{tol}), on
every backend, \texttt{mode="ED"} included since the 2026-09-25b pass,
where \texttt{normalize()} took no \texttt{tol} and \texttt{norm()} did
not exist; \texttt{tol} is an absolute floor on the norm, so for a state
of another scale pass it relative to that scale.
\texttt{wf.get\_conjugate()} gives the complex conjugate state, and
\texttt{wf.get\_entropy(b)} the entanglement entropy at bond \texttt{b}.
\texttt{wf * x} and \texttt{A * wf} are shorthand for
\texttt{scale\_mps} and \texttt{applyoperator}.
\texttt{wf.get\_dm(inds=[...])} returns the single-particle density
matrix $\langle c_i^\dagger c_j\rangle$ and is implemented for
\texttt{Fermionic\_Chain} only.
...
\lstinline|get_gs()| takes the same keywords as \lstinline|gs_energy()|
and returns the stored state under the same condition, which is that the
stored answer already answers the call: a current state, no
\lstinline|wf0=|, and no keyword but \lstinline|reconverge=False|,
\lstinline|maxde=None| or \lstinline|maxdepth=| (on
\lstinline|"julia_live"|, no keyword at all). Every other call goes to the
solver, as on a chain that was never solved, so
\lstinline|reconverge=True| sweeps from the stored state,
\lstinline|maxde=| refines it, and a misspelled keyword raises
\lstinline|TypeError|. Until the 2026-09-25b pass a solved chain returned
the stored answer before reading any of them:
\lstinline|gs_energy(maxde=1e-4)| gave the unrefined $-4.1432$ against
$-4.2580$ on a 10-site Heisenberg chain at \lstinline|maxm=3|, and
\lstinline|wf=x| or \lstinline|reconverg=False| were swallowed.
\lstinline|get_gs(wf0=x)| sweeps from x, and
\lstinline|get_gs(wf0=x, reconverge=False)| returns the unit vector
$x/\lVert x\rVert$ with \lstinline|e0| =
$\langle x|H|x\rangle/\langle x|x\rangle$, on a solved chain as on a fresh
one and on every route, those that resolve to ED included, and every later
reader measures that state. A state and its multiples are one state: every
setter normalizes what it is given, once, and a state whose norm is zero
or not finite raises \lstinline|ValueError|. Until the 2026-09-25 fixes a
solved chain returned its stored state without reading x, and until the
2026-09-25b pass the readers met a state whose norm is not one at two
different normalizations, and the routes that resolve to ED raised
\lstinline|TypeError| or dropped x.
...
(\lstinline|set_initial_wf_guess|, or simply a previous
\lstinline|gs_energy()| call's own solution; \lstinline|set_initial_wf|
instead takes the state unswept, normalized), the ramp's starting bond
dimension is floored at whatever that state already carries, so a re-run
can only improve the energy. On \lstinline|itensor_version="julia_live"|
the setters follow the same contract since the 2026-09-25 fixes: a state
set with \lstinline|set_gs()| or \lstinline|set_initial_wf()| is taken
unswept with its own energy $\langle\psi|H|\psi\rangle$,
\lstinline|set_initial_wf_guess()| is swept from, and
\lstinline|submode="KPM"| measures a set state from its own energy on the
band-edge window. Before, both setters were dropped and the next read
solved from a random start, \lstinline|set_gs()| raised
\lstinline|AttributeError|, and \lstinline|gs_energy(wf0=x)| on a solved
chain raised \lstinline|TypeError|. The ramp applies to the ground-state solve
on all three DMRG backends (\lstinline|itensor_version| 2, 3 and
\lstinline|"python"|); \lstinline|"julia_live"| keeps its own schedule,
but since the 2026-09-25b pass a solve there that has no state of its own
starts from a random MPS of link dimension
\lstinline|min(maxm, bond_ramp_start)| and puts \lstinline|sc.noise| on
the first half of the sweeps, as the other backends do. It used to start
from a product state with no noise, so a Hamiltonian whose couplings skip
a site stopped at a classical product state with no warning: $-0.5$
against the exact $-1.0$ for the Heisenberg chain on the even sites of 6,
and $\langle H\rangle=-0.5$ against $-1.0$ for a 3-site
\lstinline|Thermal_Spin_Chain| at $T=0$, whose physical Hamiltonian lives
on the even sites of the doubled chain. Every \lstinline|"julia_live"|
solve now varies from run to run within the solver's noise, and a warm
16-site solve takes 2.1 to 2.35\,s where it took 1.5\,s.
...
automatic DMRG-to-ED fallbacks: a chain that \emph{explicitly} asked for
\lstinline|itensor_version=2|/\lstinline|3| on a machine where that
extension was never compiled, or an \lstinline|itensor_version=3| chain
too short for ITensor's two-site DMRG ($n<3$), answers the sector
correctly through ED instead of refusing. (A chain that named no
\lstinline|itensor_version| at all no longer reaches that fallback: it is
resolved to \texttt{"python"} at construction time when no extension is
compiled, and the pure-Python backend implements sectors itself.)
...
\lstinline|chain.get_sector_charge_operators()| returns the conserved
quantities this chain offers to \lstinline|set_conserved_sector|, as a
\texttt{\{name: MultiOperator\}} dict measuring each one over the whole
chain --- the same names and the same integer $2S_z$ units. They are
ordinary observables, so \lstinline|chain.vev(ops["Nf"])| works directly.
A chain that conserves nothing (parafermions) raises.
...
state the chain holds is kept --- the sector's ground state, or one set
with \lstinline|set_gs()|/\lstinline|set_initial_wf()|, taken unswept with
its own energy $\langle\psi|H|\psi\rangle$ on the next read, so a bare
\lstinline|gs_energy()| afterwards returns that energy rather than
re-solving; call \lstinline|restart()| if an unconstrained re-solve is
what you want. Until the third 2026-09-24 pass a state set by hand was
dropped by the promotion, or re-swept on the dense sites, which on
\texttt{"python"} could leave the sector altogether (\S21). A wavefunction Python is already
...
\texttt{npow=} now means $\langle O^n\rangle$ on the ED route too. Under
\texttt{mode="ED"} it used to be accepted and ignored, returning
$\langle O\rangle$ for every $n$ --- so any previously recorded ED value
for \texttt{npow>1} is a different quantity and is not comparable.
\texttt{npow=0} returns \texttt{1.0} on both routes. The two guards
that were added are \textbf{ED-route only}: on that route a negative
\texttt{npow} now raises \texttt{ValueError} and, at $T>0$
(\texttt{vev(..., T=...)}), an \texttt{npow} other than 1 raises
\texttt{NotImplementedError} telling you to pass the explicit product
operator instead --- where both used to be accepted silently and answer
with $\langle O\rangle$. The DMRG routes are unchanged and still answer
rather than raise: under \texttt{itensor\_version="python"},
\texttt{vev(Sz[0], npow=-1)} returns ${\approx}0$ and
\texttt{vev(Sz[0], npow=2, T=1.0)} returns 0.25. This matters beyond an
explicit \texttt{mode="ED"}: it also reaches the \emph{automatic} ED
routes (no compiled C++ extension, or \texttt{itensor\_version=3} on a
chain with fewer than 3 sites), which nobody opts into.

The model classes wrap the most common of these profiles, so the loop
above rarely has to be written by hand. Each takes the same
\texttt{mode=}/\texttt{**kwargs} as \texttt{vev} and returns one value
per site: \texttt{.get\_density()} gives $\langle n_i\rangle$ (summed
over spin on spinful chains) and
\texttt{.get\_density\_fluctuation()} gives
$\langle n_i^2\rangle-\langle n_i\rangle^2$, both on fermionic chains;
\texttt{.get\_onsite\_pairing()} gives
$\langle c_{i\uparrow}c_{i\downarrow}\rangle$ on spinful fermionic
chains; and \texttt{.get\_magnetization()} gives the $3\times n_s$ array
of $\langle S^x_i\rangle,\langle S^y_i\rangle,\langle S^z_i\rangle$ on
spin and spinful fermionic chains, also writing a
\texttt{MAGNETIZATION.OUT} file into the working directory as a side
effect.
...
=\big\lVert(H-\langle H\rangle)|\psi\rangle\big\rVert
\end{equation}
...
It is computed as the right-hand side on every mode: $\langle H\rangle$
is subtracted first and $(H-\langle H\rangle)|\psi\rangle$ is applied
with no bond-dimension cap, so the truncation, set only by
\texttt{cutoff}, acts on the small residual itself. Until the third
2026-09-24 pass it was the left-hand side, with $H|\psi\rangle$ truncated
to the chain's \texttt{maxm}, and that number was set by the truncation
rather than by the state: a state solved and measured at the same
\texttt{maxm} reported its fluctuation 10 to 51 times too low (6.7e-03
against 0.344 on an 8-site chain at \texttt{maxm=3}), and one wider than
\texttt{maxm} reported it at order one (\S21).
\texttt{get\_gs(maxde=...)}/\texttt{gs\_energy(maxde=...)} reads the same
quantity \textbf{per site}, $\delta E/n_s$, doubling \texttt{maxm} until
it drops below \texttt{maxde}, and returns the refined energy, the one it
leaves on the chain. It is therefore
\texttt{gs\_energy\_fluctuation()/n\_s}, not
\texttt{gs\_energy\_fluctuation()} itself, that is compared with
\texttt{maxde}, and each step of the loop prints it as
\texttt{Energy fluctuation per site}.

\texttt{mode=} is now actually forwarded here:
\texttt{gs\_energy\_fluctuation(mode="ED")} runs ED, where it used to
return the DMRG number byte for byte --- so a value recorded as an ``ED
energy fluctuation'' from before this change is a DMRG value. Together
with the \texttt{npow} fix above, every ED route now evaluates
$\sqrt{|\langle H^2\rangle-\langle H\rangle^2|}$ rather than
$\sqrt{|\langle H\rangle-\langle H\rangle^2|}$: on a 4-site Heisenberg
chain \texttt{mode="ED"} reports 0.0 where it used to report 2.0561, and
a 2-site chain on the \emph{default} backend --- which \texttt{mode.py}
routes to ED by itself --- reports 1.05e-08 where it used to report
1.1456. Anything using this number as a convergence criterion
(\texttt{get\_gs(maxde=...)}, or a script that tightens \texttt{maxm}
until it drops) therefore behaves differently on an ED route.
\texttt{npow=} is rejected with a \texttt{TypeError}: this function sets
the powers itself.

What the number measures below full bond dimension is the truncation of
the state, which is why it is worth watching while tuning \texttt{maxm}.
On a 10-site Heisenberg chain at the stock \texttt{maxm=30}, one below
the full bond dimension 32, \texttt{itensor\_version=3} reports 1.4e-05
and \texttt{itensor\_version="python"} 8.4e-06, and \texttt{mode="ED"}
4.2e-15. The two DMRG numbers used to read 1.7e-07 and 6.3e-06, which
this guide attributed to backend-dependent floors (Lanczos accuracy on
\texttt{"python"}, a double-precision cancellation floor on v3); both
were artefacts of truncating $H|\psi\rangle$ at \texttt{maxm}.
...
\lstinline|gs_energy_generalized|).} Afterward, \lstinline|fc.wf0| holds
the eigenstate $|w_g\rangle$ of the \emph{generalized} problem, not a plain
eigenstate of \lstinline|fc.hamiltonian| alone, and \lstinline|fc.e0| that
state's own energy $\langle w_g|H|w_g\rangle/\langle w_g|w_g\rangle$ (on a
non-Hermitian chain the biorthogonal
$\langle\psi_L|H|\psi_R\rangle/\langle\psi_L|\psi_R\rangle$ of the pair),
while $\lambda$, still what the call returns, is kept as
\lstinline|fc.lam_generalized| --- every other method that reads
...
reference state if called afterward. Since the 2026-09-25 fixes this is
what every correlator does, on a Hermitian and a non-Hermitian Hamiltonian
alike and whether or not the chain was solved before. It used to re-solve
a plain ground state over the generalized one at the first correlator on
a Hermitian chain never solved ($|\langle w_g|w_0\rangle|^2=0.60$ on a
...
generalized solve. A bare \lstinline|gs_energy()| afterwards returns that
energy, since the stored state is current, and every submode measures the
state's lines from it, as TD and EX always did. Until the 2026-09-25b pass
\lstinline|e0| was $\lambda$, so KPM, CVM, ROOTN and TDZ put every line
$\lambda-E_{w_g}$ away from where TD and EX put it: for
$A=2\,\mathrm{Id}$, whose generalized state is the ground state itself,
the $(S^z_0,S^z_0)$ lines of a 4-site chain in a field sat at $-0.15$ and
$+0.56$ against the exact $+0.66$ and $+1.365$, a line at negative
frequency in a ground-state autocorrelator. Call
\lstinline|gs_energy_generalized()| as the last step of a calculation, or
\lstinline|restart()| and then \lstinline|gs_energy()| if you need a
genuine ground state for one of those other methods.

Unlike plain \lstinline|gs_energy()|, this method has no ED fallback for
a chain too short for the backend's two-site sweep, so on
\texttt{itensor\_version="python"} it \emph{raises}
\texttt{RuntimeError} below 2 sites rather than returning a number. That
is deliberate: the outer self-consistent iteration would otherwise still
return the Rayleigh quotient of a state no sweep ever touched ---
measured at $-0.3049$ for an exact $-0.5$ --- i.e.\ a silently wrong
answer. Use \lstinline|mode="ED"| for a chain that short.
...
\texttt{get\_correlation\_matrix(dmmode=...)} picks how $C_{ij}$ is
evaluated (\texttt{"simple"}, \texttt{"fast"}, \texttt{"explicit"},
\texttt{"full"}). The default, \texttt{dmmode=None}, is resolved from
\textbf{the state being measured}, not from the backend the chain was
built with, and only after that state exists: with no conserved sector
it is \texttt{"fast"} (apply each $c_i$ to the state and overlap the
results --- much the cheapest); with a sector, that step would change
the particle number, so it is \texttt{"full"} when the wavefunction
carries a live DMRG session handle that can answer the matrix directly
(\texttt{itensor\_version=3} and \texttt{"python"}), and the
backend-agnostic \texttt{"explicit"} otherwise --- \texttt{mode="ED"},
\texttt{"julia\_live"}, \texttt{basis="Nambu"}, or any state with no
session handle. That last case is why a conserved sector plus
\texttt{mode="ED"} now answers \texttt{get\_correlation\_matrix},
\texttt{get\_correlation\_eigenvalues},
\texttt{get\_correlation\_entropy},
\texttt{get\_correlation\_entropy\_density},
\texttt{get\_correlated\_orbitals} and
\texttt{get\_correlated\_density} instead of dying with an
\texttt{AttributeError}: the sector case used to hardcode the
session-only \texttt{"full"}, on the since-obsolete premise that only
DMRG could reach a sector at all.

A \textbf{misspelled} \texttt{dmmode}, \texttt{basis}, \texttt{ctmode}
or \texttt{fpmode} now raises \texttt{ValueError} naming the argument
and listing the accepted values, where it used to surface as
\texttt{RuntimeError: No active exception to reraise} --- or, for
\texttt{basis=}, as no error at all: the old \texttt{else} branch served
both \texttt{"electron"} and ``you typo'd it'', so a typo silently
returned the electron-basis matrix. A \texttt{dmmode} you pass
explicitly is now also rejected \emph{before} the ground-state solve
rather than after it. What has not changed is that an
explicitly-passed mode is still a hard request: it raises rather than
silently falling back when that method is not available for the
wavefunction at hand. The new error only distinguishes ``you misspelled
this'' from ``this mode exists but not here''.

\texttt{get\_four\_correlation\_tensor(ctmode=...)} has five
...
wavefunction at hand. For \texttt{Spinful\_Fermionic\_Chain\_Native} the only method
that has no native-spinful counterpart is \texttt{ctmode="sweep"}; the
default resolver picks \texttt{"batched"} under
\texttt{itensor\_version="python"}, \texttt{"fold"} under
\texttt{itensor\_version=3}, and falls back to \texttt{"full"} and then
\texttt{"explicit"} if neither is available on the session at hand.

\section{Dynamical (frequency-dependent) correlators}
\label{sec:dynamical}

The quantity returned is the \textbf{complex Lehmann density} of the
operator pair $(A,B)$, Lorentzian-broadened by $\delta$:

\begin{equation}
S_{AB}(\omega)\;\equiv\;\sum_n M_n\,\frac{\delta}{\pi\big[(\omega-\Delta_n)^2+\delta^2\big]}
\;\xrightarrow[\delta\to0]{}\;\sum_n M_n\,\delta(\omega-\Delta_n)
\;=\;\frac{i}{2\pi}\Big[G^R_{AB}(\omega)-G^A_{AB}(\omega)\Big]
\end{equation}
...
$\delta$ is the small broadening. Here $|\mathrm{GS}\rangle$ is the
chain's own state, the solved ground state or one set with
\texttt{set\_gs()}, and $E_0$ is its energy, the one
\texttt{gs\_energy()} reports, on every submode and both modes; SECTOR,
which measures its reference sector's ground state, is the exception (see
its entry). Until the third 2026-09-24 pass DMRG KPM measured a set state
from the solved ground-state energy and \texttt{mode="ED"} from three
different origins (\S21). $M_n$ is complex in general, so the returned
array is complex. The discriminant that matters is
$\mathrm{Im}\,M_n=0$: \textbf{whenever every $M_n$ is real}, the density
is real and coincides exactly with the equally common convention
$-\frac1\pi\mathrm{Im}\,G^R_{AB}$. That is a weaker condition than the
\textbf{Hermitian pair} $A=B^\dagger$, which only adds non-negativity
($M_n=|\langle n|B|\mathrm{GS}\rangle|^2\ge0$). A real Hamiltonian with
real $A$ and $B$ has real $M_n$ whether or not $A=B^\dagger$: measured
on a 6-site Heisenberg chain with $A=S^z_0$, $B=S^z_2$,
$\max_n|\mathrm{Im}\,M_n|=0$ and the two conventions agree to 1.1e-16
--- while the density does dip negative there (min $-0.0582$ against a
peak $0.1515$), which a Hermitian pair never does. Every
\texttt{name=(A,B)} example in this guide \textbf{on a Hermitian $H$} has
real $M_n$ --- they are Hermitian pairs such as $(S^z_0,S^z_0)$ and
$(A,A^\dagger)$, plus the $S(q,\omega)$ sweep below, whose
$(S^z_i,S^z_j)$ at $i\neq j$ is \emph{not} a Hermitian pair but is still
real --- so if that is the case you work in, nothing below changes
anything for you. The qualifier is load-bearing: under a non-Hermitian
$H$ (the \texttt{submode="KPM"} example in the non-Hermitian section) the
$\Delta_n$ are themselves complex and this whole construction --- real
$\Delta_n$, orthonormal $|n\rangle$ --- does not apply; that section
defines its own quantity. The criterion is
not vacuous: mixing $S^y$ with $S^z$ on the same real Hamiltonian gives
a purely \emph{imaginary} $M_n$ ($\max_n|\mathrm{Re}\,M_n|=0$).

The two conventions differ by the dispersive term:
$-\frac1\pi\mathrm{Im}\,G^R_{AB}=\sum_n[\mathrm{Re}(M_n)\delta-\mathrm{Im}(M_n)(\omega-\Delta_n)]
/\pi[(\omega-\Delta_n)^2+\delta^2]$. The operational reason dmrgpy picks
the density is the kernel-independent sum rule

\begin{equation}
\int d\omega\,S_{AB}(\omega)=\sum_n M_n=\langle\mathrm{GS}|A\,B|\mathrm{GS}\rangle
\end{equation}

which any submode can be checked against, and which
$-\frac1\pi\mathrm{Im}\,G^R$ does \emph{not} satisfy: its dispersive
term has principal-value tails that leak arbitrarily far outside any
finite frequency window. (Measured on a 4-site chain with complex
hoppings and $A=c^\dagger_0$, $B=c_2$, over
\texttt{es=np.linspace(-12,18,1500)} at $\delta=0.05$: the exact
$\langle AB\rangle$ is
$0.11052176-0.27135291i$; \texttt{submode="KPM"} integrates to that
within 2.6e-08
under \texttt{mode="ED"} and 7.5e-06 under \texttt{mode="DMRG"} on
\texttt{itensor\_version="python"}, while
the $-\frac1\pi\mathrm{Im}\,G^R$ curve integrates to a real 0.13145.)
...
$S(q,\omega)$ measured in inelastic neutron scattering. All of the
submodes below compute $S_{AB}(\omega)$ as defined above ---
\texttt{submode="CVM"} under \texttt{mode="DMRG"} being the one that only
recently joined them, so that it now agrees with its own
\texttt{mode="ED"} counterpart (measured 1.9e-09 apart on a 6-site
Heisenberg chain --- comfortably below the solver's own per-frequency
residual, which prints at ${\sim}10^{-5}$ on that run; see
Section~\ref{sec:whatraises}), and the real-time pair
\texttt{submode="TD"}/\texttt{"TDZ"} being the one that joined them on
2026-09-22, having returned the complex one-sided Fourier transform
until then (see that section and Section~\ref{sec:whatraises}). Sharing
the convention is not the same as agreeing pointwise, since two submodes
can still differ in kernel and in resolution, but for the default
\texttt{submode="KPM"} they no longer differ in resolution either:
\texttt{delta} now sets the same width on \texttt{mode="ED"} as on
\texttt{mode="DMRG"}, so the two routes sit a fraction of a per cent of
the peak apart instead of a whole peak apart. On a 6-site Heisenberg
chain with $A=B=S^z_0$ at $\delta=0.2$, over
\texttt{es=np.linspace(0.01,4,120)}, the two curves sit 1.65e-04 apart
on \texttt{itensor\_version="python"} against a resolvent
(\texttt{submode="INV"}) peak of 0.194, and both integrate to 0.24999
against the exact $\langle AB\rangle=0.25$. The audit that fixed this
measured the same quantity on \texttt{itensor\_version=3} across three
chains and found 4 to 7 per cent of the resolvent peak, down from 95 to
124 per cent (\texttt{docs/audit\_2026\_09\_hole\_hunt.md}, open item
O2), and most of that remainder turned out to be the DMRG routes
reconstructing from two more moments than the calibration asks for.
Since they were cut to the calibrated count
(\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, finding 4) the same
three chains sit 0.07 to 0.2 per cent of the peak apart on
\texttt{itensor\_version="python"}, while \texttt{itensor\_version=3}
agrees as closely in its median run and reaches about 1.3 per cent (worst of
25 runs on the staggered 4-site chain) only in
the runs where its band-edge estimate, which moves by up to 2e-2 from
run to run, lands below the true upper edge. What remains between KPM and a resolvent submode is the kernel
itself, a near-Gaussian Jackson line against a Lorentzian, which the
\texttt{submode="KPM"} entry below quantifies.
Otherwise the submodes differ
only in \emph{how}, and therefore in what
...
ringing from the truncation at $N$ moments.
This is the default, general-purpose method: robust, works
across the whole spectrum at once, cost grows only linearly with the
number of moments.

\emph{What \texttt{delta} buys you here.} The Jackson kernel turns an
exact pole at rescaled energy $x$ into a line of width
$\sigma=\pi\sqrt{1-x^2}/N$ (Weisse, Wellein, Alvermann \& Fehske,
\href{https://doi.org/10.1103/RevModPhys.78.275}{RMP 78, 275 (2006)},
Sec. II-B), so $N$ and the requested broadening are the same knob read
two ways. Every KPM route in dmrgpy solves that relation for $N$
(\texttt{algebra/kpm.py::polynomials\_for\_broadening}), so the
reconstructed line comes out at FWHM $=2\delta$ at the centre of the
rescaled band, which is the width the resolvent submodes
(\texttt{"CVM"}, \texttt{"INV"}, \texttt{"ROOTN"}, \texttt{"ED"}) give
for the same $\delta$. The relation holds only when the kernel is
...
$\delta=0.2$, on \texttt{itensor\_version=3} and \texttt{"python"}
alike, and now equals the ED peak
(\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, finding 4).
\texttt{kpm\_n\_scale} is a multiplier on that calibrated count, default
1, and it has to be a positive integer (a Python or numpy integer, not a
\texttt{bool}): raise it to buy a line sharper than the $\delta$ you
asked for, at proportionally higher cost. Anything else raises a
\texttt{TypeError} or \texttt{ValueError} naming \texttt{kpm\_n\_scale}
on every backend before a single moment is computed, where
\texttt{"python"}, \texttt{mode="ED"} and \texttt{"julia\_live"} used to
round it down silently (1.5 gave exactly the calibrated count, and
anything below 1 the 16-moment floor at every $\delta$) while the C++
backends raised; note that \texttt{kpm\_n\_scale=2.0}, which gave 2x on
those three routes, raises now too (finding 5 of the same record). Both
the calibration and that default are new in the 2026-09 audit's open
item O2
(Section~\ref{sec:whatraises}). Before it, \texttt{mode="DMRG"} counted
\texttt{round((emax-emin)/delta)} moments and multiplied by a
\texttt{kpm\_n\_scale} that defaulted to 3, which put the line at about
$1.6\delta$, while \texttt{mode="ED"} used a rule of its own that read
neither of those and landed at about $0.93\delta$, on a Chebyshev window
three times as wide.
Two residuals are properties of the kernel rather than of this choice,
and are documented rather than removed. The width is exact at the band
centre and tightens as $\sqrt{1-x^2}$ towards the band edges, since no
single moment count gives one width across a whole band, and for a
ground-state correlator that narrowing is the rule rather than the
exception. Every route rescales the same way, so with $s$ the
\texttt{kpm\_scale} and $W$ the many-body bandwidth the ground-state
...
on the ground-state-anchored window of
\texttt{kpm\_energy\_truncate=True} the factor at $E_0$ is 0.157.
Measured on open Heisenberg chains with $A=B=S^z_i$, the isolated lowest
pole comes out at 0.755 of $2\delta$ on 12 sites and 0.725 on 20, and
the delivered width averaged over the spectrum is 0.87 to 0.90 of the
requested one on 8 sites, 0.83 to 0.86 on 12 and 0.78 to 0.80 on 20. To
get FWHM $=2\delta$ at a chosen $\omega$, pass
$\delta/\sqrt{1-x(\omega)^2}$ instead, which is the only compensation
available, since \texttt{kpm\_n\_scale} cannot ask for fewer moments
than the calibrated count
(\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, finding 6, and
\texttt{algebra/kpm.py::polynomials\_for\_broadening}'s docstring for
the rest of the measurements). And the line is near-Gaussian rather than
Lorentzian, so at equal FWHM and equal integrated weight its peak stands
about 1.5x higher than a resolvent submode's, 1.514 being the Jackson
kernel's own ratio on one pole at the band centre; the 0.315 against
0.195 once quoted here, on a 6-site Heisenberg chain at $\delta=0.2$
with both integrating to the same 0.250000, is a ratio of 1.61 because
its dominant pole sits at $x=-0.527$, where the line is already 0.850 of
$2\delta$. One guard rail
sits underneath: a $\delta$ comparable to the bandwidth itself
calibrates to a handful of moments, which stops being a spectrum at all,
so the count is floored at 16 and the line there comes out sharper than
requested instead of unrepresentable.
...
too tight for the chosen \texttt{kpm\_scale}, or \texttt{kpm\_scale}
is below 1/2, where the ground state itself sits at
$x_0=-1/(2s)$, $s$ being \texttt{kpm\_scale}, outside $[-1,1]$, the moment recursion
raises \texttt{RuntimeError} as soon as a moment exceeds 1.5 times
$\|v_i\|\|v_j\|$, the exact bound for a spectrum inside $[-1,1]$, on
every DMRG backend and on \texttt{mode="ED"} alike (\texttt{julia\_live}
raises \texttt{juliacall.JuliaError}). Until the 2026-09-24 second-pass
audit the DMRG threshold was $10^3(\|v_i\|\|v_j\|+1)$, which let spectra
up to 109 times the true peak through, and ED had no check at all
(findings 2 and 3). This catches every case where a pole outside the
window carries enough weight to show; in a sliver just below 1/2 (0.499
on a 4-site chain at $\delta=0.1$) the elastic line is already distorted
while the moments are still under the bound, so stay above the
$\sim0.5$ floor unless \texttt{kpm\_energy\_truncate} is on. A harshly
truncated \texttt{kpmmaxm} (2 or 3 on a small chain) can also cross the
bound, and then its spectrum is tens of per cent wrong anyway.
...
\item \textbf{It is more sensitive to MPS truncation noise than
  ...
  where it belongs. That default $N\eta$ is whatever the moment
  calibration above implies for $\eta=\delta$, which is
  \texttt{JACKSON\_FWHM\_FACTOR}$/2=3.70$ at \texttt{kpm\_n\_scale=1};
  the two ratios just quoted were measured before that calibration
  landed, when the same accounting gave 4.3, and they have not been
  re-measured against it. Both values sit in the flat minimum of the
  error-versus-$\eta$ curve, which is why the default was left where it
  is rather than chased.
  The same example measures the moment error directly against
...
(\texttt{mode="ED"} raises \texttt{NotImplementedError}, and so does
\texttt{mode.py}'s fallback onto it for a 2-site
\texttt{itensor\_version=3} chain; it used to return the untruncated
curve, finding 4 of the second-pass record); there
...
\texttt{itensor\_version=2} raises \texttt{NotImplementedError} (it used
to be silently ignored there, so the run looked truncated and was not ---
see Section~\ref{sec:whatraises}). The v3 port
...
right-hand side $b=-\eta B|\mathrm{GS}\rangle$. Second, the CG loop
stops early once the bond-dimension truncation (\texttt{cvm\_maxm}) keeps
it from improving, instead of burning the full \texttt{cvm\_nit}
iteration budget. The CG starts from zero ($b=0$ returns 0 at once) and
stops when its residual reaches \texttt{cvm\_tol} (default $10^{-5}$)
relative to $\lVert b\rVert=\eta\lVert B|\mathrm{GS}\rangle\rVert$. Its
two early exits read the CG functional
$\phi(x)=\tfrac12\langle x|M|x\rangle-\mathrm{Re}\langle b|x\rangle$, $M$
the system matrix above, which exact CG lowers at every step:
\texttt{cvm\_patience} (default 50) counts iterations without a new
minimum of $\phi$, and \texttt{cvm\_blowup} (default 100) fires only when
$\phi$ is above its minimum and the running residual is that many times
the best iterate's. The solver returns the iterate that reached the
tolerance or, failing that, the one of lowest $\phi$. On an untruncated
solve neither exit can fire, so neither changes the answer there; on a
truncated one they stop a recurrence that has stopped improving, and the
point warns. Each point reports its CG iteration count and best residual;
if that residual stalls far above \texttt{cvm\_tol}, the correction vector
is not converged at this bond dimension and the fix is a larger
...
Until the 2026-09-25b pass \texttt{cvm\_tol} was absolute, the CG started
from $b$ itself, and both exits read the running residual, which exact CG
does not lower monotonically. Whenever
$\eta\lVert B|\mathrm{GS}\rangle\rVert$ was at or below $10^{-5}$ (an
operator at scale $10^{-4}$, a Hamiltonian in small units, or
\texttt{get\_kondo\_spectrum}'s documented default \texttt{delta=2e-6})
the start passed at iteration 0 and every frequency returned the flat
...
\textbf{\texttt{submode="CVM\_explicit"} --- the resolvent formed
explicitly.} Computes the same $S_{AB}(\omega)$ as
\texttt{submode="CVM"}, but without the positive-definite reformulation
above: it applies $(\omega+E_0\pm i\eta-H)^{-1}$ to $B|\mathrm{GS}\rangle$
directly, as two \texttt{applyinverse} solves at $\pm i\eta$, and combines
them. That is a more literal transcription of the definition and needs no
conjugate-gradient machinery, but it inherits \texttt{applyinverse}'s
accuracy rather than \texttt{"CVM"}'s controlled CG residual, so
\texttt{"CVM"} is the better default; use this one to cross-check a
suspicious \texttt{"CVM"} curve. \texttt{applyinverse}'s own tolerance
\texttt{delta=} is relative to the norm of the vector it inverts against
since the 2026-09-25b pass, so the solve is as accurate for a small
operator as for a large one. It takes any pair: it inverts against
...
so it raised \texttt{NotImplementedError} for a non-adjoint pair at unit
scale (and, before that, a bare \texttt{RuntimeError} after a
\texttt{print}) and answered one at scale $10^{-2}$ with the adjoint
pair's curve, 2.004 of the peak off for $(10^{-2}S^x_0,10^{-2}S^y_1)$. On
a non-Hermitian $H$ it is one of the two submodes (with \texttt{"CVM"})
that has a genuine non-Hermitian implementation.

``The same $S_{AB}$ as \texttt{"CVM"}'' is a statement that only became
true recently, and it is the one number-changing item here that hits
\emph{everyone}, Hermitian pairs included: \texttt{"CVM\_explicit"}
returned exactly \textbf{twice} the correct value, at every frequency.
This submode is DMRG-only --- \texttt{mode="ED"} raises
\texttt{NotImplementedError: submode='CVM\_explicit' has no ED
implementation} --- but it was the same factor of 2 on every DMRG
backend, since the fix is one backend-agnostic line. Any spectrum, peak
height, integrated weight or figure produced with it before that fix is
a factor of 2 too large. On the 6-site staggered Heisenberg chain the
two now agree to 5.7e-07. Its output is also plain complex now: on a
non-Hermitian $H$ it used to be passed through \texttt{np.abs()}, which
discarded the phase and reported $+|z|$ for values that are genuinely
negative or complex.
...
\textbf{Both \texttt{mode=} values return the house convention}, the
complex Lehmann density $\frac{i}{2\pi}(G^R-G^A)$ defined at the top of
this section, and each gets there by running the fractional-resolvent
recursion \textbf{twice}, once at $+i\delta$ and once at $-i\delta$.
That is the price of the convention and it is close to a literal factor
of two: \texttt{"ROOTN"} was already the most expensive submode here
($N$ sequential Lanczos subspaces of dimension \texttt{nkry} per
frequency, each step under \texttt{mode="DMRG"} a truncated MPO
application over the whole chain), and it now runs $2N$ of them.

There is no shortcut, and it is worth knowing why, because
\texttt{"CVM"} does have one. \texttt{cvm.py} solves a linear system
whose $-\eta$ version is the same system with the right-hand side
negated, so both resolvents fall out of one solve; root-$N$ applies a
\emph{function} of $H$, so the $-\eta$ pass genuinely re-seeds $N$ new
Krylov subspaces. Nor does conjugation help:
$\overline{G^R_{A,B}}=G^A_{B^\dagger,A^\dagger}$ is the advanced
resolvent of a \emph{different} operator pair, and it collapses to
$G^A_{A,B}$ only for $A=B^\dagger$ --- a case in which the two
conventions already coincide, so the shortcut exists only where it is
not needed.

Results produced before this change used
$-\frac1\pi\mathrm{Im}\,G^R$ and are \textbf{not comparable when some
$M_n$ is complex}, which for real operators on a real Hamiltonian never
happens (see the criterion at the top of this section); where the two
conventions coincide the curves coincide too, up to the recursion's own
tolerance. \texttt{mode="ED"} moved first and
\texttt{mode="DMRG"} followed; on the 4-site complex-hopping chain with
$A=c^\dagger_0$, $B=c_2$ at $N=6$, \texttt{nkry=16}, the
\texttt{mode="DMRG"} curve moves by up to 5.7e-01 against a peak of
0.61, and the returned array is genuinely complex there where it used to
be a real float.
...
transform of it is a \emph{resolvent}, not a spectral density: in the
...
\emph{forward} half of the pair $(B^\dagger,A^\dagger)$, which the same
...
which is what the returned array is. So \texttt{"TD"} returns the same
density every other submode returns, and there is nothing left to take
the \texttt{.real} of: on a Hermitian pair the imaginary part comes back
exactly zero. Measured on a 4-site Heisenberg chain with $A=B=S^z_0$
($\delta=0.3$, \texttt{dt=0.05}, \texttt{itensor\_version="python"},
resolvent peak 0.1869), max $|\mathrm{Im}\,y|$ is 0.0 and the curve sits
2.97e-04 from \texttt{submode="INV"}, which is the method's own
discretization error, with \texttt{mode="ED"} and \texttt{mode="DMRG"}
agreeing to 1.6e-10 on the same chain. When $A$ is \emph{provably}
...
\texttt{"TD"}/\texttt{"TDZ"} example in this guide, still costs one
evolution;
anything else costs two. The test is
\texttt{multioperatortk.canonical.is\_dagger\_pair}, and it refuses
rather than guesses for an operator name with no known adjoint (a
parafermionic \texttt{Sig}, a name you invented yourself), which buys a
second evolution. That second evolution is built from
\texttt{get\_dagger()}, so it is right exactly when
\texttt{get\_dagger()} knows the name's adjoint, and it did not for the
raw backend name \texttt{ISy}, which is $iS^y$ and so anti-Hermitian,
until the 2026-09-24 audit gave \texttt{get\_dagger()} a phase: before
that, \texttt{ISy} in the first operator returned exactly minus the
correlator under \texttt{"KPM"}, \texttt{"EX"}, \texttt{"TD"} and
\texttt{"TDZ"} (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md},
finding 2). Note that the cost
discriminant is not the convention discriminant: what the combination
needs is a \emph{proof} that $A=B^\dagger$, while what makes the
imaginary part vanish is $\mathrm{Im}\,M_n=0$, and the $S(q,\omega)$
sweep above sits in the gap. Its $(S^z_i,S^z_j)$ at $i\neq j$ has real
weights, so the answer is real either way, and it is still not an
adjoint pair, so under \texttt{"TD"} it pays two evolutions per site
pair. All of this is new in the
2026-09 audit's open item O1 (Section~\ref{sec:whatraises}), and it
moves numbers: a \texttt{"TD"}/\texttt{"TDZ"} spectrum from before it is
not comparable, its imaginary part most of all.
...
\item \texttt{predict} (default \texttt{True}) extrapolates the raw,
  ...
  windowing alone where truncation of the time window, rather than the
  damping, sets the width, which is the standard fix in the DMRG-dynamics
  literature
...
numbers). That comparison was made on the old frequency stage, which
evaluated the transform on an FFT grid of spacing $2\pi/(n_t\,dt)$ and
interpolated linearly onto \texttt{es}, and at the default window
($\delta T=6$) most of the narrowing it saw was that grid, which
prediction's tenfold longer series made ten times finer. Since the
damped sum is evaluated at each requested frequency
(\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, finding 7), a
4-site Heisenberg chain at $\delta=0.05$ shows the line at the exact
width with or without prediction at the default window (FWHM 0.0975 on
a 0.0025 grid, as the exact density gives), and what prediction still
buys there is accuracy, $\max|y-y_{\rm exact}|$ going from 2.55e-03 to
3.3e-06 by carrying the series past the $e^{-6}$ cut; at
\texttt{nt=200} ($\delta T=1$), where truncation sets the width, it
narrows the line from 0.2125 to 0.0975, the exact width. Note that
neither knob changes the \emph{asymptotic} algebraic
...
windowing stage, but their own defaults are unchanged
(\texttt{damping="exp"}, \texttt{predict=False}), since the empirical
comparison above was only run for \texttt{"TD"}. All three also share
the last step: they evaluate the damped sum directly at each requested
frequency. \texttt{sxt\_to\_skomega} stayed on the FFT grid for a while
after \texttt{"TD"} and \texttt{"TDZ"} left it, and at a converged
window, $\delta T=6$, that put its $S(k,\omega)$ 21 per cent of the peak
off exact, with peak heights down to 0.79 of the right ones; it moved
onto the direct sum with the 2026-09-24 second-pass audit
(\texttt{docs/audit\_2026\_09\_24b\_hole\_hunt.md}, finding 7, and
Section~\ref{sec:whatraises}).
...
The returned array is the density of this section, assembled from the
one-sided transform exactly as in \texttt{"TD"} above and through the
same shared code. The contour does not obstruct that identity: the
damping it puts on each Lehmann term, $e^{-\Delta_n\alpha t}$, is real
and is the same for a pair and for its adjoint, so conjugating the
adjoint run reproduces the backward-time half envelope and all, and a
pair that is provably its own adjoint still costs one run rather than
two. How close it lands is set by the same finite time window as
\texttt{"TD"}, not by the contour: on the 2026-09 audit's 4-site
complex-hopping chain ($A=c^\dagger_0$, $B=c_2$, $\delta=0.4$,
\texttt{dt=0.1}, exact peak 0.2313, \texttt{itensor\_version=3}) it
lands 5.4e-04 from the exact Lehmann density, which is where
\texttt{"TD"} lands at \texttt{predict=False}, while \texttt{"TD"} at
its default \texttt{predict=True} lands 3.2e-05. The contour and the
Taylor-in-$\alpha_0$ reconstruction together account for about 1e-06 of
that, the distance between \texttt{"TDZ"} and \texttt{"TD"} at
\texttt{predict=False}, meaning that \texttt{alpha0} and
\texttt{n\_max} do not move the floor. It used to read 3.31e-02, which
the 2026-09 records attributed to the contour and which was in fact the
frequency stage's linear interpolation of an FFT grid of spacing
$2\pi/(n_t\,dt)\approx1.05\delta$, shared with \texttt{"TD"} at
\texttt{predict=False}
(\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, finding 7).
\texttt{"TDZ"} also takes only the parameters it names since that
audit, so a misspelled keyword (\texttt{alpha=} for \texttt{alpha0=},
\texttt{nmax=} for \texttt{n\_max=}) raises \texttt{TypeError}, where
it used to be accepted and ignored, returning the default-parameter
spectrum bit for bit, and a \texttt{toMPO()} operator raises the
\texttt{TypeError} naming \texttt{toMPO} again (finding 9 of the same
record).
...
good as how many/which excited states were computed. Here
$|\mathrm{GS}\rangle$ is the chain's own state, the one
\texttt{set\_gs()} set if any, expressed in that subspace, and $E_n$ is
measured from its energy; a state outside the subspace raises (until the
2026-09-24 second-pass audit, finding 15, EX measured from the lowest
vector of its own subspace whatever the chain held). The \texttt{nex}
...
The $|\mathrm{GS}\rangle$ here is always the ground state of the
reference sector, solved on an internal clone, never the chain's own
state. After \texttt{set\_gs()}/\texttt{set\_initial\_wf()} the reference
charges are therefore read from the state that was set, and SECTOR
proceeds only when that state is its sector's ground state (by energy, to
$10^{-6}$ relative); any other state raises
\texttt{NotImplementedError} naming the submodes that read the chain's
state. Until the third 2026-09-24 pass it returned the ground state's
spectrum instead, silently (\S21).
...
and never falls back to ED. The reason is \texttt{promote\_mps}: this
submode solves two sectors on an internal clone and contracts their
states against each other, which needs both rebased onto the chain's
original dense indices. ED targets a sector by a different mechanism
(Section~\ref{sec:groundstate}) and deliberately provides no
\texttt{promote\_to\_dense}/\texttt{promote\_mps}, so there is nothing to
fall back \emph{to} --- and falling back to a plain ED solve would answer
with the \emph{global} excited states, a different calculation.
...
\textbf{The isotropic, degeneracy-averaged spin correlator.}
\texttt{sc.get\_full\_SS\_\allowbreak correlator(\allowbreak mode="ED",
i=0, j=None)} returns the
...
(\texttt{get\_gs\_manifold}), which is what an unpolarized INS or ESR
measurement of an isotropic magnet actually sees --- neither the
component sum nor the manifold average is a one-liner over
\texttt{get\_dynamical\_correlator}. \texttt{j} defaults to \texttt{i}.
It is \textbf{ED-only} and raises for any other mode; remaining keyword
arguments are forwarded to \texttt{get\_dynamical\_correlator}, so
\texttt{submode=} and the frequency grid still apply. See
\texttt{examples/dynamical\_correlator/full\_spin\_correlator}.
...
artifacts. \textbf{Not available in a stock checkout:} the
reconstruction itself lives in the third-party \texttt{dmrgpy.maxenttk}
(PyMaxEnt) module, which is not distributed with this package, so
\texttt{submode="maxent"} raises \texttt{NotImplementedError} until you
install it separately. The same is true of \texttt{submode="CVMimag"},
whose Pad\'e continuation needs \texttt{dmrgpy.padetk}.
...
estimator yet for a non-Hermitian spectral bound. The pair must come from
an NH-DMRG solve: after \lstinline|set_gs()|, \lstinline|set_initial_wf()|
or \lstinline|gs_energy(wf0=x, reconverge=False)| the chain holds a right
state with no left partner, and \texttt{submode="KPM"} raises
\lstinline|RuntimeError|, where until the 2026-09-25 fixes it paired that
state with the left eigenvector of an earlier solve and absorbed
$\langle\psi_L|x\rangle$ (0.93 on a 4-site chain) into the normalization.
\lstinline|gs_energy(wf0=x)| on a non-Hermitian chain raises
\lstinline|TypeError|, since NH-DMRG takes no start state, and with
\lstinline|reconverge=False| takes x, normalized, on every session backend.
Implemented so far for
...
three of \lstinline|dex| on either side, where the answer depends on
...
wider than the broadening, \lstinline|ex[nex-1] > delta|. The first alone
cannot see a cutoff that lies far above the levels it swallows, which is
where the answer depends on it most: a Hamiltonian written in small units
puts its whole spectrum below \lstinline|dex/3|, and the call then
averages over every state with equal weight, i.e.\ returns the
infinite-temperature spectrum, 0.738 of the peak off on a 6-site
Heisenberg chain with $0.3S^z_0$ written at $s=10^{-7}$, with no warning
before. There the remedy is a \lstinline|dex| in the Hamiltonian's own
units, \lstinline|dex=s*1e-5|, which is exact and quiet. A genuinely
degenerate multiplet stays quiet under the second test, since its width is
roundoff.
...
\emph{and} you have installed the separate \texttt{maxenttk} module it
needs. For
a non-Hermitian $H$, the dispatch is per submode, not wholesale:
\texttt{"KPM"} runs a genuine biorthogonal KPM (see above),
\texttt{"CVM"}/\texttt{"CVM\_explicit"} run the non-Hermitian
correction-vector resolvent (which \emph{is} their non-Hermitian
implementation, not a substitution for it), and
\texttt{"EX"}/\texttt{"maxent"} are backend-agnostic enough to work as
they are. Every other submode --- \texttt{"TD"}, \texttt{"TDZ"},
\texttt{"CVMimag"}, \texttt{"ROOTN"}, \texttt{"SECTOR"} --- raises
\texttt{NotImplementedError}. Before the 2026-08 audit these last ones
silently returned the \texttt{CVM\_explicit} resolvent instead, so a
caller's \texttt{submode=} was effectively ignored on a non-Hermitian
Hamiltonian (see Section~\ref{sec:whatraises}).
...
\texttt{evolve\_and\_measure} and \texttt{evolution\_ABA} take the same
keywords on both modes, \texttt{nt=1000}, \texttt{dt=1e-2} and
\texttt{h=} (the Hamiltonian to evolve under, the chain's own by
default), and raise \texttt{TypeError} on one they do not read. On
\texttt{mode="ED"} the propagator is
\texttt{scipy.sparse.linalg.expm\_multiply}, exact to rounding and
unitary, so ED is the reference at any \texttt{dt} and whatever the
energy origin of $H$. Until the third 2026-09-24 pass a misspelled
keyword (\texttt{DT=0.2}) ran silently at the defaults on DMRG,
\texttt{h=} raised on ED, the ED default was \texttt{nt=100}, and ED
integrated with RK45 at scipy's default tolerances, whose error grew with
\texttt{dt} times the absolute energy of the state: a constant $+20$
added to $H$ moved an ED trajectory $5.6\times10^{-4}$ off exact at
\texttt{dt=0.1} (\S21).
...
\begin{itemize}
  ...
  \item \texttt{"TDVP\_GSE"} (\texttt{itensor\_version} 3,
    ...
    route also handles a bond-dimension-1 (product-state) start fine, and
    so does \texttt{itensor\_version=3} since the third 2026-09-24 pass,
    where a start whose site 0 is one local basis vector (a product
    state, or any ladder operator or projector on site 0) used to lose
    the expansion at the left edge and leave site 0 frozen (\S21).
...
\propto e^{-\beta H}$, up to the error of the imaginary-time stepper
described below:
...
\textbf{The imaginary-time stepper.} $e^{-\beta H/2}$ is applied as
$n_{\rm st}$ steps, each the order-$k$ Taylor polynomial of
$e^{-\Delta\tau(H-E_{\rm ref})}$, with $E_{\rm ref}$ the middle of the
spectrum $[E_{\min},E_{\max}]$, $\Delta\tau=\beta/(2n_{\rm st})$ and
$n_{\rm st}=\max(1,\lceil\beta W/(2a)\rceil)$, where $a$ is
\texttt{anneal\_step} and $W=E_{\max}-E_{\min}$ comes from two band-edge
solves on a clone of the chain, on its own backend and mode. Every
eigenvalue then enters a step as $x=\Delta\tau(E-E_{\rm ref})\in[-a/2,a/2]$,
so the error depends on the dimensionless step $a$ and the order $k$
alone, in any units and at any constant offset, and converges as $a^k$.
Each polynomial is applied as its $k$ linear factors over its complex
roots, i.e.\ $k$ applications of $H$ per step, which runs on every
backend. At the defaults, \texttt{anneal\_step = 2} and
\texttt{anneal\_order = 8} (class attributes of
\texttt{Thermal\_Spin\_Chain}, which an instance overrides as it sets
\texttt{T}), the thermal energy of the open $S=1/2$ Heisenberg chain is
within $6.8\times10^{-7}$ (relative) of the Boltzmann value from 3 to 12
sites and $T$ from 0.05 to 10, for about $2\beta W$ applications of $H$.
The ground-state branch is taken at $T\le10^{-5}W$, and $T=\infty$ returns
the singlet purification without a step.
\texttt{thermal.anneal(..., step=, order=)} takes the same two directly,
and its \texttt{dbeta=} is an optional absolute cap on $\Delta\tau$.

Until the 2026-09-25b pass the stepper was first order,
$|\Psi\rangle\to(1-0.1H)|\Psi\rangle$ repeated \texttt{int(beta/2/0.1)}
times on the unshifted $H$, so its error was set by 0.1 times the
extensive energy ($-2.777892$ against the exact $-3.166396$ on 10 sites at
$T=0.5$), a constant offset changed the answer, every $T>5$ took no step at
all (a \texttt{ZeroDivisionError} for a float $T$, the $T=\infty$ state for
a numpy scalar), and it returned early once one step changed the state by
less than $10^{-7}$, so a Hamiltonian in small units came back at the wrong
...
Finite-temperature results from before are not comparable (\S21). The cost
now grows with $\beta W$: 8.6\,s where it was 4.9\,s on 12 sites at
$T=0.5$. See
\texttt{examples/finite\_temperature/\allowbreak thermal\_purification\_VS\_exact},
...
\texttt{tc.MBChain} holds the annealed state as a state set by hand, so
\texttt{vev()} and the correlators on \texttt{tc.MBChain} measure
$|\Psi(\beta)\rangle$ on every mode, and on the DMRG backends
\texttt{tc.MBChain.gs\_energy()} is the purified energy
$\langle\Psi(\beta)|H|\Psi(\beta)\rangle$ (on \texttt{mode="ED"} it stays
the lowest eigenvalue of $H$). Until the 2026-09-25 fixes
\texttt{gs\_energy()} returned the singlet Hamiltonian's energy ($-2.25$
against $-0.4164$ on a 3-site chain at $T=1$), a correlator re-solved the
plain ground state, and \texttt{vev()} and the KPM sum rule on
\texttt{mode="ED"} read the singlet state ($\langle S^z_0S^z_1\rangle$ 0.0
against $-0.0694$). Those two reference values are the first-order
stepper's of the time; the present stepper gives $-0.428230$ and
$-0.071372$, both exact. Note that a dynamical correlator of the purified state
under $H$ on the physical sites alone is not the thermal correlator, which
needs $H$ minus its ancilla copy, so of these only the static readers and
the sum rule are thermal quantities.
...
\texttt{topology.berry\_phase\_matrix(hkgen, nk=20)} computes this from a
callable \texttt{hkgen(k)} returning $H(k)$.

\textbf{Fermion parity.} For a fermionic chain, the total parity
...
The two \texttt{fpmode} values are the same quantity by two routes ---
\texttt{"full"} builds the whole parity string as one operator,
\texttt{"iterative"} applies it site by site --- and any other value
raises. See \texttt{examples/topological/parity} for a sweep of $P$
against chemical potential across the topological transition, and
\texttt{examples/topological/parity\_modes} and
\texttt{parity\_long\_chain} alongside it.
...
\texttt{MultiOperator} you passed to \texttt{set\_hamiltonian()} --- so
any spin Hamiltonian written out of one- and two-site
\texttt{Sx}/\texttt{Sy}/\texttt{Sz} terms works, including anisotropic
exchange and bonds that are not nearest-neighbour.
\texttt{meanfield.decompose\_spin\_hamiltonian(sc)} exposes that split
directly, returning the on-site coefficients $b[i,a]$, the exchange
$J[i,j,a,b]$ and any constant offset. A Hamiltonian this decoupling is
not defined for --- a non-spin operator, a three-site term, two factors
on the same site --- raises \texttt{ValueError} rather than being
silently approximated. Components that vanish are dropped below
$10^{-10}$ of the largest coefficient of the Hamiltonian, not below an
absolute $10^{-10}$, so the decoupling reads the same in any unit of
energy; before the 2026-09-25 fixes a model written below that scale gave
an empty mean-field Hamiltonian and raised, and one at $10^{-9}$ was
solved as the zero operator.
...
Hamiltonian \emph{is} the original model and the solver reproduces its
exact ground state. Keyword arguments are forwarded to
\texttt{gs\_energy}/\texttt{get\_magnetization}, so
\lstinline|mode="ED"| picks the ED solver; the mixing scheme is
\texttt{mixmode=} (\texttt{"default"} or \texttt{"broyden"}), since
\texttt{mode=} means the DMRG/ED solver here as everywhere else.
...
antiferromagnetic model start from a staggered \texttt{m0} and/or lower
\texttt{mix}. The loop stops after \texttt{maxite} iterations with a
...
broadly spread one. \texttt{sc.get\_distribution\_moments(...)} returns
the raw $\langle X^k\rangle$ moments both reconstructions are built from
--- the distribution counterpart of
Section~\ref{sec:dynamical}'s
\texttt{get\_dynamical\_correlator\_moments}. It is a DMRG/KPM quantity
with no ED implementation (the ED path builds spectra by explicit
summation, not from moments), so it raises
\texttt{NotImplementedError} under \texttt{mode="ED"}. One thing it does
\emph{not} share with Section~\ref{sec:dynamical}: its \texttt{delta} is
deliberately left off the broadening calibration described there, and
still only sets a polynomial count. This path expands an arbitrary
operator $X$ rather than the Hamiltonian, so there is no rescaled band
whose centre the FWHM $=2\delta$ relation could be anchored to; read
\texttt{delta} here as a resolution knob and compare distributions
computed at one value of it, not across values. What it does share with
every other distribution is the normalization, $\int P(x)\,dx=1$, and
under \texttt{mode="ED"} that held only since the 2026-09-24 audit: the
ED route used to return a distribution whose total weight was exactly
$1/$\texttt{scale} (0.1000 at the default \texttt{scale=10}), and with
\texttt{xs=} given its imaginary part was a copy of its real part. An
ED distribution from before therefore moves by exactly the factor
\texttt{scale} and is now real on the requested points, a 2-site
\texttt{itensor\_version=3} chain having reached that route without
asking for it through the automatic ED fallback, while nothing moved on
any DMRG backend (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md},
finding 3).
...
\begin{itemize}
  \item \textbf{Analytic continuation} (\texttt{analyticcontinuation.py}):
    ...
    \texttt{dmrgpy.padetk} module, which is not shipped with this
    package; without it both this tool and \texttt{submode="CVMimag"}
    raise \texttt{NotImplementedError}.
  ...
  \item \textbf{Maximum-entropy reconstruction} (\texttt{reconstruct.py}):
    ...
    submode and \texttt{get\_distribution\_maxent}. Requires the
    third-party \texttt{dmrgpy.maxenttk} (PyMaxEnt) module, which is not
    shipped with this package; without it importing
    \texttt{reconstruct} raises \texttt{ModuleNotFoundError} and the two
    callers above raise \texttt{NotImplementedError}.
\end{itemize}
...
\item \texttt{mode="ED"} (default): full exact diagonalization of the
  ...
  $T\geq0$. It reads only its named parameters, and since the
  2026-09-24 audit any other keyword raises \texttt{TypeError} naming
  it, where it used to be accepted and ignored, so that \texttt{Jrho=}
  for \texttt{Jrho\_s=} silently removed the whole third-order Kondo
  peak (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, finding
  11); the moment count of its KPM correlator is set through the chain's
  \texttt{kpm\_*} attributes, not through a keyword.
...
$F(eV-\epsilon_m,T)$, one for each of its two diagrams with different
arguments (see below), that produces the characteristic zero-bias
...
\textbf{Direct and exchange diagrams.} Every third-order process comes
in two interaction orders (the paper's ``normal'' and ``reversed''), and
reversing the order both reverses the electron-spin trace and turns the
intermediate electron from electron-like into hole-like, whose $F$
...
below); for the potential-interference term the trace is the symmetric $\delta_{kj}$,
so only the hole-like flip survives and the two orders \emph{subtract},
...
of the potential term identically, and the whole term vanishes at
$B=0$, so the zero-field Kondo peak keeps its position and symmetry at
$U\neq0$ (Fig.~7d at 0~T is exactly symmetric, peak 1.39 at $eV=0$).

\emph{Behaviour change, 2026-09-12.} Until then the potential term
used the \emph{summed} combination. That kept a spurious $m=i$ term, a
$\mathrm{sign}(eV)\,F(eV)$ spike at zero bias that shifted the
zero-field $U=0.25$ peak to $-0.2$~mV (1.47 instead of 1.39) and tilted
its tails (1.156/1.052 instead of 1.145/1.135), and it gave the
exchange diagram the wrong sign, so the 10~T step asymmetry came out
$\sim0.27$ against the figure's $\sim0.05$ (now 0.047 against a
digitized 0.054). Every \texttt{U!=0, order=3} result from before that
date is not comparable; \texttt{U=0} and \texttt{order=2} are
untouched.

\textbf{Where the exchange log sits, a departure from the paper.} The
direct diagram tunnels first: its intermediate state is the tunnelled
electron in the sample with the impurity in $m$, which goes on shell at
the sample's Fermi edge when $eV=\epsilon_m-\epsilon_i$, hence
...
shell when the hole reaches the Fermi edge, which fixes the
\emph{outgoing} electron's energy at $\epsilon_i-\epsilon_m$, and the
outgoing electron has $eV-\epsilon_{if}$. So the exchange log is
$F(eV-(\epsilon_f-\epsilon_m))$. The paper's eq.~25, and the formula
under its Fig.~6, has $F(eV-\epsilon_{mi})=F(eV+\epsilon_{im})$
instead, measured from the incoming energy as if the impurity absorbed
nothing. The two agree when $f=i$, so the potential term, whose $I_{fi}$
forces $f=i$, is the same either way, and whenever every state the
initial one connects to is degenerate with it (a free $S=1/2$ at $B=0$,
a Kramers doublet). Any field, anisotropy or exchange splitting
separates them. \texttt{tests/test\_kondo\_spectrum\_tmatrix.py} builds
the second-order T-matrix of the paper's own Hamiltonian as explicit
fermion $\otimes$ impurity operators, reading every intermediate state's
energy denominator off $H_0$, and the spectrum agrees with it to
roundoff on anisotropic $S=1$ and exchange-coupled impurities in tilted
fields, where the printed form missed it by 5--20\% of the third-order
term.

\emph{Behaviour change, 2026-09-26.} Every third-order Kondo spectrum
with an inelastic transition out of an occupied state moved, on
\texttt{mode="ED"} and \texttt{mode="DMRG"} alike; a free $S=1/2$ at
$B=0$, every zero-bias value, \texttt{order=2} and the potential term did
not. On the paper's own Fig.~7d parameters at 10~T the step overshoots
go from 1.226/1.183 to 1.248/1.203 and the $\pm4$~mV tails from
1.143/1.130 to 1.149/1.137, where the figure, drawn from eq.~25, reads
1.231/1.177 and 1.146/1.128; the step asymmetry, which is the potential
term, does not move. At $T=1$~K on an anisotropic $S=1$ ($D=1$~meV,
$E=0.3$~meV, 3~T) the largest move is 0.044 on a peak of 2.52. Results
from before are not comparable.
...
\item The paper's own closed-form equations for two numerical building
  ...
  rather than saturating, and the printed $F$, its eq.~22, has a log
  that does not depend on the integration variable and an overall sign
  that makes it negative). \texttt{kondospectrumtk/stepfunctions.py}
  uses the evident intent of each instead: $\Theta$ is re-derived from
  the paper's current formula, and $F$ is the closed-form log
  $\ln[(\omega_0+|\epsilon|)/|\epsilon+i\Gamma_0|]$ convolved with the
  thermal kernel $\Theta'$, i.e.\
  $F_0(\epsilon)=\ln(\omega_0+|\epsilon|)-\tfrac12\ln(\epsilon^2+\Gamma_0^2)$
  ...
  \texttt{tests/test\_kondo\_spectrum\_paper\_fig7.py}).

  \emph{Behaviour change, 2026-09-12.} Until then $F$ was the paper's
  \emph{electron-like} defining integral (its eq.~20) evaluated
  ...
  figure reads 0.886. The thermal broadening is unchanged (eq.~20's own
  double integral reduces to the same $\Theta'$ kernel: 7.459 vs 7.460
  at the Fig.~5 peak), so nothing changes at zero bias; every
...
\item \textbf{Third order} cannot be reduced to a two-operator correlator
  ...
  every step). $G$ carries the direct diagram; the exchange diagram,
  whose log sits at $eV-(\epsilon_f-\epsilon_m)$, needs that difference
  as a frequency, which $G$'s $t_2$ axis does not have, so it comes from
  the sheared function
  $G_x(s,\tau)=G(-s,\tau+s)=\langle\psi_l(-s-\tau)|S_k|\psi_j(-s)\rangle$,
  with $\psi_j(t)=e^{-i(H-E_0)t}S_j|\mathrm{GS}\rangle$ the $t_2$
  trajectories already computed (one more short $\tau$ trajectory per
  $l$ per row, 1.5x the cost). Both are then extracted via two
  closed-form time-domain kernels (derived by inverse-Fourier-transforming
  $\Theta_0$ and $F_0(eV-\cdot)$) rather
...
modes unless you say which one you want. \texttt{mode="ED"} takes the
$T\to0^+$ limit, the equal-weight average over the degenerate manifold,
while \texttt{mode="DMRG"} uses the single state its solver converged
to, which at an accidental crossing is whichever member, or
superposition, the random start reached, so the spectrum lands anywhere
between those of the members: on an $S=1$ impurity at the crossing of
its $|0\rangle$ and $|-1\rangle$ levels, between 1.0 and 2.0 against
the 1.5 of the average. Passing \texttt{n\_gs=g} asks
\texttt{mode="DMRG"} for that same average. Every term, second order,
third-order Kondo and potential, is then averaged with equal weight over
\texttt{get\_excited\_states(n=g, purify=True)}, which is exactly the
ED average, since every term is linear in the ground-state density
matrix, and it costs $g$ times the single-state run; on that crossing
\texttt{n\_gs=2} gives 1.50002 to 1.50004 in every run. The default
\texttt{n\_gs=1} is the unchanged single-state path, a
\texttt{RuntimeWarning} fires when a member lies more than
\texttt{delta} from $E_0$, and \texttt{n\_gs>1} runs on
\texttt{itensor\_version} 2, 3 and \texttt{"python"}, measures each
member exactly as returned, with \texttt{gs\_energy()} that member's own
energy, restores the chain afterwards, \texttt{gs\_energy()} included,
and accepts only the submodes that read the chain's state (KPM, CVM,
CVM\_explicit, ROOTN, TD, TDZ, EX), raising
\texttt{NotImplementedError} for SECTOR, any other submode and
\texttt{"julia\_live"} (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md},
finding 12, and the 2026-09-24 second-pass record, findings 13 to 15:
until then each member was re-swept before it was measured, and on v2
and v3 an excited member split below \texttt{delta} relaxed into the
lower one in 16 of 24 runs).

Further \texttt{mode="DMRG"} limitations beyond the general ones above:
the \texttt{es} frequency grid has no safe default and must be supplied
explicitly, and what it has to cover is set by the chain's spectrum
rather than by the \texttt{eV} sweep range, and differs between the
terms. The second-order term needs only the transitions below
$\max|eV|$, while the potential-interference term needs every
transition up to the top of the tip-coupled site's $S_k$ spectrum, a few
$\delta$ beyond it and $\omega=0$ itself, since its kernel falls off
only as about $2eV/\omega$ inside the band. One \texttt{es} is shared
by both, so with \texttt{U!=0} it must meet the second requirement, and
the potential term issues a \texttt{RuntimeWarning} when the sum rule
$\sum_k\int S_{kk}(\omega)\,d\omega=S(S+1)$ says more than $10^{-2}$
of the weight is missing (see \texttt{second\_order\_dIdV\_dc}'s and
\texttt{potentialdc}'s docstrings). The grid may be non-uniform and in
any order, since both terms sort it and then integrate with trapezoid
weights of the grid they are given (an unsorted grid, such as a refined
block appended after a coarse one, was silently wrong until the
2026-09-24 second-pass audit, 3.04 off a 6.97 peak on the second-order
term, finding 17); the potential term weighted every point with the first spacing until the
2026-09-24 audit (finding 13), and the one example of it,
\texttt{examples/kondo/kondo\_potential\_term\_dmrg\_VS\_ED}, violated
the coverage requirement and blamed the resulting 9.7 per cent
DMRG-versus-ED gap on KPM broadening, where with \texttt{es} spanning
$\pm20$ meV it is 2.1 per cent, measured before the moment-count cut
of finding 4 (finding 14). The third-order term's
\texttt{dt2}, \texttt{n\_t2\_half},
...
term). The second-order term (\texttt{submode="KPM"},
$\delta=2\times10^{-5}$) agrees with the exact excited-state sum to
0.2\% of its maximum at every bias point on the same chain. That figure
predates the 2026-09-22 KPM broadening calibration
(Section~\ref{sec:whatraises}) and has not been re-measured against it;
what the second-order term reads is a cumulative integral of
$S(\omega)$, which the sum rule pins independently of the moment count,
so the recalibration is not expected to move it. It was
quoted as ``a few tens of percent at thresholds'' until 2026-09-12; that
error was the route's own cumulative sum, which counted the whole
frequency bin holding a threshold's delta-like peak as lying below it
(1.033 against an exact 0.808 at $eV=0$), not KPM -- it is a trapezoid
rule now.
...
fixed-length system. A unit cell of any size is supported under
\texttt{gs\_method="vumps"} (the default), which routes
\texttt{n\_uc>2} to the sequential multi-site solver
\texttt{pyitensor/vumps\_ms.py}; \texttt{gs\_method="idmrg"}, the
growing algorithm, is limited to \texttt{n\_uc<=2} and raises
\texttt{NotImplementedError} above that -- see
...
be phrased in whichever direction reads most naturally). Terms of any
finite range are accepted; each is canonicalized by translating it, as a
whole, onto the cell its leftmost site lives in, so no bond is ever
double-counted between a cell and its neighbour:
...
\textbf{Couplings longer than one unit cell:
\texttt{get\_operator(..., group=c)}.} The three flat lists reach one
cell either way. For anything further out,
\texttt{ic.get\_operator(name, i, group=c)} takes an \emph{integer} cell
offset -- \texttt{c = -1, 0, 1} are exactly \texttt{L}, \texttt{C} and
\texttt{R}, and \texttt{c = 2} is the cell after the next one -- so a
$J_1$--$J_2$ (next-nearest-neighbour) chain fits on a one-site cell:
...
Both ground-state solvers handle this, on both backends.
\texttt{gs\_method="vumps"} (the default) routes a Hamiltonian whose
couplings exceed one unit cell to the \emph{sequential} multi-site solver
(\texttt{pyitensor/vumps\_ms.py}, or \texttt{Chain::vms\_ground\_state}
on \texttt{itensor\_version=3}), whose channel-resolved environments
carry one channel per site of a term's reach and so cost linearly in it --
where the grouped path it otherwise uses would need the same chain
rewritten on an $n_{\mathrm{uc}} \ge \mathrm{range}$ cell and folded into
a $d^{\mathrm{range}}$ supersite, i.e.\ exponentially.
\texttt{gs\_method="idmrg"}'s growth loop consumes the same automaton and
handles it too, needing no dispatch at all. \texttt{kpm\_finite} follows
along (it builds its own finite window).

One thing does not follow along.
\texttt{excitation\_energies}/\texttt{excitation\_gap} raise
\texttt{NotImplementedError} on either backend: the tangent-space ansatz
is built on the reach-1 environment triple, so there is no sequential
route for it to take -- the ground state on that same chain still works,
so rewrite it on a longer unit cell if you need the excitations. \texttt{vev}/\texttt{correlator}, by
contrast, do follow along, on both backends and at any reach:
\texttt{itensor\_version=3} reads whichever snapshot the run left behind
(\texttt{Chain::vms\_onsite\_expectation}/
\texttt{vms\_two\_point\_correlator} for a sequential answer, the grouped
\texttt{vumps\_*} pair otherwise), so nothing has to be rewritten to
measure a long-range chain.

See \texttt{tests/test\_infinite\_long\_range.py}, which pins a
polarized chain carrying reach-2 and reach-3 \texttt{Sz}-\texttt{Sz}
terms against its exact energy density (and its exact
\texttt{vev}/\texttt{correlator}), and
\texttt{examples/idmrg/long\_range\_infinite\_chain}, which sweeps
$J_2$ and checks the 1- and 2-site cells against each other.
...
\textbf{What these cost in memory}
(\texttt{itensor\_version="python"}). Every fixed point and every
correlator above is a contraction along the chain's transfer matrix, and
that chain used to be assembled as one \texttt{chi\^{}2 x chi\^{}2}
array per unit-cell position, so the peak memory of a run grew as
\texttt{chi\^{}4} and was reached before any physics happened. Since
2026-09-22 the chain is carried lazily instead, as the
\texttt{(chi\_l, d, chi\_r)} site tensors it is built from, and applied
one site at a time, so nothing on that path allocates the rank-4 array
at all. Measured on a 2-site Heisenberg cell at \texttt{maxm=64} under
\texttt{gs\_method="idmrg"}, threads pinned and seeded so the growth
trajectory is identical either way: the tracemalloc peak of the first
\texttt{vev} went from 517.8\,MB to 5.8\,MB and of the growth loop from
519.0\,MB to 10.4\,MB, peak RSS from 810.8\,MB to 304.1\,MB, and the
growth loop itself from 23.92\,s to 5.23\,s, with \texttt{e0},
\texttt{vev} and an \texttt{r=1..7} correlator sweep identical to every
printed digit. Nothing here changes a number, so read it as a budget you
can spend on a larger \texttt{maxm} rather than as a result to recheck;
what is left composing a full transfer matrix is the dense eigensolve
small chains take anyway and the fallback for a non-converged ARPACK
solve, so a small cell behaves exactly as it did.
\texttt{gs\_method="vumps"} goes through the same lazy chain once per
iteration and is a wash in time at \texttt{D=16} (16.39\,s to
16.11\,s on a critical Heisenberg cell, against a run-to-run spread on
that box several times larger), the win there being the memory alone.
...
VUMPS equivalent, see their own sections below) or if the growing
algorithm's own more battle-tested behavior is preferred (VUMPS's former
$D>1$ convergence-robustness gap has since been traced to two bugs and
fixed --- see the reliability note below):
...
\texttt{local\_excitation\_gap} is one of the two methods (with
\texttt{td\_dynamical\_correlator}, below) that still require
...
\textbf{A scoped reliability note.} \texttt{D>1} VUMPS was for a while
genuinely unreliable here --- independent calls at the same \texttt{D}
could land on noticeably different energies, and a \texttt{D=4} TFIM run
occasionally missed the exact answer by ${\sim}10\%$. That is no longer
the state of the code: it was two identified bugs, both since fixed. The
first was an environment fixed point closed against a missing conjugate,
invisible at \texttt{D=1} and a real source of wrong \texttt{D>1}
energies; with it fixed, the same \texttt{D=4} TFIM($g=1.5$) case
converges to ${\sim}10^{-7}$ relative on 10/10 independent
\texttt{nrestarts=6} calls. The second was the stopping criterion of the
inner eigensolves: they stopped on the Ritz \emph{value}, which leaves an
eigen\emph{vector} accurate only to the square root of its tolerance ---
fatal for VUMPS, whose convergence test compares \texttt{AC} and
\texttt{C}, two independently-solved eigenvectors. That floored the gauge
mismatch at ${\sim}10^{-6}$, so \texttt{tol=1e-10} was unreachable at any
number of iterations while the reported energy was perfectly good. Both
backends now stop on the residual instead: \texttt{D=8} TFIM went from
\texttt{converged} in 0/3 runs to 3/3 (and 32.6s to 6.7s at $g=1.5$,
38.4s to 4.6s at critical $g=1.0$), with the energies unchanged to every
printed digit. What remains is ordinary restart-search difficulty rather
than a known defect: the $D$-ramp, multi-restart and
variational-principle safety-net machinery is load-bearing
infrastructure, not vestigial, and for a \texttt{D>2} result on a harder
or less-tested model it is still worth calling \texttt{gs\_energy()} a few
times independently and keeping the lowest reported density. Always check
\texttt{.converged} --- it is reported honestly and never silently
assumed. See \texttt{pyitensor/vumps.py}'s own ``Convergence
robustness'' docstring section for the full numerical account. See
...
longer-range term (spanning more than 2 adjacent unit cells after
\texttt{n\_uc}-grouping) is rejected with
\texttt{NotImplementedError} by \texttt{excitation\_energies}/
\texttt{excitation\_gap} themselves: the ansatz's environments are the
reach-1 \{$G_L$, $G_R$, bond channels\} triple. The ground state on that
same chain is unaffected -- \texttt{gs\_energy()} routes it to the
sequential multi-site solver instead of raising.
...
dimension. Two things bound that, and since 2026-09-22 they bound it on
\textbf{both} backends rather than only on
\texttt{itensor\_version="python"}: the momentum-dependent channel
resolvents are built once per momentum and cached with their LU
factorization (they depend only on $k$ and the momentum-independent
environment, so rebuilding and refactorizing them inside every
application of $H_{\rm eff}(k)$ was pure waste), and above a size
threshold the eigenproblem is solved by Lanczos on that Hamiltonian's
action rather than by assembling the matrix at all. Neither changes any
returned number -- the two solver paths are cross-checked against each
other in \texttt{tests/test\_infinite\_chain.py} and
\texttt{tests/test\_vumps\_excitations\_v3.py}, agreeing to 4.6e-11 on
\texttt{itensor\_version=3} -- and a momentum scan at large $D$ is
substantially cheaper than it was: over a 3-momentum scan on
\texttt{itensor\_version=3} with threads pinned to one core, a $D=16$
TFIM chain went from 164.1\,s to 10.3\,s with the resolvent cache and to
1.8\,s with both halves, and an \texttt{n\_uc=2} Heisenberg chain at
$D=10$ from 15.6\,s to 5.4\,s to 1.2\,s. The two backends keep different
thresholds, and deliberately: \texttt{itensor\_version=3} switches to
Lanczos above dimension 64 rather than \texttt{"python"}'s 256, since one
application of $H_{\rm eff}(k)$ there solves four channel resolvents,
which puts the crossover below dimension 36. One design note if you ask
for $n>1$ on a cell whose dispersion is degenerate away from $k=0$, as
the \texttt{n\_uc=2} Heisenberg cell's is: a single Krylov space holds at
most one direction out of a degenerate eigenspace, so both solvers run
one deflated Lanczos per eigenvalue rather than one Krylov space for all
of them. A plain single-vector Lanczos there returns distinct eigenvalues
where two should coincide, every one of them a genuine eigenpair, so a
residual check passes it. This guide used to say that \texttt{"python"}
never had that exposure, and it did: above its dense threshold a single
ARPACK call for all $n$ from one constant start dropped one copy of a
degenerate level and returned the next distinct one in its place, 7 of
16 calls wrong over four momenta on the \texttt{n\_uc=2} critical
Heisenberg cell at \texttt{maxm=10} (dimension 300), the earlier ``not
exposed'' having been measured at dimension 12, where the Krylov basis
is the whole space. Since the 2026-09-24 audit it runs one deflated
Lanczos per value for $n\ge2$, as the C++ solver does, and agrees with
its dense path to $1.9\times10^{-15}$ there, while $n=1$ is the
unchanged single call (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md},
finding 15).
...
\texttt{excitation\_energies}, the mpscpp3 port is energies-only, so it
has neither the eigenvectors nor the mixed-transfer source vector a
weight is built from, and that pair is now the whole of what is missing
there: the solver work \texttt{excitation\_energies} was waiting on (the
cached, LU-factored channel resolvents and the Lanczos eigensolve) was
ported on 2026-09-22, so a dispersion scan costs the same on both
backends and only the weights are still one-sided
(\texttt{docs/idmrg\_improvement\_plan.md}) --- plus
...
...), independent of \texttt{ic}'s own \texttt{maxm}/etc.; a key the
temporary chain does not hold as a setting (a misspelling, a method or a
private name) raises \texttt{TypeError}, and so do
\texttt{itensor\_version} and \texttt{mode}, since the window always
runs on \texttt{itensor\_version="python"} (until the 2026-09-24
second-pass audit a misspelled key was stored where nothing reads it and
the call returned the default spectrum, finding 6); remaining
...
obtained via a spatial DFT ($S(k,t)=\sum_x e^{-ikx}S(x,t)$),
conjugated, and followed by the \emph{same} damping and direct
per-frequency sum (\texttt{delta} $\to$ Lorentzian broadening)
\texttt{submode="TD"} uses. The conjugation is the finite TD route's own
conjugate-at-return: $S(x,t)$ carries $e^{-i(H-E_{GS})t}$, and without
it every line came out at $\omega=-D_n$ instead of $+D_n$ on both
backends, mostly below the default \texttt{window=[-1,10]}, until the
2026-09-24 second-pass audit (finding 8). There is one deliberate
difference at the end: what comes back is the raw one-sided
transform, not the density of Section~\ref{sec:dynamical}. The reduction
that produces $S(k,t)$ never names the operator pair, so the
adjoint-pair identity \texttt{"TD"} completes its transform with (see
that submode) has nothing to act on here. Take the real part if you want
the density, which is the right thing to do whenever the Lehmann weights
are real, and note that this is the one dynamical-correlator route in
the library where that instruction still stands.
...
\texttt{ic.\_result} (\texttt{itensor\_version="python"} only --- unlike
\texttt{vev}/\texttt{correlator}, which work on both backends):
...
\texttt{"CVM"}/\texttt{"SECTOR"} correlators --- each checked against the
NumPy result. Three more \textbf{run and give the same answer but do not
stay on the device}: infinite chains (\texttt{gs\_method="idmrg"} and
\texttt{"vumps"}) and non-Hermitian DMRG (\texttt{nhdmrg}) move data to
the host on every iteration, so do not expect them to get faster on a GPU
(see \texttt{docs/gpu\_cpu\_performance.md}'s device-compatibility
section).
...
\begin{itemize}
  \item \texttt{backend.set\_pad\_bonds(K)} (with $K$ your bond
    ...
    inside one process, where it costs 6--31\%. It pads the state's
    bonds, never the Hamiltonian's, whose bond dimension does not move.
    The padding is exact for the state, and it leaves every method's
    algorithm alone except one-site TDVP, where a padded zero direction
    is a live basis vector the integrator can populate:
    \texttt{tevol\_method="TDVP\_GSE"} under padding used to run
    one-site TDVP on the manifold of bond dimension $K$ instead of its
    Krylov expansion, 0.49 off the unpadded run on a 10-site quench at
    \texttt{K=maxm}. Since the 2026-09-24 audit that route is exempt
    from the padding the way the Hamiltonian is, the padding being
    stripped once at trajectory entry from whatever state the evolution
    is handed (keyed on the state rather than on the flag since the
    2026-09-24 second-pass audit, finding 18, so a state padded earlier
    and evolved after the flag is cleared is stripped too), so a padded
    \texttt{TDVP\_GSE} run follows the unpadded one to roundoff
    (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, finding 16,
    and Section~\ref{sec:whatraises}). The one case still started padded
    is \texttt{submode="TDZ"} with \texttt{tevol\_method="TDVP\_GSE"}
    at \texttt{tdvp\_gse\_sweeps=0}, since TDZ carries its wavefunction
    between calls; at \texttt{tdvp\_gse\_sweeps>0} its first expansion
    strips the padding. Under JAX with \texttt{set\_jit("auto")} the
    exempt route retraces once per bond dimension it grows through,
    which is the cost padding is meant to remove, though that route never
    kept frozen shapes anyway.
    On a \emph{consumer} card skip it above small bond dimension
    entirely: padding trades arithmetic for shape stability, and where
    FP64 is 1/32-rate the arithmetic is the expensive half (measured on
    a GTX 1060: a win at \texttt{maxm=30}, 2.6$\times$ slower at
    \texttt{maxm=120}).
...
\texttt{kpm\_energy\_truncate=True} with far too small a \texttt{kpm\_scale} & plausible spectrum at the wrong energy & \texttt{RuntimeError} in the worst regime (see \texttt{docs/\allowbreak known\_issue\_\allowbreak kpm\_\allowbreak energy\_\allowbreak truncation\_\allowbreak window.md}) \\
...
\texttt{gs\_energy\_generalized()} on a chain below 2 sites (\texttt{"python"}) & the Rayleigh quotient of an untouched random state ($-0.3049$ for an exact $-0.5$) & \texttt{RuntimeError} \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{itensor\_version="python"}, and on \texttt{"julia\_live"} since
the 2026-09-25b pass, where it raised \texttt{BoundsError}.
\end{itemize}
...
\subsection*{The 2026-09 audit}

A second cross-backend audit
(\texttt{docs/audit\_2026\_09\_hole\_hunt.md}) recorded 36 confirmed
findings, all of them now fixed. The last to land was the iDMRG
matrix-free item, which stood as PARTIAL for a while because its compute
half arrived without its memory half; the second pass closed on
2026-09-22, and what it buys an infinite-chain run is in
Section~\ref{sec:infinite}. It also left two \emph{open items}, found while
...
correlator convention (O1) and \texttt{delta} meant a different width
under \texttt{submode="KPM"} on each solver (O2). The list below is the
physics-facing
half --- what changes for a user who never reads the architecture docs.

\textbf{Results that are not comparable across this change.} Read this
the way you read the two 2026-08 items above.

\begin{itemize}
\item \textbf{\texttt{submode="CVM\_explicit"} returned exactly twice the
correct spectrum}, at every frequency. The submode is DMRG-only
(\texttt{mode="ED"} raises \texttt{NotImplementedError}), and the factor
of 2 was the same on every DMRG backend, the fix being one
backend-agnostic line. Verified directly on the 6-site staggered Heisenberg
chain: the first four points came back as
\texttt{[0.2169497, 0.0920095, 0.1731231, 0.1051759]} against
\texttt{submode="CVM"}'s
\texttt{[0.1084749, 0.0460046, 0.086561, 0.0525879]}, i.e.\ a ratio of 2
to six digits; they now agree to 5.7e-07. Any peak height, integrated
weight or figure produced with \texttt{"CVM\_explicit"} before this is a
factor of 2 too large. This is the one correlator change that hits
Hermitian pairs as well.
\item \textbf{Three dynamical-correlator routes moved onto the house
convention} of Section~\ref{sec:dynamical} (the complex Lehmann density)
from $-\frac1\pi\mathrm{Im}\,G^R$: \texttt{mode="DMRG" submode="CVM"},
\texttt{submode="ROOTN"} under both \texttt{mode=} values, and
\texttt{mode="ED" submode="ED"} (at $T=0$ and $T>0$), which is the exact
reference every other submode is validated against. \textbf{Where every
$M_n$ is real nothing moves at all} --- the two conventions are then
identical (measured to 1.1e-16), and that covers every
\texttt{name=(A,B)} example in this guide, Hermitian pairs and the
$S(q,\omega)$ sweep's $(S^z_i,S^z_j)$ alike; Section~\ref{sec:dynamical}
states the criterion and why $A=B^\dagger$ is not it. For a pair with
complex $M_n$ the curves move by more than their
own peak: on a 4-site complex-hopping chain with $A=c^\dagger_0$,
$B=c_2$ at $\delta=0.15$ the two conventions differ by up to 5.720e-01
against a peak of 0.61. \texttt{submode="ED"} also returns a complex array now where it
returned \texttt{float64} before (same real part, to the digit; the
imaginary part it used to drop peaks at 0.5682 on that pair), so a caller
that assumed a float array should take \texttt{.real}.
\texttt{submode="KPM"} (the default), \texttt{"INV"}, \texttt{"CVM"}
under \texttt{mode="ED"}, \texttt{"EX"}, \texttt{"TD"}, \texttt{"TDZ"}
and \texttt{"SECTOR"} were untouched \emph{by that change};
\texttt{"TD"}, \texttt{"TDZ"} and \texttt{"KPM"} moved later, on
2026-09-22, for the two reasons in the following two entries.
\texttt{mode="ED" submode="ROOTN"}
and \texttt{mode="DMRG" submode="ROOTN"} are also roughly \textbf{2x
slower} now, for the reason given in Section~\ref{sec:dynamical}.
\item \textbf{\texttt{submode="TD"} and \texttt{submode="TDZ"} now
return the density too}, where they used to return the complex one-sided
transform it is assembled from (the audit's open item O1, closed
2026-09-22). Section~\ref{sec:dynamical}'s \texttt{"TD"} entry carries
the identity and the cost rule; what matters for a result you already
have is that the array moved, and that the imaginary part moved most. On
the audit's 4-site complex-hopping chain ($A=c^\dagger_0$, $B=c_2$,
$\delta=0.4$, \texttt{dt=0.1}, exact peak 0.2313) the distance to the
exact Lehmann density went from 1.16e-01 to 2.57e-04 under
\texttt{mode="DMRG"}, from 2.76e-01 to 2.57e-04 under
\texttt{mode="ED"}, and from 1.13e-01 to 3.31e-02 for \texttt{"TDZ"}, a
residual this entry used to put down to the contour and which was in
fact the frequency stage's FFT grid; since the 2026-09-24 audit (finding
7, in the subsection below) \texttt{"TDZ"} lands 5.4e-04 from exact and
\texttt{"TD"} 3.2e-05. On a Hermitian pair the
imaginary part used to run at 60\% of the peak (1.11e-01 against 0.1869
on a 4-site Heisenberg chain at $\delta=0.3$) and is now exactly zero,
with the curve itself landing 2.82e-04 from exact. Taking
\texttt{.real} of the result, which this guide used to instruct, is now
a no-op on a real-weight pair and \emph{wrong} on a complex-weight one.
\texttt{mode="ED"} moved further than \texttt{mode="DMRG"} because that
route additionally read the operator pair in the opposite order,
returning $S_{BA}$ where every other route returns $S_{AB}$; it was
invisible whenever $A$ and $B$ are the same operator, which is every
example here. A pair that is not provably its own adjoint now costs two
evolutions rather than one.
\item \textbf{\texttt{delta} under \texttt{submode="KPM"} is now the
same broadening it is in the resolvent submodes}, FWHM $=2\delta$ at the
band centre, on both solvers (the audit's open item O2, closed
2026-09-22), and \texttt{kpm\_n\_scale}'s default changes from 3 to 1
because the base moment count it multiplies is now the calibrated one.
Neither solver realized the intended width before, and they missed it
differently: \texttt{mode="ED"} rescaled onto a window three times the
bandwidth and counted moments by its own rule, giving about
$0.93\delta$, while \texttt{mode="DMRG"} gave about $1.6\delta$. Every
KPM curve therefore changes width, and the two solvers stop disagreeing:
measured with $A=B=S^z_0$ on three chains,
$\max|\mathrm{ED}-\mathrm{DMRG}|$ against the resolvent peak went from
95 to 124 per cent down to 4 to 7 per cent, and to 0.07 to 0.2 per cent
on \texttt{itensor\_version="python"} once the 2026-09-24 audit cut the
DMRG routes to the calibrated moment count (finding 4, in the subsection
below). Peak heights change with the
width and the sum rule does not, which is exactly why nothing caught
this: both routes integrated to 0.250000 against an exact 0.250000
before and after. A script that was tuning \texttt{delta} by eye against
a KPM curve, or carrying \texttt{kpm\_n\_scale=3} explicitly, gets a
different spectrum now. \texttt{get\_distribution()}'s own KPM path is
deliberately left off the calibration
(Section~\ref{sec:densitymatrices}).
\item \textbf{\texttt{vev(op, npow=n)} on an ED route} returned $\langle
O\rangle$ for every $n$; it now returns $\langle O^n\rangle$. Measured on
a 4-site Heisenberg chain, \texttt{vev(Sz[0], npow=2, mode="ED")} went
from 5.6e-17 (i.e.\ $\langle S^z_0\rangle$) to 0.25.
\item \textbf{\texttt{gs\_energy\_fluctuation} on an ED route} inherited
that and also ignored \texttt{mode=}. On the same chain
\texttt{sc.mode="ED"} reported 2.0561 and now reports 0.0; a 2-site chain
on the \emph{default} backend (which \texttt{mode.py} routes to ED by
itself) reported 1.1456 and now reports 1.05e-08. Anything using it as a
convergence criterion behaves differently on those routes. DMRG-route
values are bit-unchanged.
\item \textbf{\texttt{exponential(h, wf)} on the DMRG backends} --- see the
\emph{MPS and operator algebra} paragraph above. Measured as
$\langle\mathrm{GS}|e^{zH}|\mathrm{GS}\rangle$ on a 4-site Heisenberg
chain under \texttt{itensor\_version="python"}, against the ED value:
0.6776 vs 0.6676 at $z=0.25$, 0.5184 vs 0.4457 at $z=0.5$, 0.6897 vs
0.1987 at $z=1$ (1.5\%, 16\%, 247\%). It now agrees with ED to
$\sim$1e-8 at all three.
\item \textbf{Boson occupation projectors under \texttt{mode="ED"}} were
wrong on any Hamiltonian that does not conserve the total boson number.
On a 3-site, 4-level chain with an $a_i+a^\dagger_i$ drive, site 0's
$P(n)$ read \texttt{[0.0042, 0.0054, -0.048, 0.0706]}, summing to 0.032
--- a negative probability included. It now reads
\texttt{[0.0322, 0.2393, 0.4732, 0.2553]}, summing to 1.0000 and matching
DMRG. A number-\emph{conserving} Hamiltonian is unaffected to machine
precision, which is why nothing caught this; $\langle n_i\rangle$, the
energies and every DMRG backend were always right.
\item \textbf{\texttt{SpinBoson\_Chain(..., maxnb=[...])}} silently built
the 4-level site regardless:
\texttt{SpinBoson\_Chain(["B","S=1/2"], maxnb=[6,None])} gave sites
\texttt{[104, 2]} and now gives \texttt{[106, 2]}. Earlier results at a
non-default \texttt{maxnb} are results at \texttt{maxnb=4}. With no
\texttt{maxnb=} nothing changes.
\item \textbf{\texttt{itensor\_version="python"}}, several ways. Its
\texttt{applyMPO} used to truncate in a gauge where discarding the
smallest singular values is not the optimal truncation, so every
repeated-application consumer moved at the 1e-5..1e-9 level:
\texttt{applyoperator}, \texttt{vev(..., npow>1)},
\texttt{gs\_energy\_fluctuation} (5.06e-05 $\to$ 6.27e-06 on a 10-site
Heisenberg chain at \texttt{maxm=30} --- the old number was mostly the
MPO-application error rather than the state's), the KPM Chebyshev
recursion and therefore every KPM correlator and
\texttt{get\_distribution}, CVM, \texttt{applyinverse}, and the
MPO-Taylor evolution paths. The new values are the ones that match
\texttt{itensor\_version=3} and ED. The product is now formed exactly and
truncated once afterwards, which is more work per application; the cost
is workload-dependent, so budget for it rather than expecting a fixed
factor. Separately, its TDVP real-time evolution
did not canonicalize the state before the first half-sweep, so a
trajectory whose start state was not already in that gauge took its
first step under an unprojected generator; the audit record
(\texttt{docs/audit\_2026\_09\_hole\_hunt.md}, finding \#1) carries the
reproduction and the mechanism. Which start states escaped it is not
recorded there and is not claimed here. And a chain object reused
across several
\texttt{set\_hamiltonian()} calls could keep a start state belonging to
the previous Hamiltonian. Both are fixed; no \texttt{itensor\_version}
other than \texttt{"python"} was affected by any of this.
\item \textbf{\texttt{promote\_to\_dense()} now keeps the sector's
energy on \texttt{itensor\_version="python"}} --- the guarantee
the sector-targeting part of the ground-state section states (``a bare
\texttt{gs\_energy()}
afterwards still returns the sector's energy rather than re-solving'')
was true on \texttt{itensor\_version=3} and \texttt{mode="ED"} but not
...
Hubbard chain confined to \texttt{Nf=3} the audit recorded a
post-promotion energy of $-2.3399130755$ with $\langle N\rangle=2$
against the sector's own $-1.9408140222$ with $\langle N\rangle=3$; it
now returns the sector value, re-measured here. Any \texttt{"python"}
result that called \texttt{gs\_energy()}/\texttt{get\_gs()} after
\texttt{promote\_to\_dense()} --- the documented workflow --- is a
different number than it was. The other backends are unchanged, and a
chain where the sector and global ground states coincide never saw it.
\end{itemize}

\textbf{And these now raise, or now work, where they used to be silent:}
...
\textbf{Call} & \textbf{Was} & \textbf{Now} \\
\midrule
\texttt{Bosonic\_Chain(n, maxnb=[...])} with \texttt{maxnb != 4} on \texttt{itensor\_version=2} & SIGABRT inside ITensor & \texttt{ValueError} naming the working backends \\
\texttt{itensor\_version=} on \texttt{Bosonic\_\allowbreak Chain}, \texttt{SpinBoson\_\allowbreak Chain}, \texttt{Parafermionic\_\allowbreak Chain}, \texttt{Spin\_\allowbreak Fermion\_\allowbreak Hamiltonian} & \texttt{TypeError: unexpected keyword argument} & honored \\
\texttt{SpinBoson\_Chain(["boson", ...])}, or any unknown site label & \texttt{RuntimeError: No active exception to reraise} & \texttt{ValueError} listing the accepted labels \\
\texttt{SpinBoson\_Chain(..., maxnb=[...])} of the wrong length, or \texttt{n=} contradicting \texttt{len(sitesin)} & silently dropped & \texttt{ValueError} \\
\texttt{SpinBoson\_Chain} under \texttt{mode="ED"} & failure inside an unimplemented stub & \texttt{NotImplementedError} naming the DMRG alternatives \\
\texttt{vev(..., npow=n)} with \texttt{n<0}, or with \texttt{n!=1} at $T>0$, \textbf{on an ED route} & accepted silently, answering with $\langle O\rangle$ & \texttt{ValueError} / \texttt{NotImplementedError} (the DMRG routes still answer) \\
\texttt{gs\_energy\_fluctuation(npow=...)} & accepted and misused & \texttt{TypeError} \\
\texttt{exponential(h, wf)} with \texttt{h} neither Hermitian nor anti-Hermitian, on DMRG & a warning and a number & \texttt{NotImplementedError} \\
\texttt{applyoperator}, \texttt{summps}, \texttt{applyinverse}, \texttt{scale\_mps}, \texttt{exponential} with a \texttt{mode=} naming the other backend & ran the wavefunction's own backend anyway (or a bare \texttt{raise}) & \texttt{TypeError} saying which backend the state belongs to \\
\texttt{get\_rdm()} on any ED route & \texttt{AttributeError} several frames deep & \texttt{NotImplementedError} (on \texttt{"julia\_live"} since the 2026-09-25b pass) \\
the bond entanglement entropy on any ED route & \texttt{AttributeError} several frames deep & \texttt{NotImplementedError} pointing at \texttt{get\_site\_\allowbreak entropy}, \texttt{get\_pair\_\allowbreak entropy} (on \texttt{"julia\_live"} since the 2026-09-25b pass) \\
\texttt{get\_bond\_entropy(wf, i, j)} with a site out of range & the audit recorded a SIGABRT on the C++ backends & \texttt{IndexError} \\
\texttt{get\_hamiltonian()} before \texttt{set\_hamiltonian()} & \texttt{'int' object is not iterable}, or \texttt{takes 0 positional arguments} & \texttt{ValueError} naming the fix \\
a misspelled \texttt{ctmode}/\texttt{dmmode}/\texttt{fpmode} & \texttt{RuntimeError: No active exception to reraise} & \texttt{ValueError} listing the accepted values \\
a misspelled \texttt{basis=} in \texttt{get\_correlation\_matrix} & the electron-basis matrix, silently & \texttt{ValueError} \\
\texttt{submode="CVM\_explicit"} with $A^\dagger\neq B$ & a \texttt{print} then \texttt{RuntimeError: No active exception to reraise} & \texttt{NotImplementedError} naming the submode; since the 2026-09-25b pass it returns $C[A,B]$ \\
\texttt{get\_distribution()} with no \texttt{X=} & \texttt{RuntimeError: No active exception to reraise} & \texttt{TypeError} naming \texttt{X=} \\
\texttt{MultiOperator / "a"}, \texttt{*} by an ndarray, or \texttt{multioperator.\allowbreak obj2MO} on an unrecognized object & \texttt{RuntimeError: No active exception to reraise} & \texttt{TypeError} naming what was passed \\
\texttt{sc.mode = "dmrg"} (any unrecognized spelling) & accepted, and silently decided by the automatic fallbacks & \texttt{ValueError} \\
a conserved sector under \texttt{mode="ED"} + the \texttt{get\_correlation\_*} family & \texttt{AttributeError} & works, via the backend-agnostic \texttt{dmmode="explicit"} \\
\texttt{get\_dynamical\_correlator()} on \texttt{Parafermionic\_Chain} without an explicit \texttt{mode=} & the audit recorded a process abort & works, and obeys \texttt{self.mode} and the automatic ED fallbacks like every other chain class \\
\bottomrule
\end{tabular}
\normalsize
\end{center}

\texttt{Parafermionic\_Chain} lost its own
\texttt{get\_dynamical\_correlator} override in the process --- it was
the base-class method minus the mode resolution and minus the
\texttt{name="..."} string resolution --- so both the documented string
form of \texttt{name=} and \texttt{mode=}/\texttt{self.mode} work there
now. \texttt{Parafermionic\_Chain.test(ntries=...)} is honored too, as
\texttt{Spin\_Chain.test(ntries=...)} already was.

\subsection*{The 2026-09-24 audit}

A third hunt (\texttt{docs/audit\_2026\_09\_24\_hole\_hunt.md}, five
lenses over the commits \texttt{1c2606d..765b537} that closed the
2026-09 audit's open items) recorded 16 confirmed findings. Every code
defect among them is fixed, with one residual kept by design,
\texttt{submode="TDZ"} under \texttt{set\_pad\_bonds} at
\texttt{tdvp\_gse\_sweeps=0} still being started from the padded state
(Section~\ref{sec:gpu}), and finding 6, which was documentation only,
is closed by the \texttt{"KPM"} entry of Section~\ref{sec:dynamical}. Several of the findings corrected what the
entries above say, the \texttt{"TDZ"} residual and the KPM agreement
between the two solvers among them, and those entries now point here.
Each item below names its finding, and the record carries the
reproduction that was actually run.

\textbf{Results that are not comparable across this change.}

\begin{itemize}
\item \textbf{\texttt{submode="TDZ"}, and \texttt{submode="TD"} at
\texttt{predict=False}, were evaluated on an FFT grid} of spacing
$2\pi/(n_t\,dt)$, about $1.05\delta$ at the default
\texttt{damping\_periods=6}, and interpolated linearly onto
\texttt{es}, meaning that a Lorentzian of half-width $\delta$ got about
one sample per $\delta$. The damped sum is now evaluated at each
requested frequency. On the 2026-09 audit's complex-hopping chain at
$\delta=0.4$ (exact peak 0.2313) the distance to the exact density goes
from 3.314e-02 to 5.408e-04 for \texttt{"TDZ"} and for \texttt{"TD"} at
\texttt{predict=False}, and from 2.571e-04 to 3.169e-05 for
\texttt{"TD"} at its default, and the returned curves move by up to 14
per cent of the peak for the first two (19 per cent at $\delta=0.25$)
and by 2.89e-04 for the third. \texttt{sxt\_to\_skomega}, the
infinite-chain $S(k,\omega)$ reduction, stayed on the FFT stage and
returned the same bits as before (Section~\ref{sec:dynamical}, finding
7), until the second pass below.
\item \textbf{Every DMRG KPM spectrum moves by roughly $2/N$ onto the ED
one}, $N$ being the calibrated moment count, since the DMRG routes
reconstructed from $N+2$ moments. On two decoupled Heisenberg dimers the
band-centre peak goes from 0.666761 to 0.620591 at $\delta=0.2$ and from
1.266336 to 1.220236 at $\delta=0.1$, on \texttt{itensor\_version=3} and
\texttt{"python"} alike, both now equal to the ED peak, and the three
residuals of the 2026-09 record's O2 table go from about 1.3e-02 to
2.55e-04, 2.09e-04 and 3.50e-04 on \texttt{"python"}.
\texttt{mode="ED"} does not move (Section~\ref{sec:dynamical}, finding
4).
\item \textbf{\texttt{get\_distribution(mode="ED")} moves by exactly
the factor \texttt{scale}}: on a 4-site open Heisenberg chain with
$X=S^z_0$ at the default \texttt{scale=10} and $\delta=0.05$ the total
weight goes from 0.1000 to 1.0000 and the peak from 0.517316 to
5.173159, and with \texttt{xs=} the imaginary part, which was a copy of
the real one, goes from $\max|\mathrm{Im}\,y|=4.96$ to exactly 0.
Nothing moves on any DMRG backend
(Section~\ref{sec:densitymatrices}, finding 3).
\item \textbf{\texttt{ISy} in the first operator of a correlator}
returned exactly minus the correlator under \texttt{"KPM"},
\texttt{"EX"}, \texttt{"TD"} and \texttt{"TDZ"}, since
\texttt{get\_dagger()} treated this anti-Hermitian name as Hermitian. On
a 4-site Heisenberg chain in a field, the KPM curve of
\texttt{(get\_operator("ISy",0), Sz[1])} goes from exactly minus the
curve of \texttt{(1j*Sy[0], Sz[1])} to identical to it, and its first
moment from +0.079470 to $-0.079470$ (exact $-0.079685$);
\texttt{sc.is\_hermitian(ISy)} is now \texttt{False} and
\texttt{sc.is\_hermitian(1j*ISy)} \texttt{True}, where both were the
other way round. Only operators that contain \texttt{ISy} move
(Section~\ref{sec:dynamical}, finding 2).
\item \textbf{An operator that mixes pre-Jordan-Wigner \texttt{C}-type
names with bare \texttt{A}-type ones} was canonicalized with the wrong
sign, so \texttt{simplify()} changed its value and the Hermiticity, zero
and adjoint-pair proofs could be false. Such terms are now left as
written (see \emph{Building a Hamiltonian and observables}). A
\texttt{"TD"} pair of that kind on a 4-site complex-hopping chain goes
from 3.616e-02 off exact, with one evolution, to 3.063e-05 with two, on
\texttt{"python"} and v3 alike, and no dmrgpy chain class builds such a
term, so no ordinary Hamiltonian moves (finding 1).
\item \textbf{\texttt{mode="ED"} \texttt{submode="TD"} on a degenerate
ground state} built the two halves of the density on two different
randomly chosen ground states: three identical calls on a degenerate
3-site Heisenberg chain spread by 2.03e-01 against a 1.768e-01 peak.
Both halves now use the cached ground state, so the calls are identical
and agree with \texttt{"KPM"}, \texttt{"INV"}, \texttt{"CVM"} and
\texttt{"ROOTN"}, though still not with \texttt{submode="ED"}'s average
over the manifold, a split between the two ED conventions that predates
this fix. \texttt{evolution\_ABA(mode="ED")} and
\texttt{evolve\_and\_measure(mode="ED")} without \texttt{wf=} change the
same way, and a non-degenerate ground state moves by at most 3e-11
(finding 8).
\item \textbf{The Kondo potential-interference term on a non-uniform
\texttt{es}} weighted every point with the grid's first spacing. On a
single $S=1/2$ at 10 T the peak on a sinh-mapped grid goes from 68.2332
to 0.6704 against an exact 0.6709, and a uniform grid moves by 2.4e-08
(Section~\ref{sec:kondospectrum}, finding 13).
\item \textbf{\texttt{excitation\_energies(k, n)} with \texttt{n>=2} on
\texttt{itensor\_version="python"}}, and
\texttt{spectral\_weights}/\texttt{dynamical\_structure\_factor} with
it, could drop one copy of a degenerate level of $H_{\rm eff}(k)$ above
the dense threshold (dimension 256) and return the next distinct level
in its place. At $k=0.37$ on the \texttt{n\_uc=2} critical Heisenberg
cell at \texttt{maxm=10}, \texttt{[0.289896072 0.291597913
0.313428946]} becomes \texttt{[0.289896072 0.289896072 0.291597913]},
and the $S^x$ and $S^z$ weight fractions go from 0.058 to 0.184014 and
from 0.339 to 0.041282, both now equal to the dense path's.
\texttt{n=1}, \texttt{excitation\_gap} and dimensions up to 256 are
byte-identical (Section~\ref{sec:infinite}, finding 15).
\item \textbf{\texttt{tevol\_method="TDVP\_GSE"} under
\texttt{set\_pad\_bonds(K)}} on \texttt{itensor\_version="python"} ran a
different algorithm, one-site TDVP on the manifold of bond dimension
$K$, its bond growth coming from a QR completion of the padded zero
directions rather than from the Krylov expansion. Padded runs through
\texttt{quench\_tdvp\_gse} and \texttt{evolve\_and\_measure\_tdvp\_gse}
now follow the unpadded ones: on a 10-site XXZ quench at
$K=\texttt{maxm}=4$ the two went from 0.4928 apart to 8.9e-16 at
\texttt{tdvp\_gse\_sweeps=0} and from 4.1e-06 to 1.7e-07 at
\texttt{tdvp\_gse\_sweeps=3}. Note the direction at
\texttt{tdvp\_gse\_sweeps=0}, where the padded run used to sit 9.8e-05
from ED only because the larger manifold held the answer, and now sits
0.4929 from it, like the unpadded frozen product state, which is the
correct one-site answer (Section~\ref{sec:gpu}, finding 16).
\end{itemize}
...
\textbf{Call} & \textbf{Was} & \textbf{Now} \\
\midrule
\texttt{kpm\_n\_scale} not a positive integer, on \texttt{"python"}, \texttt{mode="ED"} or \texttt{"julia\_live"} & rounded down silently (1.5 gave 1x, anything below 1 the 16-moment floor) & \texttt{TypeError}/\texttt{ValueError} naming it, as v2/v3 already did; \texttt{kpm\_n\_scale=2.0} raises too \\
a misspelled keyword to \texttt{submode="TDZ"} (\texttt{alpha=}, \texttt{nmax=}) & ignored, the default-parameter spectrum bit for bit & \texttt{TypeError} \\
a \texttt{toMPO()} operator to \texttt{submode="TDZ"} & a crash several frames deep & \texttt{TypeError} naming \texttt{toMPO} \\
\texttt{get\_dynamical\_\allowbreak correlator\_MB(name="ZZ", i=1, j=1)} under \texttt{"TD"}/\texttt{"TDZ"} & $C[S^z_0,S^z_0]$, the sites dropped & the sites asked for (the public route was never affected; \texttt{"ROOTN"} on this route followed on 2026-09-25) \\
\texttt{get\_kondo\_spectrum(mode="ED")} with a keyword it does not read & ignored, so \texttt{Jrho=} for \texttt{Jrho\_s=} removed the Kondo peak & \texttt{TypeError} naming the unknown keywords \\
\texttt{get\_kondo\_spectrum(mode="DMRG")} at a degenerate ground state & one arbitrary member's spectrum & the same by default; \texttt{n\_gs=g} gives the equal-weight average over the manifold that \texttt{mode="ED"} takes \\
the potential term with an \texttt{es} that misses spectral weight & a silently low spectrum & \texttt{RuntimeWarning} from the sum rule \\
\bottomrule
\end{tabular}
\normalsize
\end{center}

\subsection*{The 2026-09-24 second pass}

A fourth hunt (\texttt{docs/audit\_2026\_09\_24b\_hole\_hunt.md}, five
lenses over the single commit \texttt{30200a4} that closed the third)
recorded 18 confirmed findings, twelve of them older than that commit and
reached by probing next to it, and every one is fixed. Each item below
names its finding; the record carries the reproduction that was actually
run.

\textbf{Results that are not comparable across this change.}

\begin{itemize}
\item \textbf{Every infinite-chain $S(k,\omega)$}, on v3 and
  \texttt{"python"}, was mirrored in frequency and interpolated off an FFT
  grid. It is now conjugated after the spatial sum and evaluated directly
  at each frequency: on the transverse-field paramagnet the $k=\pi$ magnon
  moves from $\omega=-1.045$, with 0.928 of its weight below zero, to
  $+0.905$ against $\varepsilon=0.900$, and on a single magnon at
  $\delta T=6$ the curve goes from 21 per cent of the peak off the closed
  form to $2\times10^{-14}$ (Section~\ref{sec:infinite}, findings 7 and 8).
\item \textbf{\texttt{evolve\_and\_measure} and \texttt{evolution\_ABA} on
  \texttt{mode="ED"} ran backwards in time}, and on every DMRG backend
  returned the conjugate of $\langle O\rangle$. Larmor precession of a
  $+x$ chain in a field now gives $\langle S^y_0\rangle(t=1)=+0.4207$ on
  ED, where it gave $-0.4207$, and $O=S^z_0+iS^x_0$ on an eigenstate now
  gives $+0.5i$ on DMRG, where it gave $-0.5i$. A real Hamiltonian, a real
  start and a Hermitian observable with real matrix elements, which is
  what every earlier test measured, do not move (findings 9 and 10).
\item \textbf{A state set by hand now reaches the solver.} After
  \texttt{set\_gs()} every DMRG correlator measured the session's own
  solved state and then overwrote the chain's with it (TD on a pure
  doublet member goes from $2.04\times10^{-1}$ to $3.3\times10^{-6}$ off
  exact), and \texttt{set\_initial\_wf}/\texttt{set\_initial\_wf\_guess}
  never reached the session, so the transverse-field Ising example
  plotted $M_z/n\approx0.001$ in the ordered phase where the seeded branch
  gives 0.4998 (findings 11 and 12).
\item \textbf{\texttt{get\_kondo\_spectrum(mode="DMRG", n\_gs>1)}}
  re-swept each member before measuring it, so on v2 and v3 an excited
  member split below \texttt{delta} relaxed into the lower one (1 of 6
  runs at the average 1.5, now 6 of 6), left \texttt{gs\_energy()} at the
  last member's energy afterwards, and was inert under
  \texttt{submode="EX"}, which now measures from the chain's own state
  (1.1392 to 1.8711 across seeds, now 1.512780 on every one; findings 13
  to 15).
\item \textbf{Smaller moves}: the Kondo terms on a non-increasing
  \texttt{es} (3.035 to 0.03183 off exact on a refined block appended
  after a coarse one, finding 17); a padded ground state evolved with
  \texttt{set\_pad\_bonds} cleared under \texttt{TDVP\_GSE} at
  \texttt{tdvp\_gse\_sweeps=0} (0.4929 to $1.2\times10^{-15}$ off the
  unpadded run, finding 18); \texttt{mpsalgebra.disentangle\_manifold} on
  a Hermitian operator the canonical proof cannot see (a non-orthonormal
  basis, Gram error 0.32, now $10^{-15}$, and proven operators move at the
  gauge level, finding 1).
\end{itemize}

\textbf{And these now raise where they used to be silent:}
...
\textbf{Call} & \textbf{Was} & \textbf{Now} \\
\midrule
KPM with a pole outside the rescaled window (\texttt{kpm\_scale} below 1/2) & DMRG spectra up to 109 times the true peak, ED integrals up to $10^6$ & \texttt{RuntimeError} on every backend, the guard being 1.5 times the exact moment bound (findings 2 and 3) \\
\texttt{kpm\_energy\_truncate=True} on \texttt{mode="ED"} & the untruncated curve & \texttt{NotImplementedError}, also through \texttt{mode.py}'s fallback (finding 4) \\
\texttt{kpm\_n\_scale=1.5} on a non-Hermitian \texttt{mode="ED"} KPM & \texttt{TypeError}, where \texttt{mode="DMRG"} accepted it & accepted on both, neither reading it (finding 5) \\
a misspelled key, a method or \texttt{itensor\_version} in \texttt{kpm\_finite}'s \texttt{window\_chain\_kwargs} & ignored, the default spectrum bit for bit & \texttt{TypeError} (finding 6) \\
\texttt{name=(A,B)} with \texttt{i=}/\texttt{j=} in \texttt{get\_dynamical\_\allowbreak correlator} and every wrapper of it & the sites silently dropped & \texttt{TypeError} (finding 16) \\
\texttt{n\_gs>1} under \texttt{submode="SECTOR"} & the single-state value & \texttt{NotImplementedError} (finding 15) \\
\bottomrule
\end{tabular}
\normalsize
\end{center}

\subsection*{The 2026-09-24 third pass}

A fifth hunt (\texttt{docs/audit\_2026\_09\_24c\_hole\_hunt.md}, four
lenses over the single commit \texttt{867e2b4} that closed the second
pass) recorded 18 findings, six of them from that commit and twelve
older, and every one is fixed; two needed a rebuild of the compiled
extensions. Each item below names its finding.

\textbf{Results that are not comparable across this change.}

\begin{itemize}
\item \textbf{\texttt{gs\_energy\_fluctuation()} below full bond
  dimension, on every DMRG backend}, was set by truncating $H|\psi\rangle$
  to \texttt{maxm}. It is now $\lVert(H-\langle H\rangle)|\psi\rangle
  \rVert$ with an uncapped application: on an 8-site Heisenberg chain
  solved and measured at \texttt{maxm=3} it goes from 6.7e-03 to 0.344 on
  \texttt{"python"} and from 1.5e-02 to 0.347 on v3, and an exact state
  measured at \texttt{maxm=3} from 1.64 (v3) to $10^{-11}$.
  \texttt{mode="ED"} subtracts $\langle H\rangle$ first too, so its value
  at an exact eigenstate goes from the $10^{-7}$ roundoff floor to
  $10^{-15}$. With it, \texttt{gs\_energy(maxde=...)} stops later, since
  it used to stop on an under-reported number, and returns the refined
  energy it leaves on the chain, $-3.374933$ where it returned the
  unrefined $-3.279373$; a correlator afterwards measures that refined
  state instead of re-solving at the original \texttt{maxm} (\S3,
  findings 6 and 7).
\item \textbf{After \texttt{set\_gs()} of a state off the ground
  manifold} every submode on both modes measures it from its own energy.
  DMRG KPM and ED KPM, CVM, INV, ROOTN and TD move by $E_x-E_0$ (0.3 on
  the 3-site chain in a field, where the elastic line sat at $+0.3$), and
  ED \texttt{submode="ED"}, which read no state at all, now gives the set
  state's density, 2.834 away from what it returned on a 2.835 peak (\S6,
  findings 1 and 2).
\item \textbf{\texttt{promote\_to\_dense()} keeps the state the chain
  holds.} A state set in a sector and promoted with no read in between
  reads $-0.957107$, its own energy, where it read the sector ground
  energy $-1.616025$, and a carried state that is not the sector ground
  state is kept ($-1.131526$) where \texttt{"python"} re-swept it out of
  the sector to the global ground state $-1.857107$ (\S3, findings 3 and
  4).
\item \textbf{\texttt{set\_hamiltonian(H2, restart=False)}} now drops the
  stored ground state, so \texttt{gs\_energy()}, \texttt{vev()},
  \texttt{get\_excited()} and the direct KPM moments answer for H2:
  $-1.780099$ where they gave H1's $-1.616025$ on a 4-site chain (finding
  5).
\item \textbf{Weakly non-Hermitian operators.} The Hermiticity probe is
  scale-free, so an anti-Hermitian part below about $10^{-2}$ in absolute
  size is no longer called Hermitian: a weak loss term keeps its decay
  rate ($\mathrm{Im}\,E_0=-0.002134$, where v2 and v3 returned 0 and
  \texttt{"python"} a real part 3 to 18 per cent off), a non-Hermitian
  chain written in small units is solved as one, and
  \texttt{disentangle\_manifold} diagonalizes such an operator rather than
  its Hermitian part (\S2, finding 12).
\item \textbf{\texttt{"python"} operators whose coefficients are all
  below about $2\times10^{-7}$} were built as a different operator, a lone
  \texttt{1e-7*Sz0} as the zero MPO; \texttt{vev(1e-7*Sz0)} goes from 0 to
  $-1.8936\times10^{-8}$ (finding 13).
\item \textbf{v3 \texttt{TDVP\_GSE} from a start whose site 0 is one local
  basis vector} (any ladder operator or projector on site 0, or a product
  state) left site 0 frozen: \texttt{evolution\_ABA(A=C\_0)} goes from
  0.47 to $10^{-7}$ off exact, the public (Cdag\_0, C\_0) TD spectrum's peak
  from 0.081 to 0.47, and a N\'eel start is exact in 8 of 8 runs where up
  to 5 of 8 failed (\S7, finding 14).
\item \textbf{Every \texttt{mode="ED"} real-time route}
  (\texttt{evolve\_and\_measure}, \texttt{evolution\_ABA},
  \texttt{submode="TD"}) uses an exact propagator, and moves by $10^{-8}$
  to $10^{-6}$ on the small chains the tests use and up to $10^{-3}$ at
  \texttt{dt=0.2} on an 8-site chain with an extensive energy (\S7,
  finding 17).
\end{itemize}

\textbf{Faster, with no number changed.} The first read after
\texttt{set\_gs()} no longer solves the upper band edge and an energy
fluctuation it discarded (7.1\,s on a 24-site \texttt{"python"} chain for
a 0.04\,s result), and \texttt{"python"}'s upper band edge caps its local
Krylov dimension at 20, costing about one ground-state solve where it
cost 5 to 9 on a Heisenberg chain (findings 9 and 10). A malformed KPM
call raises before any ground-state work again, and a SECTOR call makes
no solve on the caller's chain (finding 11).

\textbf{And these now raise, or now work, where they used to do something
else:}
...
\textbf{Call} & \textbf{Was} & \textbf{Now} \\
\midrule
a misspelled keyword to \texttt{evolve\_and\_\allowbreak measure}/\allowbreak\texttt{evolution\_ABA} on DMRG & ignored, the run at the defaults & \texttt{TypeError}, as on ED (finding 15) \\
\texttt{h=} to \texttt{evolve\_and\_\allowbreak measure}/\allowbreak\texttt{evolution\_ABA} on ED & \texttt{TypeError}, ``multiple values for argument h'' & evolves under \texttt{h}, as on DMRG (finding 16) \\
\texttt{submode="SECTOR"} after \texttt{set\_gs()} of a state that is not its sector's ground state & the ground state's spectrum & \texttt{NotImplementedError} (finding 8) \\
\texttt{disentangle\_manifold} on an ED manifold & \texttt{AttributeError} for every operator & the eigenbasis (finding 18) \\
an unknown keyword to \texttt{gs\_energy\_\allowbreak fluctuation()} on DMRG & forwarded and dropped & \texttt{TypeError} (finding 7) \\
\bottomrule
\end{tabular}
\normalsize
\end{center}

\subsection*{The 2026-09-25 open items}

A sixth pass (\texttt{docs/audit\_2026\_09\_25\_open\_items.md}) took ten
of the items the five hunts had left open or recorded as unreviewed leads,
reproduced each on \texttt{8dd2198}, fixed it and handed every fix to a
reviewer briefed to refute it; one needed a rebuild of both compiled
extensions. Each item below names its entry in that record.

\textbf{Results that are not comparable across this change.}

\begin{itemize}
\item \textbf{A setting passed to a chain constructor} now takes effect: a
  12-site Heisenberg chain built with \texttt{maxm=4, nsweeps=10} on v3
  goes from $-5.1420906326$, the \texttt{maxm=30} answer, to
  $-5.1323602278$, the number the same request made by assignment returns,
  and a constructor \texttt{mode="ED"} answers every read that does not
  pass \texttt{mode=} itself by ED (the KPM ZZ peak of a 4-site Heisenberg
  chain in a field of 0.2, 0.205144 on \texttt{"python"} and 0.205151 on
  v3, both DMRG, becomes 0.205190). The same holds for
  \texttt{Thermal\_Spin\_Chain(..., mode="ED")}, which ran DMRG (\S1,
  init-kwargs).
\item \textbf{v2 and v3 Hamiltonians in small units} (largest coefficient
  below about $10^{-6}$): the ground state, the excited states, both band
  edges, the KPM spectra and \texttt{gs\_energy\_generalized} now match
  the unit-scale calculation, where a 6-site Heisenberg chain written as
  $sH$ returned the N\'eel $-1.25$ for $E_0/s$ against $-2.4936$ from
  $s\approx4\times10^{-7}$ (v3) and $10^{-7}$ (v2) down. Every v2 or v3
  calculation whose largest coefficient is below 1 now runs on $2^kH$,
  which moved nothing beyond the run-to-run noise at ordinary scales (\S2,
  small-units).
\item \textbf{Operators with coefficients below $10^{-8}$} keep their
  terms on every backend: \texttt{vev(eps*Sz0)/eps} at \texttt{eps=1e-9}
  goes from exactly 0 to $-0.189361$ on a 6-site chain in a field, and
  $E_0/s$ of a Hamiltonian at $s=10^{-8}$ from 0 to the right answer on ED
  and \texttt{"python"}. A term at or below $10^{-12}$ of its operator's
  largest coefficient is now dropped where the absolute rule kept it when
  it was above $10^{-8}$, and an anti-Hermitian part between about
  $10^{-10}$ and $10^{-8}$ of the largest coefficient now makes
  \texttt{gs\_energy()} take the non-Hermitian route (\S2,
  clean-threshold).
\item \textbf{\texttt{"julia\_live"} warm starts}:
  \texttt{set\_initial\_wf\_guess()} of an exact doublet member goes from
  $|\langle t|\mathrm{gs}\rangle|^2=0.0008$ (0.0008 to 0.68 over runs) to
  1.0000, and \texttt{set\_initial\_wf(x)} followed by
  \texttt{gs\_energy()} from the solved $-1.0$ to $\langle x|H|x\rangle$
  (\S3, julia-warm-start).
\item \textbf{\texttt{get\_gs(wf0=x, reconverge=False)} on a solved
  chain} returns x with \texttt{e0} = $\langle x|H|x\rangle$
  (0.0388284989 where it returned the stored $-2.5231886435$ on a 6-site
  \texttt{"python"} chain), and every later reader measures x;
  \texttt{get\_gs(wf0=x)} sweeps from x (\S3, get-gs-wf0). Since the
  ...
  \texttt{TypeError}.
\item \textbf{Readers after a correlator that followed
  \texttt{gs\_energy\_generalized()}} measure the generalized state: on a
  Hermitian chain never solved before, \texttt{e0} goes from $-2.493577$
  to $\lambda=-3.597994$ and $\langle S^z_0\rangle$ from 0 to $-0.4801$
  (6 sites, $A=1+0.8S^z_0$); on a non-Hermitian chain with no correlator
  before the generalized solve, solved first or not, \texttt{e0} goes from
  $-1.596396$ to $\lambda=-2.17581-0.213083i$ (\S3, generalized-cache).
  Since the 2026-09-25b pass \texttt{e0} is the generalized state's own
  energy instead of $\lambda$, which is kept as \texttt{lam\_generalized}
  (next subsection).
\item \textbf{\texttt{tc.MBChain} after
  \texttt{Thermal\_Spin\_Chain.get\_gs()} at $T>10^{-5}$}: on a 3-site
  chain at $T=1$, \texttt{gs\_energy()} goes from $-2.25$ to $-0.416388$
  and the KPM sum rule from $-0.166370$ to $-0.069291$ on the DMRG
  backends, and on \texttt{mode="ED"} \texttt{vev(Sz0*Sz1)} from 0 to
  $-0.069398$ and \texttt{gs\_energy()} from $-2.25$ to $-1.0$, the lowest
  eigenvalue (\S9, thermal-bypass). The 2026-09-25b stepper moved these
  again, to $-0.428230$ for \texttt{gs\_energy()}, $-0.071372$ for
  \texttt{vev(Sz0*Sz1)} (both exact) and $-0.071331$ (DMRG) and
  $-0.071188$ (ED) for the sum rule, and the switch is now at
  $T>10^{-5}W$, $W$ the spectral width.
\item \textbf{\texttt{gs\_energy(wf0=x, reconverge=False)} on a
  non-Hermitian chain} is $\langle x|H|x\rangle$ on \texttt{"python"}, v2
  and v3, where it returned the NH-DMRG $-1.596396$ (\S6, nh-injected).
\item \textbf{\texttt{submode="ROOTN"} on the lower-level route}
  (\texttt{get\_dynamical\_\allowbreak correlator\_MB} with a string name)
  honours \texttt{i=}/\texttt{j=}: at (1,1) on a 4-site \texttt{"python"}
  chain it goes from the (0,0) curve, $4.07\times10^{-2}$ off ED's (1,1)
  on a 0.103 peak, to $4\times10^{-16}$ (rootn-ij).
\end{itemize}

\textbf{And these now raise, where they used to do something else:}
...
\textbf{Call} & \textbf{Was} & \textbf{Now} \\
\midrule
a chain constructor keyword that is not a setting (a misspelling, the chain's state, a model-built operator list) & ignored & \texttt{TypeError} naming every offending key (init-kwargs) \\
\texttt{maxm=0} or an unknown \texttt{mode=} at construction & ignored & \texttt{ValueError}, as on assignment (init-kwargs) \\
an unknown keyword to \texttt{submode="ROOTN"} & ignored & \texttt{TypeError} (rootn-ij) \\
\texttt{get\_gs(best=True, wf0=x)} on a solved chain & the stored state & \texttt{TypeError}, as on a fresh chain (get-gs-wf0) \\
non-Hermitian \texttt{submode="KPM"} after \texttt{set\_gs()} & a spectrum built on the left state of an earlier solve & \texttt{RuntimeError} naming the missing left state (nh-injected) \\
\texttt{gs\_energy(wf0=x)} on a non-Hermitian chain & the NH-DMRG energy, x dropped & \texttt{TypeError}; \texttt{reconverge=False} takes x (nh-injected) \\
\bottomrule
\end{tabular}
\normalsize
\end{center}

\textbf{And these now work:} on \texttt{"julia\_live"}, \texttt{set\_gs()}
and \texttt{gs\_energy(wf0=x)} on a solved chain, which raised
\texttt{AttributeError} and \texttt{TypeError};
\texttt{gs\_energy\_generalized()} on a v3 MPO Hamiltonian, which raised
\texttt{AttributeError} ($-3.59799449$, the \texttt{MultiOperator} route's
value); and the first correlator after a plain NH solve no longer repeats
the solve.

\subsection*{The 2026-09-25b hole hunt}

A sixth hunt (\texttt{docs/audit\_2026\_09\_25b\_hole\_hunt.md}, four
lenses over the single commit \texttt{e7b1196} that fixed the open items
above) recorded 29 findings and two unreviewed leads, and its fix pass
found one more (finding 30). Three of the 29 came from that commit and the
rest are older, reached by probing next to it, and seventeen are one
family: a threshold in absolute energy units, which the relative term
cutoff of \texttt{e7b1196} let a Hamiltonian or an operator written in
small units reach for the first time. Every one is fixed, and both
compiled extensions were rebuilt. Each item below names its finding; the
record carries the reproduction that was actually run, and every number is
from it.

\textbf{Results that are not comparable across this change.}

\begin{itemize}
\item \textbf{A state whose norm is not one}, set with
  \texttt{set\_gs()}, \texttt{set\_initial\_wf()} or
  \texttt{set\_initial\_wf\_guess()}, or passed as \texttt{wf0=} with
  \texttt{reconverge=False}, is now normalized once, where it becomes the
  chain's, on every backend. Before, \texttt{e0}, the session's
  \texttt{vev()} and the energy fluctuation divided by
  $\langle x|x\rangle$ while KPM, CVM, TD, TDZ, ROOTN,
  \texttt{evolve\_and\_measure()} and, on \texttt{mode="ED"} and
  \texttt{"julia\_live"}, \texttt{vev()} did not, so each of those now
  drops by exactly $1/\langle x|x\rangle$. On a 4-site Heisenberg chain at
  $x=2s$, $s$ its ground state: the KPM sum rule of $(S^z_0,S^z_0)$ goes
  from 0.999995 to 0.249999, \texttt{evolve\_and\_measure(Sz0*Sz1)} at
  $t=0$ and ED's \texttt{vev(Sz0*Sz1)} from $-0.910684$ to $-0.227671$,
  \texttt{gs\_energy\_fluctuation()} on ED and \texttt{"julia\_live"} from
  9.696152 to 0 ($8\times10^{-9}$ on \texttt{"julia\_live"}), and
  \texttt{"julia\_live"}'s \texttt{get\_excited\_states(n=2)} from
  $[-6.464102, -0.957107]$ to $[-1.616025, -0.957107]$.
  \texttt{get\_gs()} hands back $x/\lVert x\rVert$, and \texttt{get\_rdm()}
  is the density matrix of the ray on all four backends, where it was
  $\rho/c^2$ for a state of norm $c$ (trace 0.25 at $c=2$). An explicit
  \texttt{wf=} to \texttt{evolve\_and\_measure}/\texttt{evolution\_ABA} is
  left as given (findings 1 and 11).
\item \textbf{Readers after \texttt{gs\_energy(wf0=x, reconverge=False)}
  on a route that resolves to ED} (the chain's \texttt{mode="ED"}, a
  \texttt{mode="ED"} call, or v3's own fallback below three sites) measure
  x, where the ED ground state stayed on the chain: on a 4-site Heisenberg
  chain with $0.4S^z_0+0.3S^x_3$, \texttt{vev(Sz0)} goes from
  $-0.2286763037$ to $\langle x|S^z_0|x\rangle$, $-0.1089088536$ and
  $0.0390101221$ for the random states drawn on \texttt{"python"} and v3.
  The returned energy stays the lowest eigenvalue (finding 2).
\item \textbf{A chain whose \texttt{mode} is \texttt{"DMRG"} no longer
  overrides a \texttt{mode="ED"} call}, so the ED cross-check is ED again:
  \texttt{gs\_energy(mode="ED")} on an 8-site Heisenberg chain in a
  staggered field at \texttt{maxm=2} goes from $-3.6734578613$ to the exact
  $-3.7040879103$, and \texttt{get\_correlation\_matrix(T=1)}, which is ED
  only, from occupations up to 0.149 off the Fermi function to
  $5.6\times10^{-16}$, and from 4.2\,s to 0.1\,s (finding 3).
\item \textbf{\texttt{gs\_energy(reconverge=True)} and
  \texttt{gs\_energy(maxde=...)} on a solved chain} reach the solver, with
  \texttt{get\_gs()} alike: the first sweeps from the stored state
  ($-4.2548001917$ to $-4.2548333556$ on a 10-site Heisenberg chain at
  \texttt{maxm=6} and one sweep, \texttt{"python"}), and the second refines
  ($-4.1431954920$ to $-4.2580352072$ at \texttt{maxm=3}, exact
  $-4.2580352073$) (finding 5, and the 2026-09-25 record's \texttt{maxde=}
  lead).
\item \textbf{\texttt{"julia\_live"} solves on a Hamiltonian whose
  couplings skip a site} leave the product state they were trapped in
  (\S3): the ground energy goes from $-0.5$ to $-1.0$ on the Heisenberg
  chain on the even sites of 6 and from $-1.5$ to $-3.232051$ on an 8-site
  chain with next-nearest-neighbour exchange only,
  \texttt{Thermal\_Spin\_Chain} at $T=0$ from $\langle H\rangle=-0.5$ to
  $-1.0$, \texttt{gs\_energy\_generalized} from $-0.833333$ to
  $-1.496331$, the non-Hermitian solve from $-1.4997+0.0994i$ to
  $-3.219401$, and the lowest excited state from $-3.190943$ to
  $-3.232051$, all now exact. Every other \texttt{"julia\_live"} solve
  moves within its run-to-run noise, and a warm 16-site solve costs about
  40 per cent more (finding 7).
\item \textbf{\texttt{gs\_energy()} and \texttt{e0} after
  \texttt{gs\_energy\_generalized()}} are the generalized state's own
  energy (\S4), where they were $\lambda$: on a 4-site Heisenberg chain in
  a field of 0.3, from $-2.162930$ to $-1.385986$ for $A=1+0.8S^z_0$, from
  $-0.808013$ to $-1.616025$ for $A=2\,\mathrm{Id}$ and from $-1.115761$ to
  $-1.565921$ for $A=1.5+0.4S^z_0$, on \texttt{"python"}, v3 and
  \texttt{"julia\_live"}, and on a non-Hermitian chain the biorthogonal
  energy of the pair. $\lambda$ is still the return value and is kept as
  \texttt{sc.lam\_generalized}. Every KPM, CVM, ROOTN and TDZ spectrum
  afterwards moves rigidly by $\lambda-E_{w_g}$ onto the lines TD and EX
  already gave ($-0.777$ for $A=1+0.8S^z_0$, $+0.808$ for
  $A=2\,\mathrm{Id}$), and on \texttt{"julia\_live"} the KPM window takes
  its lower edge from a solve of $H$, so the calls that raised ``KPM
  moments diverging'' return, and the $A=1+0.8S^z_0$ line's height goes
  from 1.2248 to 1.3037 (findings 8 and 10).
\item \textbf{\texttt{submode="CVM"} whenever
  $\eta\lVert B|\mathrm{GS}\rangle\rVert$ is small, and on exact solves at
  small $\eta$} (\S6): on a 6-site Heisenberg chain with $0.3S^z_0$, every
  frequency of $(S^z_0,S^z_0)$ at an operator scale of $10^{-4}$ was the
  flat $\eta\langle AB\rangle/\pi$ (0.0159 after dividing by
  $\epsilon^2$) and is now the ED curve, and the same for $sH$ at
  $s=10^{-4}$; \texttt{get\_kondo\_spectrum(mode="DMRG",\allowbreak{}
  submode="CVM",\allowbreak{} order=2,\allowbreak{} T=0)} at its default
  \texttt{delta=2e-6} on three spins at $J=10^{-3}$ goes from about
  $10^{-8}$, flat, to the exact curve on the same grid (4.6981 at the
  ends, 0.6763 in the middle); the on-line point $\omega_0=0.52710639$ of
  that 6-site chain goes from $1.5915\times10^{-4}$ to 13.2636 at
  $\eta=2\times10^{-3}$ and from $1.5915\times10^{-5}$ to 132.6333 at
  $\eta=2\times10^{-4}$, as ED; and on a 10-site chain on a 121-point grid,
  v3, 10 wrong points (up to 0.989 of the peak) become none at
  $\eta=0.05$, and 45 become none at $\eta=0.02$. At unit scale the
  tolerance tightens by $1/\lVert b\rVert$ (tenfold at $\eta=0.2$), so
  ordinary curves move within the old tolerance, the 6-site one from
  $6.8\times10^{-7}$ to $6.7\times10^{-9}$ of the peak on
  \texttt{"python"}. A truncated solve still stops early and warns, and is
  not converged either way: a 20-site chain at
  \texttt{maxm=cvm\_maxm=30} gives 0.30333897 at $\omega=0.3$ where it gave
  the flat 0.01193662. The 121-point grid takes 1214\,s where it took
  677\,s, and a truncated point about twice as long.
  \texttt{applyinverse} stops relative to the norm of the vector it
  inverts against, so \texttt{CVM\_explicit} on
  $(\epsilon S^z_0,\epsilon S^z_0)$ is $1.4\times10^{-6}$ of the peak off
  at every $\epsilon$ on v3, where it was 0.37 off at $\epsilon=10^{-4}$
  (findings 13 and 14).
\item \textbf{Non-adjoint pairs under \texttt{CVM\_explicit}, the
  non-Hermitian CVM and INV, and \texttt{cvm\_solver="variational"}}
  return their own $C[A,B]$ (\S6), where an absolute adjoint gate answered
  small pairs with another pair's density: \texttt{CVM\_explicit} on
  $(10^{-2}S^x_0,10^{-2}S^y_1)$ from 2.004 of the peak off to
  $3.5\times10^{-5}$, the variational solver on
...
\item \textbf{Every finite-temperature \texttt{Thermal\_Spin\_Chain}
  result} (\S9). At $T=0.5$, on 10 sites ($\langle H\rangle$,
  \texttt{"python"} DMRG at \texttt{maxm=64}) from $-2.777892$ to
  $-3.166391$ (exact $-3.166396$), on 12 sites from $-3.287097$ to
  $-3.849203$ (exact $-3.849213$), on 6 sites on \texttt{mode="ED"} from
  $-1.674494$ to $-1.800863$, and on 3 sites from $-0.754223$ to
  ...
  chain in a field $B$ at $T=B$, $\langle S^z_{\rm tot}\rangle$ goes from
  $-0.089480$, $-0.013179$ and $-0.002545$ at $B=10^{-2}$,
  $5\times10^{-3}$ and $10^{-3}$ to $-0.231057$ at all three (exact
  $-0.231059$). A float $T$ above 5 returns ($-0.071119$ at $T=5.5$) where
  it raised \texttt{ZeroDivisionError}, and a numpy scalar above 5 gets its
  temperature ($-0.055408$ at \texttt{np.float64(7.0)}) where it got
  $T=\infty$. The 2026-09-25 thermal-bypass numbers move with it,
  \texttt{tc.MBChain.gs\_energy()} at $T=1$ on 3 sites from $-0.416388$ to
  $-0.428230$ and \texttt{vev(Sz0*Sz1)} from $-0.069398$ to $-0.071372$. On
  \texttt{"julia\_live"} a 4-site chain at $T=0.5$ comes out
  $2.3\times10^{-5}$ off where the other backends are $2\times10^{-8}$ off,
  since its band-edge solves returned a narrower width (not examined
  further). The cost grows with $\beta W$: 8.6\,s where it was 4.9\,s on
  12 sites at $T=0.5$, and 19.8\,s where it was 0.7\,s for the field chain
  at $B=10^{-3}$ (findings 17 to 19).
\item \textbf{\texttt{get\_excited(n, mode="ED")} above 2000 states in
  small units} keeps every copy of a degenerate level: on a 12-site
  ferromagnet at $s=2\times10^{-7}$ the six-fold ground level came back
  with a magnon at $-2.715926s$ in place of a copy of $-2.75s$ in 8 of 8
  calls, and the calls that stalled at $s=10^{-5}$ and $10^{-6}$ return.
  Below an infinity norm of 1 the levels also move at roundoff
  (finding 20).
\item \textbf{\texttt{submode="ROOTN"} in small units} keeps its Lanczos
  basis, where it collapsed to the seed alone and returned one Lorentzian:
  the peak of $sC_s$ on a 6-site chain goes from 0.393371 to the
  unit-scale 0.157205 at $s=10^{-12}$ on ED and $s=10^{-10}$ on v3
  (finding 21).
\item \textbf{A KPM cross-correlator of two small operators} is no longer
  read as an autocorrelator: $C[\epsilon S^z_0,\epsilon S^z_3]/\epsilon^2$
  at $\epsilon=1.2\times10^{-10}$ and $10^{-11}$ was $C[S^z_3,S^z_3]$, 2.070
  of the peak off, and is now $C[S^z_0,S^z_3]$ to $10^{-14}$ on v2, v3 and
  \texttt{"python"} and to $7\times10^{-5}$, its run-to-run noise, on
  \texttt{"julia\_live"}; \texttt{get\_distribution} likewise, from 1.935
  to $10^{-14}$ (finding 23).
\item \textbf{VUMPS in small units or next to a large constant}, on v3 and
  \texttt{"python"}, reports \texttt{converged=True} where it could not:
  TFIM at $D=8$ from $s=0.1$ down to $10^{-8}$ and at constant offsets of
  $+10$, $+100$ and $-100$ on v3; below about $s=10^{-6}$ the energy moves
  too, $e_0/s$ from $-0.440126707917$ to $-0.440127030549$ at
  $s=10^{-8}$ (finding 24).
\item \textbf{v3 iDMRG in small units}: the energy density of TFIM is
  $4.8\times10^{-12}$ off the closed form at every $s$ down to
  $10^{-14}$, where it was 0.12 off at $s=10^{-13}$ and of the wrong sign
  at $10^{-14}$ (finding 27).
\item \textbf{NH-DMRG in small units or next to a large constant}, on v2,
  v3 and \texttt{"python"}, lands on the lowest level where it returned an
  excited one: $E_0/s$ on a 6-site Heisenberg chain with $0.3iS^z_0$ at
  $s=10^{-7}$ goes from $-0.9904-0.0568i$ to $-2.4610410218$ on v2,
  $E_0-c$ at $c=5\times10^5$ from $-1.9919-0.1109i$ to $-2.4610374186$ on
  \texttt{"python"}, and v3 at $s=10^{-13}$ from 0.570 off to
  $7\times10^{-15}$. At unit scale the tie window between Ritz values is
  about three times wider than it was, which moved nothing measured
  (findings 25 and 27).
\item \textbf{v2 and v3 Hamiltonians whose duplicate terms cancel} are
  solved at the scale of the MPO they build (\S2): $sH+S^z_0-S^z_0$ at
  $s=10^{-10}$ from 0.09 to 0.53 off in $E_0/s$ on a 6-site Heisenberg
  chain to $5\times10^{-15}$. On v3 the XX chain at $J=1$, whose realified
  coefficients are 0.5, now also solves at scale 2, moving within its
  run-to-run noise (finding 28).
\item \textbf{\texttt{"python"} operators whose largest coefficient is
  below 1} are built at unit scale (\S2): a Heisenberg MPO at $s=10^{-7}$
  goes from $1.1\times10^{-9}$ off relative to itself ($2.5\times10^{-4}$
  at $s=2^{-40}$) to $10^{-15}$ at every scale, on numpy with OpenBLAS, and
  every \texttt{"python"} \texttt{MultiOperator*MPS} in small units moves
  with it, $\lVert(sH)|u\rangle\rVert/s$ reading 0.897242774928 at every
  $s$ down to $10^{-16}$, where it read 1.432 at $10^{-16}$ (finding 30).
\item \textbf{\texttt{mpsalgebra.lowest\_energy\_arnoldi} in small units},
  a non-default route: $E_0/s$ on a 4-site Heisenberg chain at
  $s=10^{-6}$ goes from $-1.6150192976$ to $-1.6160253791$, and at
  $s=10^{-7}$ with \texttt{delta=s*1e-3} from $+0.3510023868$ to
  $-1.6160254038$ (lead
  \texttt{scale-\allowbreak arnolditk-\allowbreak absolute-\allowbreak stop}).
\end{itemize}

\textbf{New attributes.} \texttt{sc.lam\_generalized} holds $\lambda$
after \texttt{gs\_energy\_generalized()}; \texttt{tc.anneal\_step} and
\texttt{tc.anneal\_order} set the thermal stepper (\S9);
\texttt{sites.SETTINGS} is the explicit list of the 36 settings a chain
constructor accepts (\S1); and on \texttt{mode="ED"} the state gains
\texttt{normalize(tol=)} and \texttt{norm()}, as on the MPS backends
(\S2).

\textbf{Quieter changes.} With \texttt{verbose} on, v2 and v3 print one
line before each solve that runs at $2^kH$, saying that the energies
logged below it are $2^k$ times those of the operator (finding 29); the
returned energies never were. NH-DMRG's convergence certificate is
relative to the Hamiltonian's units and constant, so a run it used to
accept silently in small units or next to an offset now retries up to
\texttt{ntries} times and warns with its ``best relative residual''
(finding 26). \texttt{State.normalize()} on \texttt{mode="ED"} prints the
MPS backends' warning where it was silent below its floor (finding 22).
...
\textbf{Call} & \textbf{Was} & \textbf{Now} \\
\midrule
a constructor keyword naming one of nine attributes that are not settings (\texttt{hopping}, \texttt{hubbard}, \texttt{pairing}, \texttt{exchange}, \texttt{fields}, \texttt{resorder}, \texttt{resordered\_\allowbreak indexes}, \texttt{hubbard\_\allowbreak matrix}, \texttt{fit\_td}) & accepted; \texttt{hubbard=2.0} added $2\,\mathrm{Id}$ to the Hamiltonian & \texttt{TypeError} naming the key, \texttt{Thermal\_\allowbreak Spin\_\allowbreak Chain} included; the last five are gone from the chain (finding 4) \\
a misspelled keyword (\texttt{wf=}, \texttt{reconverg=}) to \texttt{gs\_energy()}, \texttt{get\_gs()} or \texttt{get\_excited(n=1)} on a solved chain & swallowed, the stored answer returned & \texttt{TypeError}, as on a fresh chain (finding 5) \\
a reader on a chain with no Hamiltonian & an error not naming the cause & \texttt{ValueError} saying to call \texttt{set\_hamiltonian()} (finding 6) \\
\texttt{gs\_energy(H=H2)} on a non-Hermitian chain & H2's pair stored as the chain's state & \texttt{TypeError} naming \texttt{nhdmrg(H=...)} (finding 9) \\
\texttt{get\_gs(wf0=x, reconverge=False)} on a route that resolves to ED & \texttt{TypeError} & $x/\lVert x\rVert$ (finding 2) \\
an MPS as \texttt{wf0=}, or a misspelled keyword, to \texttt{gs\_energy()} on a route that resolves to ED & ignored & \texttt{TypeError} (finding 2) \\
a set state whose norm is zero or not finite & \texttt{None} stored as the state & \texttt{ValueError} (finding 1) \\
an unknown single-site name on an ED spin chain (\texttt{ISy}, \texttt{Adag}) & \texttt{RuntimeError: No active exception to reraise} & \texttt{ValueError} naming it (finding 6) \\
\texttt{mode="ED"}, \texttt{submode="ED"} with the averaged manifold wider than \texttt{delta} & silent & \texttt{RuntimeWarning} (finding 15) \\
\texttt{Thermal\_Spin\_Chain} with a negative or NaN \texttt{T} set after construction & the ground state & \texttt{ValueError} (finding 19) \\
\texttt{get\_rdm(i=ns-1)} on \texttt{"julia\_live"} & \texttt{BoundsError} & the matrix (finding 12) \\
\texttt{get\_rdm()} or the bond entropy under an ED mode on \texttt{"julia\_live"} & the DMRG matrix, or \texttt{AttributeError} & \texttt{NotImplementedError}, as on the other backends (lead \texttt{session-\allowbreak julia-\allowbreak ed-\allowbreak guard-\allowbreak carveout-\allowbreak rdm-\allowbreak bond-\allowbreak entropy}) \\
\texttt{CVM\_explicit}, or the non-Hermitian CVM/INV, on a non-adjoint pair & \texttt{NotImplementedError} & $C[A,B]$ (finding 16) \\
\bottomrule
\end{tabular}
\normalsize
\end{center}
...
\subsection*{The 2026-09-26 Kondo exchange diagram}
...
Hamiltonian puts it at $eV-(\epsilon_f-\epsilon_m)$ (\S17, ``Where the
exchange log sits''). \texttt{get\_kondo\_spectrum} now uses the latter
on both \texttt{mode="ED"} and \texttt{mode="DMRG"}. Every third-order
spectrum with an inelastic transition out of an occupied state moves, by
up to 5--20\% of the third-order term at $T=0$ and a few per cent of the
total at 1~K (the Fig.~7d 10~T overshoots from 1.226/1.183 to
1.248/1.203); zero-bias values, a free $S=1/2$ at $B=0$,
\texttt{order=2} and the potential term do not. Results from before are
not comparable. The lower-level
\texttt{kondospectrumtk.twotime.kondo\_term\_from\_two\_time} now takes
\texttt{(t, G, Gx)} triples and raises on the old \texttt{(t, G)} pairs,
and its kernel \texttt{K\_W} is replaced by the one-sided \texttt{K\_F}.

\subsection*{The 2026-09-26 review of \texttt{"python"} real-time evolution}

Not an audit: a check of \texttt{itensor\_version="python"}'s real-time
evolution against exact propagation. The integrators themselves were
right. Two-site and one-site TDVP reproduce $e^{-iH\,dt}$ to $10^{-11}$ at
full bond dimension on spin-1/2 (long-range and Dzyaloshinskii-Moriya),
spin-1, Jordan-Wigner fermion and boson chains, at real and complex $dt$,
and below full bond dimension they track ED as closely as v3 does. Three
things around them changed numbers:
\begin{itemize}
\item \textbf{The Krylov exponentiator} behind every \texttt{"python"}
  TDVP route (TD, TDZ, \texttt{evolve\_and\_measure},
  \texttt{evolution\_ABA}, METTS, the infinite-chain window, the Kondo
  two-time construction) now stops when
  ...
  width equal to 100); such a step is now split. This closes the
  ``absolute Krylov error goal'' the 2026-09-25 record left open, on
  \texttt{"python"}: $C[\epsilon S^z_0,\epsilon S^z_3]/\epsilon^2$ under
  \texttt{submode="TD"} was $8.8\times10^{-2}$ off $C[S^z_0,S^z_3]$ at
  $\epsilon=10^{-8}$ and $0.24$ at $10^{-9}$, and is $10^{-11}$ at both,
  and a N\'eel quench under $sH$ at $dt/s$ was $0.48$ off ED at
  $s=10^{-12}$ and is at the $2.9\times10^{-6}$ of $s=1$. At ordinary
  units results move below $10^{-10}$. v3's \texttt{applyExp} is vendored
  ITensor and keeps its absolute goal.
\item \textbf{\texttt{evolve\_and\_measure} and \texttt{evolution\_ABA}}
  on \texttt{"python"} (TDVP, TDVP\_GSE, TEBD) and on v3 TEBD now put the
  state back to its initial norm after every step, as v3's TDVP and every
  \texttt{"julia\_live"} loop already did. Real-time evolution conserves
  the norm, so what the truncation removes from it is error, and it used
  to scale every later $\langle\psi(t)|O|\psi(t)\rangle$: on a 12-site
  N\'eel quench at \texttt{maxm=8}, $\langle\psi|\psi\rangle$ fell to
  0.982 by $t=5$ under TDVP and to 0.970 under TEBD, and $\langle
  H\rangle$ drifted by $4.9\times10^{-2}$ where v3 drifted by
  $1.8\times10^{-4}$ (now the same $1.8\times10^{-4}$ on both). Nothing
  moves where nothing truncates, and the \texttt{quench} loops behind
  \texttt{submode="TD"} always renormalized.
\item \textbf{\texttt{tevol\_method="TDVP\_GSE"} on \texttt{"python"}}
  now normalizes each Krylov vector $H^k|\psi\rangle$ before choosing the
  new directions, as v3's expansion does. The choice used to depend on
  $\lVert H\rVert$: that 8-site N\'eel quench was $1.0\times10^{-4}$ off
  ED at $s=1$ and $1.3\times10^{-3}$ at every $s\le10^{-4}$, and is
  $2.7\times10^{-5}$ at all of them.
\end{itemize}
Results from before are not comparable where these apply. The
regressions are in
\texttt{tests/\allowbreak test\_pyitensor\_time\_evolution\_review.py}.
One thing was measured and left as it is, on every backend: under
\texttt{TDVP\_GSE} the bond dimension stops growing after
\texttt{tdvp\_gse\_sweeps} steps (3 by default), so a quench whose
entanglement keeps growing saturates. On the 12-site N\'eel quench the
largest error up to $t=5$ stays near $2\times10^{-3}$ from
\texttt{maxm=16} to 64,
where \texttt{"TDVP"} reaches $9\times10^{-7}$ at \texttt{maxm=64}. Raise
\texttt{tdvp\_gse\_sweeps} to the number of steps, or use
\texttt{"TDVP"}, for such a run.
