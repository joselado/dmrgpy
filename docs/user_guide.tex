...
ground-state solver: the v3 C++ backend's \texttt{gs\_method="idmrg"}
...
algorithm) computes the energy density only -- \texttt{vev}/
\texttt{correlator} there raise \texttt{NotImplementedError};
\texttt{gs\_method="vumps"} supports \texttt{vev}/\texttt{correlator} on
BOTH backends (see "Static correlators" below). Relative speed depends
...
\texttt{itensor\_version="python"} supports this under both
\texttt{gs\_method="vumps"} (the default, see below) and
\texttt{gs\_method="idmrg"} (described in this paragraph);
\texttt{itensor\_version=3} supports it under \texttt{gs\_method="vumps"}
only -- calling \texttt{vev}/\texttt{correlator} on an
\texttt{itensor\_version=3} chain with \texttt{gs\_method="idmrg"} still
raises \texttt{NotImplementedError} (even after a successful
\texttt{gs\_energy()}), since that solver's result keeps no
per-sublattice tensor list to build a correlator from:
...
equivalent), and only on \texttt{itensor\_version="python"} at that
(\texttt{itensor\_version=3} has no correlator machinery under
\texttt{gs\_method="idmrg"} at all, see above); conversely
