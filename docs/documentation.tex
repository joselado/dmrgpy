...
stack, see below) \texttt{get\_dynamical\_correlator} -- no excited
...
above, \texttt{Infinite\_Many\_Body\_Chain.get\_dynamical\_correlator}
deliberately does \textbf{not} add any new Chebyshev-recursion code to
\texttt{idmrg.py}. $H$ is extensive/unbounded in the thermodynamic
limit, so there is no infinite-chain analogue of the transfer-matrix
formalism \texttt{vev}/\texttt{correlator} use -- a literal KPM
expansion of the full infinite $H$ has no meaning (the same reason
\texttt{apply\_mpo} restricts \texttt{W\_bulk} to \emph{bounded}
periodic operators, see above). Instead,
\texttt{get\_dynamical\_correlator} builds an ordinary finite,
...
\texttt{get\_dynamical\_correlator}'s own docstring (and
