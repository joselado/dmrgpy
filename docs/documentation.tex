...
\texttt{Chain::idmrg\_ground\_state} directly -- originally a
line-by-line C++ port of the same algorithm (built from the start with
fresh Index objects on every automaton tensor to sidestep the
duplicate-Index bug class described below, rather than needing the same
retrofit), but \textbf{no longer equivalent}: the Python side has since
gained the wavefunction prediction, the theta-cell extraction and the
energy baseline described below, and none of the three has been ported.
The cross-backend test
(\texttt{test\_itensor\_version3\_matches\_python\_backend}) compares
only the energy density, which was never the part that differed, so it
does not guard this -- see \texttt{CLAUDE.md} for the standing note. It is
used for the energy density only; it has no static-correlator support, so
...
Hamiltonian, which has no VUMPS equivalent). \texttt{itensor\_version=3}
now mirrors this too for \texttt{gs\_method="vumps"} (see below) --
\texttt{gs\_method="idmrg"} there still has no correlator support at all
(\texttt{IdmrgResult} keeps no per-sublattice unit cell) --
