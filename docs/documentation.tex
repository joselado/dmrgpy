...
nothing. Filling half the schedule instead measures \textbf{2.0x} at
\texttt{maxm=60} and \textbf{2.15x} at \texttt{maxm=90} on that model, for
the same energy to $3\times10^{-8}$ and $4\times10^{-9}$ (BLAS pinned to
one thread on two dedicated cores, as any timing in this repo must be).
Rerun it with \texttt{examples/groundstate/bond\_dimension\_ramp}.
