...
\textbf{Infinite-boundary-condition (IBC) window construction, first
step toward a real-time dynamical correlator
...
\textbf{What this covers, and what doesn't yet}: this is Phase 0-1 of
the paper's method only -- build and validate the IBC-capped window
itself (\texttt{tests/test\_idmrg\_window.py}). Perturbing the window
($\hat A^\dagger_0$/$\hat B_0$ at its center), time-evolving both
branches (reusing \texttt{pyitensor/tdvp.py}'s existing Krylov
machinery, seeded with these same environment caps rather than
\texttt{tdvp.py}'s own \texttt{(None,None)} open-boundary seed),
reading off $S(x,t)$ for every distance $x$ from a single pair of
shifted overlaps (Eq.~7 of the paper), and Fourier-transforming to
$S(k,\omega)$ are follow-up work, not implemented here.
