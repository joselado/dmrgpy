...
see \texttt{examples/non\_hermitian/nhdmrg\_generalized\_benchmark}, which
cross-checks it against \texttt{mpsiram\_generalized} (ARPACK mode 2
needs no adaptation at all for a non-Hermitian primary operator, only its
own $M$ positive-definite precondition was ever required) on a
non-Hermitian test problem, with the same accuracy/speed pattern as the
Hermitian benchmark.
