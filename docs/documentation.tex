...
One exception to that shared-surface rule: \texttt{dmrg.py::dmrg\_generalized}
(exposed as \texttt{Chain.gs\_energy\_generalized}/
...
this backend (\texttt{Many\_Body\_Chain.gs\_energy\_generalized} raises
\texttt{NotImplementedError} for \texttt{itensor\_version} 2/3/
\texttt{"julia\_live"}) --- unlike every other pyitensor feature described
above, \texttt{mpscpp2}/\texttt{mpscpp3}'s \texttt{chain\_session.h} have
no analogous session method to fall back to, so there is no "third
option, no separate code path" here yet. See
\texttt{examples/groundstate/dmrg\_generalized\_benchmark}, which
cross-checks it against \texttt{algebra/arpacktk.py}'s pre-existing
ARPACK-mode-2 route (\texttt{mpsiram\_generalized}) on the same test
problem.
