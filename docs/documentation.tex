...
solver, reached via \texttt{self.gs\_method="vumps"}
(\texttt{itensor\_version="python"} only -- checked \emph{before} the
\texttt{itensor\_version}/\texttt{gs\_method} combination is dispatched
in \texttt{gs\_energy}, alongside the existing \texttt{itensor\_version}
check) rather than \texttt{pyitensor/idmrg.py}'s growing algorithm.
...
\texttt{itensor\_version=3} already sets (no correlator support there
yet, for either \texttt{gs\_method}) --
\texttt{excitation\_energies}/\texttt{excitation\_gap} are the opposite:
they require \texttt{gs\_method="vumps"} (see below), since the
tangent-space excitation ansatz needs exactly the mixed-gauge
\texttt{\{AL,AR,C,GL,GR\}} \texttt{VUMPSResult} carries, which
...
\texttt{gs\_method="vumps"}, the opposite requirement from
\texttt{vev}/\texttt{correlator}/\texttt{local\_excitation\_gap} -- see
above): a tangent-space vector
