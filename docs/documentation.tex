...
\texttt{IDMRGResult}) holds the converged state; since it is a
grouped-supersite representation with no per-sublattice tensor list,
\texttt{vev}/\texttt{correlator}/\texttt{local\_excitation\_gap} all
remain \texttt{gs\_method="idmrg"}-only, mirroring the precedent
\texttt{itensor\_version=3} already sets for the same methods --
