...
trips are confined to three $O(1)$/$O(\chi)$ places (the Lanczos
$\alpha/\beta$ coefficients, the singular-value vector \texttt{svd.py}'s
truncation rule branches on, and measurement results); and almost
...
There is deliberately no \texttt{jax.jit} on the hot path: eager dispatch
has a per-call floor ($\sim$0.35\,ms on an H200), which is why the device
cannot win below $\chi \sim 120$, and \texttt{jax.jit} retraces per shape,
which DMRG's growing bonds defeat --- the same trap \texttt{kernels.py}
documents for numba.
