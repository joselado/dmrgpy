...
`vev`/`correlator`/`local_excitation_gap` all raise `NotImplementedError` under `gs_method="vumps"` (VUMPS's own converged state is a grouped-supersite representation with no per-sublattice `U_list`, see `pyitensor.vumps.VUMPSResult`'s own docstring) — but `excitation_energies`/`excitation_gap` (below) *require* `gs_method="vumps"` specifically, the reverse restriction: they need `VUMPSResult`'s own mixed-gauge `{AL,AR,C,GL,GR}`, which the growing algorithm's `IDMRGResult` (`gs_method="idmrg"`, the default) has no equivalent of.
