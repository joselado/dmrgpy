...
Two things do not follow along. `excitation_energies`/`excitation_gap` raise `NotImplementedError` on either backend: the tangent-space ansatz is built on the reach-1 environment triple, so there is no sequential route for it to take — the ground state on that same chain still works, so rewrite it on a longer unit cell if you need the excitations. And on `itensor_version=3`, `vev`/`correlator` raise for anything the sequential solver answered — a pre-existing gap of that backend (`Chain::vumps_onsite_expectation`/`vumps_two_point_correlator` read the grouped snapshot, and `vms_ground_state` has never had a static-observable port) that was previously reachable only at `n_uc>2` and that a long-range Hamiltonian at `n_uc<=2` now reaches too; use `itensor_version="python"` for those.
