...
`itensor_version="python"` (default) or `3` (`ic = infinitechain.Infinite_Spin_Chain(["1/2"], itensor_version=3)`) are both supported — `2` and `"julia_live"` have no iDMRG port and also raise `NotImplementedError`. `ic.gs_method` (`"vumps"` by default, on EITHER backend since 2026-08-08 — see below) picks the ground-state solver: the v3 C++ backend's `gs_method="idmrg"` (`mpscpp3/chain_session.h`'s `Chain::idmrg_ground_state`, a line-by-line port of `pyitensor/idmrg.py`'s own growing algorithm) computes the energy density only — `vev`/`correlator` there raise `NotImplementedError`; `gs_method="vumps"` supports `vev`/`correlator` on BOTH backends (see "Static correlators" below). Relative speed depends on the model: v3's own `idmrg_ground_state` is ~2.6-2.7x slower than `"python"`'s for a gapless (critical) chain (the local 2-site solve needs close to its full Krylov dimension every macro-iteration), but can be *faster* for a gapped model, where that same solve converges quickly — see `docs/documentation.md`'s iDMRG section for benchmark numbers.
