...
**Cost.** The eigenproblem `excitation_energies` solves has dimension `D*D*(d_g-1)`, so its cost grows quickly with the converged bond dimension. On `itensor_version="python"` two things bound that: the momentum-dependent channel resolvents are built once per momentum and cached (they do not depend on the excitation tensor, so rebuilding them per matrix element was pure waste), and above a size threshold the eigenproblem is solved by Lanczos on the effective Hamiltonian's action rather than by assembling that matrix at all. Neither changes any returned number — the two solver paths are cross-checked against each other in `tests/test_infinite_chain.py` — but a momentum scan at a large `D` is substantially cheaper than it was. `itensor_version=3` still assembles the matrix (and still rebuilds its resolvents per matrix element), so the two backends now differ in cost structure for this call, in the `"python"` backend's favour at large `D`.
...
**`itensor_version=3` currently raises here, on purpose.** The v3 window tiles the raw per-micro-step `idmrg_U_` factors rather than the gauge-consistent unit cell every other v3 static observable uses, so its `S(x,t)` is quantitatively wrong for *any* operator, spin included: the exact `S(x,0) == correlator(...)` identity misses by up to ~1.7e-1 on a plain Heisenberg chain whose energy density the two backends agree on to 6.7e-11. Shape and finiteness look fine, which is how it went unnoticed. Rather than hand back numbers like that, `td_dynamical_correlator` raises `RuntimeError` for `itensor_version=3` and `Chain::td_dynamical_correlator_window` throws unless a caller explicitly opts in (`Chain::set_allow_defective_window`, used only by the tests that pin the defect). Use `itensor_version="python"`, which is exact on that identity — see `docs/known_issue_v3_window_gauge.md` for the diagnosis and the fix, and `examples/idmrg/td_dynamical_correlator_python_VS_v3/main.py`, which now measures the defect instead of asserting agreement.
