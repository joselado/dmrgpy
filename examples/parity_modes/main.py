...
def get_den_par(mu=0.,U=0.5,Bz=1.0,delta=0.5,fpmode="full"):
    ...
    p = wf.get_fermionic_parity(fpmode=fpmode) # parity of the state
    n = wf.dot(sum(fc.N)*wf) # total number of electrons 
    return n,p


mus = np.linspace(-1.,0.,20)
nps_full = np.array([get_den_par(mu=mu,fpmode="full") for mu in mus])
nps_ite = np.array([get_den_par(mu=mu,fpmode="iterative") for mu in mus])

ns = nps_full[:,0] # densities
ps_full = nps_full[:,1] # parities
ps_ite = nps_ite[:,1] # parities
...
plt.subplot(1,2,2)
plt.scatter(mus,ps_full,c="red",s=100,label="Full mode")
plt.scatter(mus,ps_ite,c="blue",s=50,label="Iterative mode")
plt.legend()
plt.xlabel("Chemical potential")
