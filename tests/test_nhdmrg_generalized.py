...
nhdmrg_generalized() for the self-consistent Lagrange-multiplier
algorithm (complex lambda, biorthogonal Rayleigh quotient).

Only implemented for itensor_version="python" so far (see CLAUDE.md's
"Implement in pyitensor first" scoping) -- mpscpp2/mpscpp3 have no
analogous session method yet.
...
from dmrgpy.pyitensor.mpsalgebra import inner as pyi_inner
...
def _setup(fc, maxm=40, nsweeps=14, cutoff=1e-12):
    fc.setup_python()
    fc.maxm = maxm
    fc.nsweeps = nsweeps
    fc.cutoff = cutoff
...
def test_gs_energy_generalized_nonhermitian_matches_ed():
...
def test_nhdmrg_generalized_session_matches_ed_and_is_biorthogonal():
    """Drive the pyitensor session method directly (not through the
    top-level retry wrapper) to check both the eigenvalue and the
    biorthogonal normalization <psi_L|psi_R>=1 of the returned pair."""
    fc, h, m = _nh_generalized_fermion_problem(n=4, seed=2)
    w = _nh_generalized_ground_truth(fc, h, m)
    _setup(fc)
    lam, psil, psir = fc._session.nhdmrg_generalized(
        h.to_terms(), h.get_dagger().to_terms(), m.to_terms(),
        krylovdim=20, restarts=3)
    assert abs(lam - w[0]) < 1e-4
    assert abs(pyi_inner(psil, psir) - 1.0) < 1e-6


def test_nhdmrg_generalized_residual_is_small():
    ...
    fc, h, m = _nh_generalized_fermion_problem(n=4, seed=2)
    _setup(fc)
    terms_h, terms_hadj, terms_a = h.to_terms(), h.get_dagger().to_terms(), m.to_terms()
    lam, psil, psir = fc._session.nhdmrg_generalized(
        terms_h, terms_hadj, terms_a, krylovdim=20, restarts=3)
    Hop = fc._session.build_operator(terms_h)
    Aop = fc._session.build_operator(terms_a)
    r = fc._session._apply_mpo(Hop, psir)
    a = fc._session._apply_mpo(Aop, psir) * lam
    from dmrgpy.pyitensor.mpsalgebra import sum as pyi_sum
    resid = pyi_sum(r, a * (-1.0))
    resid_norm = np.sqrt(abs(pyi_inner(resid, resid)))
    assert resid_norm < 1e-3


def test_nhdmrg_generalized_identity_matches_plain_nhdmrg():
...
def test_gs_energy_generalized_nonhermitian_leaves_computed_gs_true():
...
def test_gs_energy_generalized_nonhermitian_requires_python_backend():
    """The non-Hermitian generalized solver only exists in pyitensor so
    far -- mpscpp2/mpscpp3 have no Chain.nhdmrg_generalized yet."""
    fc, h, m = _nh_generalized_fermion_problem(n=4, seed=2)
